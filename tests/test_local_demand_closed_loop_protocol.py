...
from scripts.cluster.run_tsc_external_local_demand_closed_loop_development_shard import (
    shard_identities,
)
...
def test_v75_matrix_is_paired_and_evenly_sharded() -> None:
    protocol = _protocol()
...
def test_v75_execution_candidate_is_the_predeclared_450_second_choice() -> None:
    candidate = development_candidate(_protocol())

    assert candidate.key == CANDIDATE_KEY

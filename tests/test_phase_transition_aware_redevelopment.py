...
from cf_h2o.eval.traffic_signal_phase_transition_aware_redevelopment import (
    ACTION_AWARE_POLICY,
    action_aware_candidate,
    all_rollout_identities,
    phase_transition_alignment,
)
from scripts.cluster import (
    run_tsc_external_phase_transition_aware_redevelopment_shard as shard_runner,
)
...
V87_PROTOCOL_PATH = PROJECT_ROOT / (
...
def test_v87_freezes_one_stay_aware_candidate_on_54_development_seeds() -> None:
    v86 = _read(V86_PROTOCOL_PATH)
    v87 = _read(V87_PROTOCOL_PATH)
    candidate = action_aware_candidate(v87, v86)
    identities = all_rollout_identities(v87)

    assert candidate.key == ACTION_AWARE_POLICY
    ...
    assert {identity[3] for identity in identities} == {ACTION_AWARE_POLICY}
    assert set(v87["development"]["seeds"]).isdisjoint(
        v87["development"]["sealed_prospective_seeds"]
    )
...
def test_v87_six_shards_partition_every_seed_once() -> None:
    protocol = _read(V87_PROTOCOL_PATH)
    shards = [
...
def test_v87_spawn_workers_start_from_snapshot_root(monkeypatch) -> None:
    observed: list[Path] = []
    monkeypatch.setattr(
        shard_runner.os,
        "chdir",
...
def _selection_rule() -> dict[str, object]:
    return _read(V87_PROTOCOL_PATH)["development"]["selection_rule"]


def test_v87_aggregate_collision_gate_reports_seed_discordance() -> None:
...
def test_v87_aggregate_collision_gate_rejects_total_method_excess() -> None:
    summary = _candidate_summary(
        policy="candidate",

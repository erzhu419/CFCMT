...
def test_v10_route_spec_keeps_complete_demand_contract() -> None:
    spec = build_spec(
        snapshot_root=Path("/snapshot"),
        package_root=Path("/package-v10"),
        expected_package_manifest_sha256="a" * 64,
        remote_output_root=Path("/remote/route-v8"),
        local_output_root=Path("/local/route-v8"),
    )
    ...
    assert spec["ram_resource_family"].endswith("v10")


def test_v10_route_launcher_is_directly_executable() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "scripts/cluster/launch_eth_boston_route_admission_v8.py",
            "--help",
        ],
        text=True,
        capture_output=True,

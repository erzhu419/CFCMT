...
from scripts.cluster.launch_tsc_multicity_mechanism_compatibility_gate import (
    CONFIG_RELATIVE,
    MODULE,
    RESOURCE_FAMILY,
    SIGNATURE_PREFIX,
...
def _specs() -> list[dict]:
    return build_specs(
        ...
        remote_output_root=Path("/remote/v148"),
        local_output_root=Path("/local/v148"),
        cache_workers=8,
        fit_workers=8,
        module=MODULE,
...
def test_v148_config_and_specs_cover_all_cities_without_node004() -> None:
    ...
    assert all(spec["ram_mb"] == 65_536 for spec in specs)


def test_v148_launcher_is_directly_executable() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "scripts/cluster/launch_tsc_multicity_mechanism_compatibility_gate.py",
            "--help",
        ],
        text=True,
        capture_output=True,

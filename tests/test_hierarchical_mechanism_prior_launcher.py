...
from scripts.cluster.launch_tsc_hierarchical_mechanism_prior import (
    CONFIG_RELATIVE,
    FIT_WORKERS,
    PROJECT_ROOT,
    RAM_MB,
...
def test_v152a_config_and_launcher_contract() -> None:
    ...
    for spec in specs:
        command = spec["cmd"]
        assert command.count("--v151a-rejection ") == 1
        assert command.count("--v151a-rejection-sha256 ") == 1
        assert "traffic_signal_hierarchical_mechanism_prior target" in command

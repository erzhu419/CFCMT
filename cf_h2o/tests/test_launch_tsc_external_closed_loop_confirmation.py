...
from scripts.cluster.launch_tsc_external_closed_loop_confirmation import (
    NODES,
    build_specs,
)
...
def test_closed_loop_launcher_builds_complete_six_node_matrix(tmp_path):
    ...
    assert len(specs) == 56
    assert len({spec["signature"] for spec in specs}) == 56
    ...
    assert all("--seed 8081" in spec["cmd"] or "--seed 9091" in spec["cmd"] for spec in specs)
    assert all("--expected-sumo-version 1.22.0" in spec["cmd"] for spec in specs)

"""Static-topology execution veto for frozen CFCMT proposals."""
...
VETO_PROTOCOL = "target-simulator-450s-static-topology-veto-v1"
ARTIFACT_PROTOCOL = "cfcmt-static-topology-target-veto-model-v1"


@dataclass(frozen=True)
class TopologyVetoConfig:
    enabled: bool
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("invalid topology veto configuration")


class TopologyTargetVeto:
    """Veto an existing proposal using current state and static topology."""
    ...
    def __init__(
        ...
        *,
        ...
        expected = (*self.candidate_feature_names, *topology_context.feature_names)
        if (
            ...
            raise ValueError("topology model feature contract changed")
    ...
    def _combined_matrix(self, dataset: MechanismDataset) -> np.ndarray:
        ...
        if missing:
            raise KeyError(f"topology dataset missing features: {missing}")
        tls_ids = tuple(str(value) for value in dataset.metadata.get("row_tls", ()))
        if len(tls_ids) != dataset.size:
            raise ValueError("topology runtime TLS metadata changed")
        columns = np.asarray(
        ...
        combined = np.concatenate([candidate, topology], axis=1)
        if (
            combined.shape != (dataset.size, len(self.model.feature_names))
            or not np.isfinite(combined).all()
        ):
            raise ValueError("topology runtime matrix changed")
        return combined
    ...
    def evaluate(
        ...
        *,
        ...
        if not 0 <= candidate < dataset.size or not 0 <= reference < dataset.size:
            raise IndexError("topology action index is outside the group")
        ...
        if (
            ...
            raise ValueError("topology candidate/reference is invalid")
        ...
        return {
            ...
            "runtime_tls_id": str(dataset.metadata["row_tls"][candidate]),
    ...
    def diagnostics(self) -> dict[str, Any]:
        return {
            "protocol": VETO_PROTOCOL,
            "role": "static_topology_execution_veto_only",

"""Seed-blocked OOF diagnostic for hierarchical action-state residuals."""
...
from scripts.cluster.freeze_tsc_external_v9_hierarchical_action_state_residual_diagnostic import (
    MODEL_FAMILY,
    PROTOCOL,
)
...
RESULT_PROTOCOL = "tsc-v83r79-hierarchical-action-state-residual-result-v1"
AUTHORIZATION_DECISION = "authorize_hierarchical_action_state_residual_closed_loop_development"
REJECTION_DECISION = "retain_phase_pressure_and_reject_hierarchical_action_state_residual"
...
def _process_fold(heldout_seed: int) -> dict[str, Any]:
    if _PROCESS_FOLD_CONTEXT is None:
        raise RuntimeError("hierarchical residual process context is not initialized")
...
def run_diagnostic(
    *,
    ...
    evidence_ok = (
        ...
        and v74_protocol.get("protocol") == V74_PROTOCOL
        and v74_result.get("protocol") == V74_RESULT_PROTOCOL
        and v74_result.get("status") == "PASS"
        and v74_result.get("decision") == V74_REJECTION_DECISION
        and v74_result.get("integrity_gate", {}).get("passed") is True
        and v74_result.get("diagnostic_protocol_sha256")
        ...
        and _sha256(v74_protocol_path) == frozen.get("v74_protocol_sha256")
        and _sha256(v74_result_path) == frozen.get("v74_result_sha256")
    ...
    if not evidence_ok:
        raise ValueError("hierarchical action-state residual evidence changed")
    ...
    if (
        ...
        raise ValueError("hierarchical residual development partition changed")
    ...
    if len(selected_specs) != 1:
        raise ValueError("hierarchical residual scenario is not unique")
    ...
    if observed_seeds != set(seeds):
        raise ValueError("hierarchical residual contrast seed set changed")
    ...
    if (
        ...
        != {MODEL_FAMILY, HIERARCHICAL_RESIDUAL_MODEL_FAMILY}
    ):
        raise ValueError("hierarchical residual model grid changed")
    ...
    fit_contract = all(
        ...
        is False
        and result["fit_diagnostics"].get("cell_count", 0) > 0
        for fold in folds
        for result in fold["model_results"]
    )
    ...
    integrity = {
        ...
        "hierarchical_residual_fit_contract_exact": fit_contract,
    ...
    return {
        ...
        "parent_v74_result_sha256": _sha256(v74_result_path),
        ...
        "offline_comparators": dict(v74_result.get("offline_comparators", {})),
        "parent_distributional_rejection": {
            "decision": v74_result["decision"],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError(
            f"refusing to overwrite hierarchical residual result: {args.out}"
        )

"""Run one frozen v71 demand-conditioned 450-second closed-loop rollout."""
...
PROTOCOL = "tsc-v71r67-external-v9-demand-conditioned-closed-loop-development-v1"
RESULT_PROTOCOL = (
    "tsc-v71r67-external-v9-demand-conditioned-closed-loop-development-rollout-v1"
)
TRAINING_PROTOCOL = "tsc-v70r66-external-v9-demand-conditioned-450s-veto-training-v1"
TRAINING_RESULT_PROTOCOL = (
    "tsc-v70r66-external-v9-demand-conditioned-450s-veto-freeze-v1"
)
JOINT_AUDIT_PROTOCOL = "tsc-v70r66-demand-conditioned-450s-veto-joint-audit-v1"
JOINT_AUDIT_DECISION = (
    "authorize_demand_conditioned_450s_closed_loop_development_only"
)
ANALYSIS_STAGE = "demand_conditioned_closed_loop_development"
PROJECT_ROOT = Path(__file__).resolve().parents[2]


@dataclass(frozen=True)
class DemandConditionedDevelopmentCandidate:
    key: str
    runtime_policy: str
...
def development_candidate(
    protocol: Mapping[str, Any]
) -> DemandConditionedDevelopmentCandidate:
    row = protocol["development"]["candidate"]
    candidate = DemandConditionedDevelopmentCandidate(
        key=str(row["key"]),
    ...
    if (
        ...
        raise ValueError("v71 development candidate changed")
    return candidate
...
def development_candidates(
    protocol: Mapping[str, Any]
) -> tuple[DemandConditionedDevelopmentCandidate, ...]:
    """Return the single pre-frozen v71 execution candidate."""

    return (development_candidate(protocol),)
...
def _load_veto(
    *,
...
) -> tuple[dict[str, Any], DemandConditionedTargetVeto]:
    ...
    if (
        ...
        raise ValueError(f"v71 target-veto artifact changed for {city}")
    ...
    if (
        ...
        or not isinstance(veto, DemandConditionedTargetVeto)
        ...
        raise ValueError(f"v71 target-veto model contract changed for {city}")
    return artifact, veto


def run_demand_conditioned_closed_loop_development(
    *,
    protocol_path: Path,
    ...
    training_result = json.loads(
        Path(training_result_path).read_text(encoding="utf-8")
    )
    veto_audit = json.loads(Path(veto_joint_audit_path).read_text(encoding="utf-8"))
    if (
        protocol.get("protocol") != PROTOCOL
        or training_protocol.get("protocol") != TRAINING_PROTOCOL
        or _sha256(training_protocol_path)
        != protocol["training_protocol"]["sha256"]
        ...
        or _sha256(training_result_path)
        != protocol["target_veto_training_result"]["sha256"]
        ...
        raise ValueError("v71 closed-loop evidence chain changed")
    runtime_source_gate = {
    ...
    if not runtime_source_gate or not all(runtime_source_gate.values()):
        raise ValueError(f"v71 runtime source freeze changed: {runtime_source_gate}")
    if (
        freeze_audit.get("status") != "PASS"
        ...
        raise ValueError("v71 environment evidence changed")
    development = protocol["development"]
    if int(seed) not in {int(value) for value in development["seeds"]}:
        raise ValueError("seed is outside v71 development")
    matching_cities = [
        str(city)
    ...
    if len(matching_cities) != 1:
        raise ValueError("scenario lacks one v71 development city")
    city = matching_cities[0]
    candidate = development_candidate(protocol)
    if str(policy) != candidate.key:
        raise ValueError("policy is outside v71 development")
    ...
    if (
        ...
        raise ValueError("v71 closed-loop environment changed")
    ...
    try:
        ...
        if not bool(metrics.get("ok", False)):
            raise RuntimeError(f"v71 rollout failed: {metrics.get('error')}")
        if int(metrics.get("starting_teleports", -1)) != 0 or int(
            metrics.get("ending_teleports", -1)
        ) != 0:
            raise RuntimeError("v71 rollout contains teleports")
        if not temporary_tripinfo.is_file() or temporary_tripinfo.stat().st_size <= 0:
            raise RuntimeError("v71 rollout lacks tripinfo evidence")
    ...
    return {
        ...
        # Legacy-key aliases let the shared paired-development auditor verify
        # the same evidence chain without weakening the explicit v71 names.
        ...
        "target_veto_training_result_sha256": _sha256(training_result_path),
        ...
        "demand_profile": target_veto.demand_profile.to_dict(),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.tripinfo_out.exists():
        raise FileExistsError("refusing to overwrite v71 rollout")
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.tripinfo_out.parent.mkdir(parents=True, exist_ok=True)
    payload = run_demand_conditioned_closed_loop_development(

"""Run one frozen v84 uncertainty-horizon confirmation rollout."""
...
RESULT_PROTOCOL = "tsc-v84r80-uncertainty-horizon-confirmation-rollout-v1"
ANALYSIS_STAGE = "uncertainty_horizon_confirmation"


@dataclass(frozen=True)
class UncertaintyHorizonConfirmationCandidate:
    key: str
    artifact_key: str
    candidate_role: str
...
def confirmation_candidates(
...
) -> tuple[UncertaintyHorizonConfirmationCandidate, ...]:
    rows = (dict(protocol["confirmation"]["method_candidate"]),)
    candidates = tuple(
        UncertaintyHorizonConfirmationCandidate(
            key=str(row["key"]),
    ...
    if not (
        tuple(candidate.key for candidate in candidates) == (METHOD_KEY,)
        ...
        raise ValueError("v84 confirmation candidate changed")
    return candidates
...
def all_rollout_identities(
    ...
    policies = tuple(str(value) for value in confirmation["policies"])
    identities = tuple(
        (str(city), str(scenario), seed, policy)
        for city, scenarios in sorted(confirmation["city_scenarios"].items())
    ...
    if not (
        ...
        and len(identities) == int(confirmation["matrix_size"]) == 64
        and len(identities) == len(set(identities))
    ):
        raise ValueError("v84 rollout matrix changed")
    return identities


def candidate_payload(
    candidate: UncertaintyHorizonConfirmationCandidate,
) -> dict[str, Any]:
    return {
        "key": candidate.key,
...
def _validate_evidence(
    *,
    ...
    confirmation_parent = dict(protocol["confirmation_parent"])
    checks = [
        (_sha256(v83_protocol_path), confirmation_parent["v83_protocol"]["sha256"]),
        (
            _sha256(v83_audit_path),
            confirmation_parent["v83_independent_audit"]["sha256"],
        ),
    ...
    checks.extend(
        (
            (
                _sha256(Path(artifact_root) / ARTIFACT_KEY / "model.pkl"),
                artifact["model"]["sha256"],
            ),
            (
                _sha256(Path(artifact_root) / ARTIFACT_KEY / "freeze.json"),
                artifact["certificate"]["sha256"],
            ),
        )
    )
    if any(str(observed) != str(expected) for observed, expected in checks):
        raise ValueError("v84 confirmation evidence changed")
...
def run_uncertainty_horizon_confirmation(
    *,
    protocol_path: Path,
    v83_protocol_path: Path,
    v83_audit_path: Path,
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("v84 confirmation protocol changed")
    _validate_evidence(
        protocol=protocol,
    ...
    if identity not in set(all_rollout_identities(protocol)):
        raise ValueError("rollout identity is outside frozen v84 confirmation")
    candidates = {row.key: row for row in confirmation_candidates(protocol)}
    ...
    if not (
        ...
        raise ValueError("v84 SUMO environment changed")
    ...
    if policy != PHASE_POLICY:
        ...
        cooldown_intervals = candidate.cooldown_intervals
        artifact_record = protocol["artifact"]
        ...
        if not (
            ...
            raise RuntimeError("v84 originator runtime injection changed")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.tripinfo_out.exists():
        raise FileExistsError("refusing to overwrite v84 rollout")

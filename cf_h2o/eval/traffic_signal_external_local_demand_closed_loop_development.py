"""Closed-loop development for the frozen v74 local-demand execution veto."""
...
from cf_h2o.eval.traffic_signal_external_local_demand_veto_freeze import (
...
PROTOCOL = "tsc-v75r71-external-v9-local-demand-closed-loop-development-v1"
RESULT_PROTOCOL = (
    "tsc-v75r71-external-v9-local-demand-closed-loop-development-rollout-v1"
)
ANALYSIS_STAGE = "local_demand_closed_loop_development"
CANDIDATE_KEY = "cfcmt_local_demand_veto_cd450"
PHASE_POLICY = "phase_pressure"
PROJECT_ROOT = Path(__file__).resolve().parents[2]


@dataclass(frozen=True)
class LocalDemandDevelopmentCandidate:
    key: str
    runtime_policy: str
...
def development_candidate(protocol: Mapping[str, Any]) -> LocalDemandDevelopmentCandidate:
    row = protocol["development"]["candidate"]
    candidate = LocalDemandDevelopmentCandidate(
        key=str(row["key"]),
    ...
    if (
        ...
        raise ValueError("v75 development candidate changed")
    return candidate


def candidate_payload(candidate: LocalDemandDevelopmentCandidate) -> dict[str, Any]:
    return {
        "key": candidate.key,
...
def _load_veto(
    *,
...
) -> tuple[dict[str, Any], LocalDemandTargetVeto]:
    ...
    if (
        ...
        raise ValueError(f"v75 local-demand artifact changed for {city}")
    ...
    if (
        ...
        or not isinstance(veto, LocalDemandTargetVeto)
        or artifact.get("zero_shot") is not False
        or artifact.get("scenario_id_is_model_feature") is not False
        or artifact.get("veto_role")
        != "local_demand_veto_only_never_action_originator"
        or bool(veto.config.enabled) != bool(audited["active"])
    ):
        raise ValueError(f"v75 local-demand model contract changed for {city}")
    return artifact, veto


def run_local_demand_closed_loop_development(
    *,
    protocol_path: Path,
    ...
    if (
        ...
        != protocol["local_demand_freeze_result"]["sha256"]
        ...
        != protocol["local_demand_joint_audit"]["sha256"]
    ):
        raise ValueError("v75 closed-loop evidence chain changed")
    source_gate = {
    ...
    if not source_gate or not all(source_gate.values()):
        raise ValueError(f"v75 runtime source freeze changed: {source_gate}")
    if (
        ...
        raise ValueError("v75 environment evidence changed")
    development = protocol["development"]
    if int(seed) not in {int(value) for value in development["seeds"]}:
        raise ValueError("seed is outside v75 development")
    matching_cities = [
        str(city)
    ...
    if (
        ...
        raise ValueError("v75 closed-loop environment changed")
    ...
    try:
        ...
        if not bool(metrics.get("ok", False)):
            raise RuntimeError(f"v75 rollout failed: {metrics.get('error')}")
        if int(metrics.get("starting_teleports", -1)) != 0 or int(
            metrics.get("ending_teleports", -1)
        ) != 0:
            raise RuntimeError("v75 rollout contains teleports")
        if not temporary_tripinfo.is_file() or temporary_tripinfo.stat().st_size <= 0:
            raise RuntimeError("v75 rollout lacks tripinfo evidence")
    ...
    return {
        ...
        "seed": int(seed),
        "policy": str(policy),
        "policy_role": "local_demand_cfcmt" if is_method else "paired_phase_pressure",

"""Run one day of the frozen Chicago v107 full-week safety admission."""

from __future__ import annotations

import argparse
from collections import Counter
from datetime import datetime, timezone
import hashlib
import json
from pathlib import Path
import time
from typing import Any, Mapping, Sequence
import xml.etree.ElementTree as ET

from cf_h2o.sumo_runtime import (
    libsumo_version,
    load_libsumo,
    runtime_metadata,
    sumo_state_directory,
)
from cf_h2o.traffic_signal.dataset_cache import atomic_write_json
from scripts.data.acquire_chicago_for_hire_unseen import DATES
from scripts.data.prepare_chicago_for_hire_demand import EXPECTED_NETWORK_SHA256
from scripts.data.route_chicago_for_hire_demand import (
    END_SEC,
    EXPECTED_INCLUDED_TRIPS,
    EXPECTED_SUMO_VERSION,
    PROTOCOL as ROUTE_PROTOCOL,
    SAFETY_SEEDS,
)


PROTOCOL = "cfcmt-chicago-full-week-strict-libsumo-safety-admission-v1"
EXPECTED_TLS_COUNT = 4_960


def _sha256(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as handle:
        for block in iter(lambda: handle.read(4 * 1024 * 1024), b""):
            digest.update(block)
    return digest.hexdigest()


def _read_json(path: Path) -> dict[str, Any]:
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError(f"expected JSON object: {path}")
    return value


def _config_values(path: Path) -> dict[str, str]:
    root = ET.parse(path).getroot()
    return {
        element.tag: str(element.attrib["value"])
        for element in root.iter()
        if "value" in element.attrib
    }


def _validate_route_package(
    package_root: Path,
    *,
    expected_manifest_sha256: str,
    day_index: int,
) -> tuple[Path, dict[str, Any], dict[str, Any], str]:
    if not 0 <= day_index < len(DATES):
        raise ValueError(f"invalid Chicago day index: {day_index}")
    package_root = package_root.resolve()
    manifest_path = package_root / "route_package_manifest.json"
    manifest_sha256 = _sha256(manifest_path)
    if manifest_sha256 != expected_manifest_sha256:
        raise ValueError(
            "Chicago route manifest identity changed: "
            f"{manifest_sha256} != {expected_manifest_sha256}"
        )
    manifest = _read_json(manifest_path)
    identity = {
        "protocol": manifest.get("protocol"),
        "status": manifest.get("status"),
        "sumo_version": manifest.get("sumo_version"),
        "network_sha256": manifest.get("network_sha256"),
        "routed_vehicle_count": manifest.get("routed_vehicle_count"),
        "bulk_routing": manifest.get("bulk_routing"),
        "repair": manifest.get("repair"),
        "ignore_errors": manifest.get("ignore_errors"),
    }
    expected_identity = {
        "protocol": ROUTE_PROTOCOL,
        "status": "PASS",
        "sumo_version": EXPECTED_SUMO_VERSION,
        "network_sha256": EXPECTED_NETWORK_SHA256,
        "routed_vehicle_count": EXPECTED_INCLUDED_TRIPS,
        "bulk_routing": False,
        "repair": False,
        "ignore_errors": False,
    }
    if identity != expected_identity:
        raise ValueError(f"Chicago route package changed: {identity}")
    execution = dict(manifest.get("strict_execution", {}))
    expected_execution = {
        "step_length_sec": 1.0,
        "end_sec": END_SEC,
        "demand_scale": 1.0,
        "max_depart_delay_sec": -1,
        "time_to_teleport_sec": -1,
        "collision_action": "warn",
        "collision_check_junctions": True,
        "route_errors_fatal": True,
    }
    if execution != expected_execution:
        raise ValueError(f"Chicago strict execution changed: {execution}")
    daily = list(manifest.get("daily_routes", []))
    if len(daily) != len(DATES):
        raise ValueError("Chicago route package does not contain all seven days")
    row = dict(daily[day_index])
    if (
        row.get("date") != DATES[day_index].isoformat()
        or int(row.get("day_index", -1)) != day_index
        or int(row.get("strict_seed", -1)) != SAFETY_SEEDS[day_index]
        or int(row.get("vehicle_count", -1)) <= 0
    ):
        raise ValueError(f"Chicago daily route identity changed: {row}")
    route_path = package_root / str(row["route_file"])
    if (
        not route_path.is_file()
        or route_path.stat().st_size != int(row["route_size_bytes"])
        or _sha256(route_path) != str(row["route_sha256"])
    ):
        raise ValueError(f"Chicago daily route file changed: {route_path}")
    config = package_root / str(row["strict_config"])
    if not config.is_file():
        raise FileNotFoundError(config)
    values = _config_values(config)
    required_values = {
        "route-files": route_path.name,
        "step-length": "1.0",
        "end": str(END_SEC),
        "scale": "1",
        "max-depart-delay": "-1",
        "time-to-teleport": "-1",
        "time-to-teleport.highways": "-1",
        "collision.action": "warn",
        "collision.check-junctions": "true",
        "ignore-route-errors": "false",
        "seed": str(SAFETY_SEEDS[day_index]),
    }
    observed_values = {key: values.get(key) for key in required_values}
    if observed_values != required_values:
        raise ValueError(f"Chicago strict config changed: {observed_values}")
    network_root = Path(str(manifest["network_root"]))
    network_manifest = _read_json(network_root / "network_manifest.json")
    network_inventory = dict(network_manifest.get("network_inventory", {}))
    if (
        network_manifest.get("status") != "PASS"
        or network_manifest.get("network_sha256") != EXPECTED_NETWORK_SHA256
        or int(network_inventory.get("traffic_light_count", -1))
        != EXPECTED_TLS_COUNT
    ):
        raise ValueError("Chicago admitted network identity changed")
    return config, manifest, row, manifest_sha256


def _final_summary_step(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise FileNotFoundError(f"SUMO summary output is missing: {path}")
    final: dict[str, str] | None = None
    for _, element in ET.iterparse(path, events=("end",)):
        if element.tag == "step":
            final = dict(element.attrib)
        element.clear()
    if final is None:
        raise ValueError(f"SUMO summary output contains no step: {path}")
    parsed: dict[str, Any] = {"time": float(final.get("time", 0.0))}
    for field in (
        "loaded",
        "inserted",
        "running",
        "waiting",
        "ended",
        "arrived",
        "collisions",
        "teleports",
    ):
        parsed[field] = int(float(final.get(field, 0)))
    return parsed


def _collision_key(collision: Any) -> tuple[str, str, str, str]:
    participants = sorted(
        (str(getattr(collision, "collider", "")), str(getattr(collision, "victim", "")))
    )
    return (
        participants[0],
        participants[1],
        str(getattr(collision, "type", "")),
        str(getattr(collision, "lane", "")),
    )


def _collision_sample(collision: Any, *, time_sec: float) -> dict[str, Any]:
    sample: dict[str, Any] = {"repr": repr(collision), "time_sec": time_sec}
    for field in ("collider", "victim", "type", "lane", "pos"):
        value = getattr(collision, field, None)
        if value is not None:
            sample[field] = value
    return sample


def _admission_checks(
    *,
    expected_vehicle_count: int,
    observed: Mapping[str, Any],
    summary: Mapping[str, Any] | None,
    error: str | None,
) -> dict[str, bool]:
    final_summary = dict(summary or {})
    return {
        "runtime_exception_free": error is None,
        "all_vehicles_completed_before_cap": (
            observed.get("termination_reason") == "all_vehicles_completed"
        ),
        "exact_demand_loaded": (
            int(final_summary.get("loaded", -1)) == expected_vehicle_count
        ),
        "exact_demand_departed": (
            int(observed.get("departed", -1)) == expected_vehicle_count
        ),
        "exact_demand_arrived": (
            int(observed.get("arrived", -1)) == expected_vehicle_count
            and int(final_summary.get("arrived", -1)) == expected_vehicle_count
        ),
        "no_running_vehicles": (
            int(observed.get("final_active_vehicles", -1)) == 0
            and int(final_summary.get("running", -1)) == 0
        ),
        "no_pending_vehicles": (
            int(observed.get("final_pending_vehicles", -1)) == 0
            and int(final_summary.get("waiting", -1)) == 0
        ),
        "no_expected_vehicles": int(observed.get("final_min_expected", -1)) == 0,
        "zero_collisions": (
            int(observed.get("unique_collision_incidents", -1)) == 0
            and int(final_summary.get("collisions", -1)) == 0
        ),
        "zero_teleports": (
            int(observed.get("starting_teleports", -1)) == 0
            and int(observed.get("ending_teleports", -1)) == 0
            and int(final_summary.get("teleports", -1)) == 0
        ),
        "exact_controllable_tls": (
            int(observed.get("controllable_tls_count", -1)) == EXPECTED_TLS_COUNT
        ),
    }


def _write_progress(path: Path | None, payload: Mapping[str, Any]) -> None:
    if path is not None:
        atomic_write_json(path, dict(payload))


def run_chicago_day_admission(
    *,
    package_root: Path,
    expected_package_manifest_sha256: str,
    day_index: int,
    expected_sumo_version: str = EXPECTED_SUMO_VERSION,
    maximum_time_sec: float = END_SEC,
    progress_interval_sec: int = 1_800,
    state_sample_interval_sec: int = 60,
    progress_out: Path | None = None,
) -> dict[str, Any]:
    started = time.perf_counter()
    config, package, day, manifest_sha256 = _validate_route_package(
        package_root,
        expected_manifest_sha256=expected_package_manifest_sha256,
        day_index=day_index,
    )
    expected_vehicle_count = int(day["vehicle_count"])
    seed = int(day["strict_seed"])
    actual_sumo_version = libsumo_version()
    if actual_sumo_version != expected_sumo_version:
        raise RuntimeError(
            f"SUMO version mismatch: {actual_sumo_version} != {expected_sumo_version}"
        )

    sumo_api = load_libsumo()
    loaded = departed = arrived = 0
    starting_teleports = ending_teleports = 0
    collision_incidents: set[tuple[str, str, str, str]] = set()
    collision_samples: list[dict[str, Any]] = []
    collision_lane_counts: Counter[str] = Counter()
    initial_min_expected = final_min_expected = -1
    final_active_vehicles = final_pending_vehicles = -1
    controllable_tls_count = -1
    max_sampled_active_vehicles = max_sampled_pending_vehicles = 0
    final_time_sec = 0.0
    termination_reason: str | None = None
    error: str | None = None
    summary: dict[str, Any] | None = None
    next_progress = float(progress_interval_sec)
    next_state_sample = 0.0

    with sumo_state_directory(prefix=f"cfcmt-chicago-d{day_index}-{seed}-") as scratch:
        summary_path = scratch / "summary.xml"
        try:
            sumo_api.start(
                [
                    "sumo",
                    "-c",
                    str(config),
                    "--seed",
                    str(seed),
                    "--summary-output",
                    str(summary_path),
                    "--summary-output.period",
                    str(state_sample_interval_sec),
                    "--no-step-log",
                    "true",
                ]
            )
            initial_min_expected = int(sumo_api.simulation.getMinExpectedNumber())
            controllable_tls_count = len(sumo_api.trafficlight.getIDList())
            while True:
                minimum_expected = int(sumo_api.simulation.getMinExpectedNumber())
                final_time_sec = float(sumo_api.simulation.getTime())
                if minimum_expected == 0:
                    termination_reason = "all_vehicles_completed"
                    break
                if final_time_sec + 1e-9 >= maximum_time_sec:
                    termination_reason = "fixed_completion_cap_reached"
                    break

                sumo_api.simulationStep()
                final_time_sec = float(sumo_api.simulation.getTime())
                loaded += int(sumo_api.simulation.getLoadedNumber())
                departed += int(sumo_api.simulation.getDepartedNumber())
                arrived += int(sumo_api.simulation.getArrivedNumber())
                starting_teleports += int(
                    sumo_api.simulation.getStartingTeleportNumber()
                )
                ending_teleports += int(sumo_api.simulation.getEndingTeleportNumber())
                for collision in tuple(sumo_api.simulation.getCollisions()):
                    incident = _collision_key(collision)
                    if incident not in collision_incidents:
                        collision_lane_counts[incident[3]] += 1
                        if len(collision_samples) < 12:
                            collision_samples.append(
                                _collision_sample(collision, time_sec=final_time_sec)
                            )
                    collision_incidents.add(incident)

                if collision_incidents:
                    termination_reason = "collision_gate_irreversibly_failed"
                    break
                if starting_teleports or ending_teleports:
                    termination_reason = "teleport_gate_irreversibly_failed"
                    break

                if final_time_sec + 1e-9 >= next_state_sample:
                    active = int(sumo_api.vehicle.getIDCount())
                    pending = max(0, minimum_expected - active)
                    max_sampled_active_vehicles = max(max_sampled_active_vehicles, active)
                    max_sampled_pending_vehicles = max(
                        max_sampled_pending_vehicles, pending
                    )
                    next_state_sample += float(state_sample_interval_sec)
                if final_time_sec + 1e-9 >= next_progress:
                    progress = {
                        "protocol": PROTOCOL,
                        "date": day["date"],
                        "day_index": day_index,
                        "time_sec": final_time_sec,
                        "maximum_time_sec": maximum_time_sec,
                        "loaded_event_total": loaded,
                        "departed": departed,
                        "arrived": arrived,
                        "minimum_expected": minimum_expected,
                        "active_vehicles": int(sumo_api.vehicle.getIDCount()),
                        "elapsed_seconds": time.perf_counter() - started,
                    }
                    _write_progress(progress_out, progress)
                    print(
                        "CHICAGO_ADMISSION_PROGRESS "
                        + " ".join(f"{key}={value}" for key, value in progress.items()),
                        flush=True,
                    )
                    next_progress += float(progress_interval_sec)

            final_min_expected = int(sumo_api.simulation.getMinExpectedNumber())
            final_active_vehicles = int(sumo_api.vehicle.getIDCount())
            final_pending_vehicles = max(
                0, final_min_expected - final_active_vehicles
            )
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
            if termination_reason is None:
                termination_reason = "runtime_exception"
        finally:
            try:
                sumo_api.close()
            except Exception:
                pass
        try:
            summary = _final_summary_step(summary_path)
        except Exception as exc:
            if error is None:
                error = f"{type(exc).__name__}: {exc}"

    observed = {
        "initial_min_expected": initial_min_expected,
        "loaded_event_total": loaded,
        "departed": departed,
        "arrived": arrived,
        "starting_teleports": starting_teleports,
        "ending_teleports": ending_teleports,
        "unique_collision_incidents": len(collision_incidents),
        "collision_samples": collision_samples,
        "unique_collision_incidents_by_lane": dict(
            sorted(collision_lane_counts.items())
        ),
        "max_sampled_active_vehicles": max_sampled_active_vehicles,
        "max_sampled_pending_vehicles": max_sampled_pending_vehicles,
        "state_sample_interval_sec": state_sample_interval_sec,
        "final_active_vehicles": final_active_vehicles,
        "final_pending_vehicles": final_pending_vehicles,
        "final_min_expected": final_min_expected,
        "controllable_tls_count": controllable_tls_count,
        "final_time_sec": final_time_sec,
        "termination_reason": termination_reason,
        "summary": summary,
    }
    checks = _admission_checks(
        expected_vehicle_count=expected_vehicle_count,
        observed=observed,
        summary=summary,
        error=error,
    )
    payload = {
        "experiment": "traffic_signal_chicago_full_week_admission",
        "protocol": PROTOCOL,
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "passed": all(checks.values()),
        "checks": checks,
        "error": error,
        "setting": {
            "package_root": str(package_root.resolve()),
            "package_manifest_sha256": manifest_sha256,
            "date": day["date"],
            "day_index": day_index,
            "sumocfg": str(config),
            "backend": "libsumo",
            "expected_sumo_version": expected_sumo_version,
            "seed": seed,
            "maximum_time_sec": maximum_time_sec,
            "expected_vehicle_count": expected_vehicle_count,
            "expected_tls_count": EXPECTED_TLS_COUNT,
            "time_based_teleportation_disabled": True,
            "junction_collision_checks_enabled": True,
        },
        "observed": observed,
        "package": {
            "protocol": package["protocol"],
            "network_sha256": package["network_sha256"],
            "routed_vehicle_count": package["routed_vehicle_count"],
            "daily_route": day,
        },
        "runtime": runtime_metadata(),
        "elapsed_seconds": float(time.perf_counter() - started),
    }
    _write_progress(progress_out, {"completed": True, **payload})
    return payload


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--package-root", type=Path, required=True)
    parser.add_argument("--expected-package-manifest-sha256", required=True)
    parser.add_argument("--day-index", type=int, required=True)
    parser.add_argument("--expected-sumo-version", default=EXPECTED_SUMO_VERSION)
    parser.add_argument("--maximum-time-sec", type=float, default=END_SEC)
    parser.add_argument("--progress-interval-sec", type=int, default=1_800)
    parser.add_argument("--state-sample-interval-sec", type=int, default=60)
    parser.add_argument("--progress-out", type=Path)
    parser.add_argument("--out", type=Path, required=True)
    args = parser.parse_args(argv)
    payload = run_chicago_day_admission(
        package_root=args.package_root,
        expected_package_manifest_sha256=args.expected_package_manifest_sha256,
        day_index=args.day_index,
        expected_sumo_version=args.expected_sumo_version,
        maximum_time_sec=args.maximum_time_sec,
        progress_interval_sec=args.progress_interval_sec,
        state_sample_interval_sec=args.state_sample_interval_sec,
        progress_out=args.progress_out,
    )
    atomic_write_json(args.out, payload)
    print(json.dumps({"passed": payload["passed"], "checks": payload["checks"]}))
    return 0 if payload["passed"] else 2


if __name__ == "__main__":
    raise SystemExit(main())

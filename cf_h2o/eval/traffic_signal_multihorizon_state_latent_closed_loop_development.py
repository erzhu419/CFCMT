"""Run one frozen v80 multihorizon state-latent closed-loop rollout."""
...
RESULT_PROTOCOL = "tsc-v83r79-multihorizon-state-latent-closed-loop-rollout-v1"
ANALYSIS_STAGE = "multihorizon_state_latent_closed_loop_development"
...
def development_candidates(
    ...
    if (
        ...
        != (0, 2, 5, 0, 2, 5)
        or tuple(candidate.artifact_key for candidate in candidates[:3])
        != (PRIMARY_ARTIFACT_KEY,) * 3
        or tuple(candidate.artifact_key for candidate in candidates[3:])
        != (SECONDARY_ARTIFACT_KEY,) * 3
        or any(
            candidate.selection_eligible_closed_loop is not (index < 3)
        ...
        raise ValueError("v80 execution candidate grid changed")
    return candidates
...
def all_rollout_identities(
    ...
    if (
        ...
        or len(identities) != 154
        or len(identities) != len(set(identities))
    ):
        raise ValueError("v80 rollout matrix changed")
    return identities
...
def _validate_evidence(
    *,
    ...
    if any(str(observed) != str(expected) for observed, expected in checks):
        raise ValueError("v80 closed-loop evidence changed")


def run_multihorizon_state_latent_closed_loop_development(
    *,
    protocol_path: Path,
    artifact_result_path: Path,
    artifact_audit_path: Path,
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("v80 closed-loop protocol changed")
    _validate_evidence(
        protocol=protocol,
    ...
    if identity not in set(all_rollout_identities(protocol)):
        raise ValueError("rollout identity is outside frozen v80 development")
    ...
    if (
        ...
        raise ValueError("v80 SUMO environment changed")
    ...
    if policy != PHASE_POLICY:
        ...
        if (
            ...
            raise RuntimeError("v80 originator runtime injection changed")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.tripinfo_out.exists():
        raise FileExistsError("refusing to overwrite v80 rollout")
    result = run_multihorizon_state_latent_closed_loop_development(

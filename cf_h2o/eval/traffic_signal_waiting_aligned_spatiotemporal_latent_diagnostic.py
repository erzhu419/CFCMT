"""Seed-blocked OOF diagnostic for target spatiotemporal action latents."""
...
RESULT_PROTOCOL = "tsc-v83r79-waiting-aligned-spatiotemporal-latent-result-v1"
AUTHORIZATION_DECISION = "authorize_waiting_aligned_spatiotemporal_latent_freeze"
REJECTION_DECISION = "retain_phase_pressure_and_reject_spatiotemporal_latent"
...
def _fit_latent_oof_fold(
    *,
    ...
    if (
        ...
        raise RuntimeError("spatiotemporal latent OOF seed partition leaked")
    ...
    candidate_states = np.asarray(validation.metadata["candidate_states"], dtype=str)
    ...
    for spec in model_specs:
        model = TargetSpatiotemporalActionLatent(
            SpatiotemporalActionLatentConfig(**dict(spec["config"]))
        )
        ...
        if not all(
            ...
            for values in (score, uncertainty, trust, level, support)
        ):
            raise ValueError("spatiotemporal latent emitted non-finite predictions")
        records = []
        for group in sorted(validation_groups):
            ...
            if rows.size != int(expected_action_count) or references.size != 1:
                raise ValueError(
                    f"latent action group {group!r} violates the action contract"
                )
...
def _process_fold(heldout_seed: int) -> dict[str, Any]:
    if _PROCESS_FOLD_CONTEXT is None:
        raise RuntimeError("spatiotemporal process fold context is not initialized")
    return _fit_latent_oof_fold(
        heldout_seed=int(heldout_seed),
...
def run_diagnostic(
    *,
    ...
    evidence_ok = (
        ...
        and ranker_protocol.get("protocol") == RANKER_PROTOCOL
        and ranker_result.get("protocol") == RANKER_RESULT_PROTOCOL
        and ranker_result.get("status") == "PASS"
        and ranker_result.get("decision") == RANKER_REJECTION_DECISION
        and ranker_result.get("integrity_gate", {}).get("passed") is True
        ...
        and _sha256(ranker_protocol_path) == frozen.get("ranker_protocol_sha256")
        and _sha256(ranker_result_path) == frozen.get("ranker_result_sha256")
        and _sha256(partition_path) == frozen.get("partition_sha256")
        and _sha256(manifest_path) == frozen.get("manifest_sha256")
    )
    if not evidence_ok:
        raise ValueError("spatiotemporal latent evidence changed")
    ...
    if set(seeds).intersection(sealed):
        raise ValueError("spatiotemporal development overlaps sealed seeds")
    ...
    if len(selected_specs) != 1:
        raise ValueError("spatiotemporal scenario is not unique")
    ...
    if observed_seeds != set(seeds):
        raise ValueError("spatiotemporal contrast seed set changed")
    ...
    fit_contract = all(
        ...
        == "target-spatiotemporal-action-latent-v1"
    ...
    return {
        ...
        "offline_comparators": {
            key: value
            for key, value in ranker_result["offline_comparators"].items()
            if key
            in {
                "phase_pressure_reference",
                "immutable_v60_hierarchical_cfcmt",
                "counterfactual_oracle_best_of_eight",
            }
        },
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError(f"refusing to overwrite latent result: {args.out}")

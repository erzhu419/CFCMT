...
train_iql_bus.py
================
IQL (Kostrikov et al., 2021) baseline on bus-holding offline data.

The H2OPlusBus pretrain mode (Q regression + V expectile regression + AWR
policy) is exactly the IQL recipe. We expose it as an explicit IQL baseline
for the R3 audit.

Run:
    cd H2Oplus/SimpleSAC
    conda run -n LSTM-RL python train_iql_bus.py --n_steps 100000 --seed 42
"""
...
# ── Output ──
out_dir = os.path.join(_H2O_ROOT, "experiment_output", f"iql_seed{args.seed}")
os.makedirs(out_dir, exist_ok=True)
print(f"[IQL] Output dir: {out_dir}")
...
# ── Save final model ──
ckpt_path = os.path.join(out_dir, "iql_final.pt")
torch.save({

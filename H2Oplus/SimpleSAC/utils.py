...
#TODO: thigh range
def update_target_env_thigh_range(variety_degree, env_name):
    old_xml_name = parse_xml_name(env_name)
    # create new xml
...
#TODO: foot shape
def update_target_env_foot_shape(env_name):
    old_xml_name = parse_xml_name(env_name)
    # create new xml
...
#TODO: foot stiffness
def update_target_env_soft_foot(variety_degree, env_name):
    old_xml_name = parse_xml_name(env_name)
    # create new xml
    ...
    with open(real_xml_path, "r") as f, open(sim_xml_path, "w+") as new_f:
        for line in f.readlines():
            if "stiffness" in line and "foot" in line:
                ...
                replace_num = " ".join(str(i) for i in new_thigh_range_list)
                replace_num = "stiffness=\"" + replace_num + "\""
                sub_result = re.sub(pattern, str(replace_num), line)
...
def update_target_env_soft_limb(variety_degree, env_name):
    old_xml_name = parse_xml_name(env_name)
    # create new xml
...
def update_target_env_tendon_elasticity(variety_degree, env_name):
    old_xml_name = parse_xml_name(env_name)
    # create new xml
...
#TODO: limb shape
def update_target_env_ellipsoid_limb(env_name):
    old_xml_name = parse_xml_name(env_name)
    # create new xml
...
#TODO: limb shape
def update_target_env_box_limb(env_name):
    old_xml_name = parse_xml_name(env_name)
    # create new xml
...
#TODO: head size
def update_target_env_head_size(variety_degree, env_name):
    old_xml_name = parse_xml_name(env_name)
    # create new xml
...
#TODO: foot length
def update_target_env_foot_length(variety_degree, env_name):
    old_xml_name = parse_xml_name(env_name)
    # create new xml
    xml_name = "{}_torso_lengthx{}.xml".format(old_xml_name.split(".")[0], variety_degree)
    real_xml_path = ensure_real_xml(old_xml_name)
...
def load_model(model_file):
    import pickle
    import sys
    sys.path.append('./SimpleSAC')
    with open(model_file, 'rb') as f:
        model = pickle.load(f)
    h2o = model['h2o']
    policy = h2o.policy
    policy = policy.to('cpu')
    state = torch.rand(size=[1, 4])
    a = policy(state)

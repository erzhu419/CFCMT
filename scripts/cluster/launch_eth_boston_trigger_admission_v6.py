#!/usr/bin/env python3
"""Submit the Boston v8 trigger-window microscopic admission."""
...
LAUNCH_PROTOCOL = "tsc-v118r-eth-boston-trigger-admission-launch-v6"
SIGNATURE = "CFCMT/v118r/eth-boston-trigger-admission-v6-uncontrolled-merge-yield"
FORMER_FAILURE_TIMES_SEC = (9_518.0, 11_504.0, 12_279.0)
...
def build_spec(
    *,
    ...
    spec.update(
        {
            "description": "CFCMT V118r ETH Boston v8 5400s trigger admission",
            "signature": SIGNATURE,
            "ram_resource_family": "CFCMT-v118-eth-boston-trigger-admission-v8",
        }
    )
    return spec
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite Boston v8 trigger launch artifact")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote Boston v8 trigger preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote Boston v8 trigger evidence changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v118r-eth-boston-trigger-admission-v6",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected Boston v8 trigger admission")
    return 0

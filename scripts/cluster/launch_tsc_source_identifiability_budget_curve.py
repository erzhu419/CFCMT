#!/usr/bin/env python3
"""Submit the 21-target V150H source-identifiability budget curve."""
...
LAUNCH_PROTOCOL = "tsc-v150h-source-identifiability-budget-launch-v1"
CONFIG_RELATIVE = Path(
    "cf_h2o/config/traffic_signal_tsc_v150h_source_identifiability_budget_curve.json"
)
MODULE = "cf_h2o.eval.traffic_signal_source_identifiability_budget_curve"
SIGNATURE_PREFIX = "CFCMT/v150h/source-identifiability-budget-v1"
RESOURCE_FAMILY = "CFCMT-v150h-source-identifiability-budget"
RAM_MB = 8_192
...
def _validate_config(config: Mapping[str, Any]) -> None:
    ...
    if (
        config.get("protocol")
        != "tsc-v150h-source-identifiability-budget-curve-v1"
        or config.get("redeveloped_after") != V150G_AGGREGATE_PROTOCOL
        ...
        raise ValueError("V150H frozen configuration changed")
...
def build_specs(
    *,
    ...
    v150g_rejection_remote: Path,
    v150g_rejection_sha256: str,
    ...
    for budget in TARGET_BUDGETS:
        budget_specs = _build_rigid_specs(
                ...
                version_label=f"V150H-B{budget}",
                resource_family=RESOURCE_FAMILY,
                extra_target_args=(
                    "--target-budget",
                    str(budget),
                    "--v150g-rejection",
                    str(v150g_rejection_remote),
                    "--v150g-rejection-sha256",
                    str(v150g_rejection_sha256),
                ),
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--v150g-rejection-local", type=Path, required=True)
    parser.add_argument("--v150g-rejection-remote", type=Path, required=True)
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite V150H launch or results")
    if not 1 <= int(args.cache_workers) <= 8:
        raise ValueError("V150H cache workers must be in [1, 8]")
    _validate_config(_read_json(PROJECT_ROOT / CONFIG_RELATIVE))

    authorization = _read_json(args.authorization_result_local)
    rejection = _read_json(args.v150g_rejection_local)
    ...
    if (
        ...
        raise ValueError("V150H requires passing V150C authorization")
    if (
    ...
    if fit.get("protocol") != FIT_RESULT_PROTOCOL or fit.get("city") != "jinan":
        raise ValueError("V150H requires the frozen V115 architecture fit")
    if (
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote V150H preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            f"remote V150H evidence identities changed: {remote_hashes}; {hash_stderr}"
        )
    specs = build_specs(
        snapshot_root=snapshot_root,
        authorization_result_remote=args.authorization_result_remote,
        authorization_sha256=expected_hashes[0],
        v150g_rejection_remote=args.v150g_rejection_remote,
        v150g_rejection_sha256=expected_hashes[1],
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v150h-source-identifiability-budget-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V150H target matrix")
    return 0

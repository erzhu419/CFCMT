#!/usr/bin/env python3
"""Submit complete-demand route admission for the Boston v8 package."""
...
LAUNCH_PROTOCOL = "tsc-v118r-eth-boston-route-admission-launch-v6"
SIGNATURE = "CFCMT/v118r/eth-boston-route-admission-v6-uncontrolled-merge-yield"
...
def build_spec(
    *,
    ...
    spec.update(
        {
            "description": "CFCMT V118r ETH Boston v8 complete-demand route admission",
            "signature": SIGNATURE,
            "ram_resource_family": "CFCMT-v118-eth-boston-route-admission-v8",
        }
    )
    return spec
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite Boston v8 route launch artifact")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote Boston v8 route preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != [args.expected_package_manifest_sha256]:
        raise RuntimeError(
            "remote Boston v8 package identity changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v118r-eth-boston-route-admission-v6",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected Boston v8 route admission")
    return 0

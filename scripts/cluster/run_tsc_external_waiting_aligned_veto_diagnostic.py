#!/usr/bin/env python3
"""Run and retrieve the waiting-aligned veto OOF diagnostic on node001."""
...
LAUNCH_PROTOCOL = "v83r79-waiting-aligned-veto-diagnostic-launch-v1"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.local_out.exists() or args.launch_out.exists():
        raise FileExistsError("refusing to overwrite local veto diagnostic")
    stage = _read_json(args.stage_manifest)
    snapshot = Path(stage["snapshot_root"])
    scheduler = _load_scheduler(args.scheduler)
    if str(args.node) not in {f"node{index:03d}" for index in range(1, 7)}:
        raise ValueError("waiting veto diagnostic node is outside node001-node006")
    ...
    if int(code) != 0:
        raise FileExistsError(f"refusing to overwrite remote veto diagnostic: {stderr}")
    command = shlex.join(
        [
            str(snapshot / "scripts/cluster/run_cfcmt_sumo122.sh"),
            "-m",
            "cf_h2o.eval.traffic_signal_waiting_aligned_veto_diagnostic",
            "--diagnostic-protocol",
    ...
    if int(returncode) != 0 or sync_error is not None or result is None:
        raise RuntimeError(
            "waiting veto diagnostic failed: "

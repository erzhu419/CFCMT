#!/usr/bin/env python3
"""Submit the frozen V133 historical action-identifiability screen."""
...
LAUNCH_PROTOCOL = "tsc-v133-historical-action-identifiability-launch-v1"
SIGNATURE = "CFCMT/v133/historical-action-identifiability-v1"
    ...
    *,
    ...
    command = shlex.join(
        [
            ...
            str(wrapper),
            "-m",
            "cf_h2o.eval.traffic_signal_historical_action_identifiability",
            "--selector-cache-root",
    ...
    return {
        "description": "CFCMT V133 historical action identifiability",
        "project": "CFCMT",
        "cmd": command,
        "cwd": str(snapshot_root),
        "signature": SIGNATURE,
        "resource_family": "CFCMT-v133-historical-action-identifiability",
        "vram": 0,
        "ram_mb": RAM_MB,
        "cpu": 20,
        "priority": "high",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite V133 launch artifact")
    if not 1 <= int(args.cache_workers) <= 20:
        raise ValueError("V133 cache workers must be in [1, 20]")
    if not 1 <= int(args.fold_workers) <= 20:
        raise ValueError("V133 fold workers must be in [1, 20]")
    nodes = tuple(value.strip() for value in args.nodes.split(",") if value.strip())
    if not nodes or len(set(nodes)) != len(nodes):
        raise ValueError("V133 execution nodes must be nonempty and unique")
    if not set(nodes).issubset(ALLOWED_NODES):
        raise ValueError("V133 execution nodes include a non-admitted CPU node")
    ...
    if (
        ...
        raise ValueError("V133 selector cache audit changed")
    ...
    for node in nodes:
        ...
        if int(code) != 0:
            raise RuntimeError(f"remote V133 preflight failed on {node}: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != [selector_sha]:
        raise RuntimeError(
            "remote V133 selector evidence changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v133-historical-action-identifiability-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V133 identifiability screen")
    return 0

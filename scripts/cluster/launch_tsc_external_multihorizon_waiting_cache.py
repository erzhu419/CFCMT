#!/usr/bin/env python3
"""Collect the frozen v76 multihorizon cache on six CPU nodes."""
...
LAUNCH_PROTOCOL = "v83r79-multihorizon-waiting-cache-six-node-launch-v1"
...
def build_specs(
    *,
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("multihorizon waiting cache protocol changed")
    ...
    if (
        ...
        raise ValueError("multihorizon remote output root changed")
    ...
    for node, seeds in collection["node_assignments"].items():
        ...
        if local_result.exists():
            raise FileExistsError(
                f"refusing to overwrite local multihorizon task: {local_result}"
            )
        command = shlex.join(
            [
                str(wrapper),
        ...
        specs.append(
            {
                "description": f"multihorizon waiting cache {node} seeds {values}",
                "project": "CFCMT",
                "cmd": command,
                "cwd": str(snapshot_root),
                "signature": f"CFCMT/v83r79/multihorizon-cache/{node}",
                "resource_family": "CFCMT-v83-multihorizon-cache-v1",
                "vram": 0,
                "ram_mb": 196608,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if int(code) != 0:
        raise FileExistsError(
            f"refusing to overwrite remote multihorizon matrix: {stderr}"
        )
        ...
        json.dumps(
            {
                ...
                "task_count": len(specs),
                "total_workers": sum(
                    int(spec["cpu"]) - 8 for spec in specs
                ),
                "out": str(args.out),
            },
            sort_keys=True,

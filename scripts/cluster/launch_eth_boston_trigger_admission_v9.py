#!/usr/bin/env python3
"""Submit the Boston v11 trigger-window microscopic admission."""
...
LAUNCH_PROTOCOL = "tsc-v118r-eth-boston-trigger-admission-launch-v9"
SIGNATURE = (
    "CFCMT/v118r/eth-boston-trigger-admission-v9-"
...
def validate_trigger_package(manifest: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or gates.get("boston_protected_uncontrolled_merge_yield_repaired")
        is not True
        ...
        raise ValueError("Boston v11 package does not cover the trigger evidence window")
...
def build_spec(
    *,
    ...
    spec.update(
        {
            "description": "CFCMT V118r ETH Boston v11 5400s trigger admission",
            "signature": SIGNATURE,
            "ram_resource_family": "CFCMT-v118-eth-boston-trigger-admission-v11",
        }
    )
    return spec
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite Boston v11 trigger admission")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote Boston v11 trigger preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote Boston v11 trigger evidence changed: "
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected Boston v11 trigger admission")
    return 0

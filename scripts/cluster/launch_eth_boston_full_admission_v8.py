#!/usr/bin/env python3
"""Submit Boston full microscopic admission after the v11 trigger passes."""
...
from scripts.cluster.launch_eth_boston_trigger_admission_v5 import ALLOWED_NODES
...
LAUNCH_PROTOCOL = "tsc-v118r-eth-boston-full-admission-launch-v8"
SIGNATURE = (
    "CFCMT/v118r/eth-boston-full-admission-v8-"
    "protected-uncontrolled-merge-yield"
)
...
def validate_trigger_result(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("Boston v11 trigger-window admission did not pass")
    if setting.get("package_manifest_sha256") != expected_package_manifest_sha256:
        raise ValueError("Boston v11 trigger-window package identity changed")
    if (
        ...
        raise ValueError("Boston v11 trigger-window execution contract changed")
    if any(
        float(observed["final_time_sec"]) <= failure_time
        for failure_time in FORMER_FAILURE_TIMES_SEC
    ):
        raise ValueError("Boston v11 trigger did not cross all prior failures")
...
def build_spec(
    *,
    ...
    spec.update(
        {
            "description": (
                "CFCMT V118r ETH Boston v11 complete-demand libsumo full admission"
            ),
            "signature": SIGNATURE,
            "ram_resource_family": (
                "CFCMT-v118-eth-boston-full-admission-v11-complete"
            ),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite Boston v11 full admission")
    if _sha256(args.package_manifest_local) != args.expected_package_manifest_sha256:
        raise ValueError("local Boston v11 package manifest identity changed")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote Boston v11 full preflight failed: {stderr}")
    hash_code, hash_stdout, hash_stderr = scheduler.run_on(
        "node001",
        shlex.join(
            ["sha256sum", str(args.trigger_result_remote), str(package_manifest_remote)]
        ),
        timeout=120,
        check=False,
    )
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote Boston v11 full identities changed: "
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected Boston v11 full admission")
    return 0

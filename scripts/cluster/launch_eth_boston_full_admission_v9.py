#!/usr/bin/env python3
"""Submit Boston full microscopic admission after the v12 trigger passes."""
...
LAUNCH_PROTOCOL = "tsc-v118r-eth-boston-full-admission-launch-v9"
SIGNATURE = (
    "CFCMT/v118r/eth-boston-full-admission-v9-"
...
def validate_trigger_result(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("Boston v12 trigger-window admission did not pass")
    if setting.get("package_manifest_sha256") != expected_package_manifest_sha256:
        raise ValueError("Boston v12 trigger-window package identity changed")
    if (
        ...
        raise ValueError("Boston v12 trigger-window execution contract changed")
    if any(
        float(observed["final_time_sec"]) <= failure_time
        for failure_time in FORMER_FAILURE_TIMES_SEC
    ):
        raise ValueError("Boston v12 trigger did not cross all prior failures")
...
def build_spec(
    *,
    ...
    spec.update(
        {
            "description": (
                "CFCMT V118r ETH Boston v12 complete-demand libsumo full admission"
            ),
            "signature": SIGNATURE,
            "ram_resource_family": (
                "CFCMT-v118-eth-boston-full-admission-v12-complete"
            ),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite Boston v12 full admission")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote Boston v12 full preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote Boston v12 full identities changed: "
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected Boston v12 full admission")
    return 0

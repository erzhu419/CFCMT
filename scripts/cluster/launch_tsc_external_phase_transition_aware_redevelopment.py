#!/usr/bin/env python3
"""Launch the frozen v87 matrix on node001-node006 and sync summaries only."""
...
LAUNCH_PROTOCOL = "v87r83-phase-transition-aware-six-node-launch-v1"
...
def _command(
    *,
    ...
    values = [
        str(snapshot_root / "scripts/cluster/run_tsc_external_phase_transition_aware_redevelopment_shard.py"),
        "--protocol",
        ...
        "--diagnostic-audit",
        str(args.diagnostic_audit_remote),
        "--v83-protocol",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--diagnostic-audit-local", type=Path, required=True)
    parser.add_argument("--v86-audit-remote", type=Path, required=True)
    parser.add_argument("--diagnostic-audit-remote", type=Path, required=True)
    ...
    if args.out.exists() or args.local_results_root.exists():
        raise FileExistsError("refusing to overwrite v87 launch outputs")
    stage = json.loads(args.stage_manifest.read_text(encoding="utf-8"))
    snapshot_root = Path(str(stage["snapshot_root"]))
    protocol_path = PROJECT_ROOT / V87_PROTOCOL_RELATIVE
    ...
    if not (
        ...
        raise ValueError("v87 launch protocol or worker allocation changed")
    ...
    if int(code) != 0 or int(str(stdout).strip().splitlines()[-1]) != 0:
        raise RuntimeError(f"v87 remote results root is not empty: {stderr or stdout}")
    ...
    if any(row["returncode"] != 0 for row in node_results):
        raise RuntimeError(f"v87 node failure: {node_results}")
    ...
    if int(code) != 0 or remote_file_count != 114:
        raise RuntimeError(
            f"v87 remote file count mismatch: {remote_file_count}; {stderr}"
        )
    ...
    payload = {
        ...
        "diagnostic_audit_sha256": _sha256(args.diagnostic_audit_local),

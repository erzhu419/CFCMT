#!/usr/bin/env python3
"""Launch the frozen v80 matrix durably on six 192-core CPU nodes."""
...
LAUNCH_PROTOCOL = "v83r79-multihorizon-state-latent-six-node-durable-launch-v1"
...
def build_specs(
    *,
    ...
    if (
        ...
        raise ValueError("v80 launch authorization changed")
    wrapper = snapshot_root / "scripts/cluster/run_cfcmt_sumo122.sh"
    runner = (
        snapshot_root
        / "scripts/cluster/run_tsc_external_multihorizon_state_latent_closed_loop_shard.py"
    )
    specs = []
    for shard_index, node in enumerate(NODES):
        ...
        specs.append(
            {
                "description": f"CFCMT v80 state-latent shard {shard_index}",
                "project": "CFCMT",
                "cmd": command,
                "cwd": str(snapshot_root),
                "signature": f"CFCMT/v83r79/multihorizon-state-latent/shard-{shard_index}",
                "resource_family": "CFCMT-v80-state-latent-closed-loop-v1",
                "vram": 0,
                "ram_mb": int(ram_mb),
                "cpu": int(cpu_cores),
                "priority": "high",
...
def _sync_results(
    *,
    ...
    if local_results_root.exists():
        raise FileExistsError(f"refusing to overwrite local v80 results: {local_results_root}")
    ...
    for attempt in range(1, 6):
        ...
        if completed.returncode == 0:
            return attempt
        if attempt == 5:
            raise RuntimeError(f"v80 result sync failed: {completed.stderr[-4000:]}")
        time.sleep(2**attempt)
    raise AssertionError("unreachable v80 rsync retry state")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.transport_root.exists():
        raise FileExistsError("refusing to overwrite v80 launch evidence")
    ...
    try:
        ...
        def run(spec: Mapping[str, Any]) -> dict[str, Any]:
            ...
            returncode, stdout, run_stderr, durable = run_durable_remote_capture(
                ...
                run_id=f"cfcmt-v80-state-latent-{node}-v1",
        ...
        if not all(row["returncode"] == 0 for row in payload["direct_results"]):
            raise RuntimeError("one or more durable v80 shards failed")

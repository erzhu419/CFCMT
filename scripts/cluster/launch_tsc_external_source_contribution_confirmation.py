#!/usr/bin/env python3
"""Launch the frozen 512-rollout V91 source-contribution confirmation."""
...
def _command(
    *,
    ...
    values = [
        str(snapshot_root / "scripts/cluster/run_tsc_external_source_contribution_confirmation_shard.py"),
        "--confirmation-protocol",
        str(snapshot_root / CONFIRMATION_PROTOCOL_RELATIVE),
        "--v43-protocol",
        str(snapshot_root / V43_PROTOCOL_RELATIVE),
...
def _run_node(scheduler: Any, node: str, command: str) -> dict[str, Any]:
    ...
    code, stdout, stderr = scheduler.run_on(
        node, command, timeout=1200, check=False
    )
    return {
        "node": node,
        "returncode": int(code),
        "elapsed_sec": float(time.monotonic() - started),
        "stdout_tail": str(stdout)[-6000:],
        "stderr_tail": str(stderr)[-6000:],
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_results_root.exists():
        raise FileExistsError("refusing to overwrite V91 outputs")

    stage = _read_json(args.stage_manifest)
    parent = _read_json(args.parent_launch_manifest)
    protocol_path = PROJECT_ROOT / CONFIRMATION_PROTOCOL_RELATIVE
    protocol = _read_json(protocol_path)
    confirmation = protocol["confirmation"]
    output_roots = protocol["output_roots"]
    snapshot_root = Path(str(stage["snapshot_root"]))
    if not (
        protocol.get("protocol") == PROTOCOL
        and parent.get("protocol") == PARENT_LAUNCH_PROTOCOL
        and int(parent.get("task_count", -1)) == 56
        and int(confirmation.get("matrix_size", -1)) == 512
        and int(args.workers_per_node) == 22
        and str(args.remote_results_root) == str(output_roots["remote_results_root"])
        and str(args.local_results_root.resolve())
        == str(output_roots["local_results_root"])
    ):
        raise ValueError("V91 launch contract changed")
    ...
    if int(code) != 0 or int(str(stdout).strip().splitlines()[-1]) != 0:
        raise RuntimeError(f"V91 remote root is not empty: {stderr or stdout}")
    ...
    if any(row["returncode"] != 0 for row in node_results):
        raise RuntimeError(f"V91 node failure: {node_results}")
    ...
    if int(code) != 0 or remote_file_count != 1030:
        raise RuntimeError(f"V91 file count mismatch: {remote_file_count}; {stderr}")
    ...
    payload = {
        ...
        "confirmation_protocol_sha256": _sha256(protocol_path),

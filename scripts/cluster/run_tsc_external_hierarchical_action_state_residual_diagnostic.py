#!/usr/bin/env python3
"""Run and retrieve the v75 hierarchical action-state residual OOF diagnostic."""
...
LAUNCH_PROTOCOL = "v83r79-hierarchical-action-state-residual-launch-v1"
DIAGNOSTIC_PROTOCOL_RELATIVE = Path(
    "cf_h2o/config/traffic_signal_tsc_v75_external_v9_"
    "hierarchical_action_state_residual_diagnostic.json"
)
V74_PROTOCOL_RELATIVE = Path(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--v74-result-local", type=Path, required=True)
    parser.add_argument("--v74-result-remote", type=Path, required=True)
    ...
    if args.local_out.exists() or args.launch_out.exists():
        raise FileExistsError("refusing to overwrite local hierarchical residual diagnostic")
    stage = _read_json(args.stage_manifest)
    snapshot = Path(stage["snapshot_root"])
    scheduler = _load_scheduler(args.scheduler)
    node = str(args.node)
    if node not in {f"node{index:03d}" for index in range(1, 7)}:
        raise ValueError("hierarchical residual node is outside node001-node006")
    ...
    if int(code) != 0:
        raise FileExistsError(
            f"refusing to overwrite remote hierarchical residual result: {stderr}"
        )
    command = shlex.join(
        [
            str(snapshot / "scripts/cluster/run_cfcmt_sumo122.sh"),
            "-m",
            "cf_h2o.eval.traffic_signal_hierarchical_action_state_residual_diagnostic",
            "--diagnostic-protocol",
            str(snapshot / DIAGNOSTIC_PROTOCOL_RELATIVE),
            "--v74-protocol",
            str(snapshot / V74_PROTOCOL_RELATIVE),
            "--v74-result",
            str(args.v74_result_remote),
            "--cache-protocol",
    ...
    payload: dict[str, Any] = {
        ...
        "v74_result_local_sha256": _sha256(args.v74_result_local),
    ...
    if int(returncode) != 0 or sync_error is not None or result is None:
        raise RuntimeError(
            "hierarchical action-state residual diagnostic failed: "

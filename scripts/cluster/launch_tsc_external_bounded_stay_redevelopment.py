#!/usr/bin/env python3
"""Launch the frozen v88 matrix on node001-node006 and sync summaries only."""
...
LAUNCH_PROTOCOL = "v88r84-bounded-stay-six-node-launch-v1"
...
V87_PROTOCOL_RELATIVE = Path(
    "cf_h2o/config/traffic_signal_tsc_v87_external_v9_"
    "phase_transition_aware_redevelopment.json"
)
...
def _command(
    *,
    ...
    values = [
        str(snapshot_root / "scripts/cluster/run_tsc_external_bounded_stay_redevelopment_shard.py"),
        "--protocol",
        str(snapshot_root / V88_PROTOCOL_RELATIVE),
        "--v86-protocol",
        str(snapshot_root / V86_PROTOCOL_RELATIVE),
        "--v86-audit",
        str(args.v86_audit_remote),
        "--v87-protocol",
        str(snapshot_root / V87_PROTOCOL_RELATIVE),
        "--v87-audit",
        str(args.v87_audit_remote),
        "--v83-protocol",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--v86-audit-local", type=Path, required=True)
    parser.add_argument("--v87-protocol-local", type=Path, required=True)
    parser.add_argument("--v87-audit-local", type=Path, required=True)
    parser.add_argument("--v86-audit-remote", type=Path, required=True)
    parser.add_argument("--v87-audit-remote", type=Path, required=True)
    ...
    parser.add_argument("--workers-per-node", type=int, default=9)
    parser.add_argument("--scheduler", type=Path, default=DEFAULT_SCHEDULER)
    parser.add_argument("--out", type=Path, required=True)
    args = parser.parse_args(argv)
    if args.out.exists() or args.local_results_root.exists():
        raise FileExistsError("refusing to overwrite v88 launch outputs")
    stage = json.loads(args.stage_manifest.read_text(encoding="utf-8"))
    snapshot_root = Path(str(stage["snapshot_root"]))
    protocol_path = PROJECT_ROOT / V88_PROTOCOL_RELATIVE
    protocol = json.loads(protocol_path.read_text(encoding="utf-8"))
    if not (
        protocol.get("protocol") == PROTOCOL
        and int(protocol["development"]["new_rollout_count"]) == 54
        and int(args.workers_per_node) == 9
    ):
        raise ValueError("v88 launch protocol or worker allocation changed")
    ...
    if int(code) != 0 or int(str(stdout).strip().splitlines()[-1]) != 0:
        raise RuntimeError(f"v88 remote results root is not empty: {stderr or stdout}")
    ...
    if any(row["returncode"] != 0 for row in node_results):
        raise RuntimeError(f"v88 node failure: {node_results}")
    ...
    if int(code) != 0 or remote_file_count != 114:
        raise RuntimeError(
            f"v88 remote file count mismatch: {remote_file_count}; {stderr}"
        )
    ...
    payload = {
        ...
        "v86_audit_sha256": _sha256(args.v86_audit_local),
        "v87_protocol_sha256": _sha256(args.v87_protocol_local),
        "v87_audit_sha256": _sha256(args.v87_audit_local),

#!/usr/bin/env python3
"""Run and retrieve the v73 state-conditioned latent OOF diagnostic."""
...
LAUNCH_PROTOCOL = "v83r79-state-conditioned-latent-launch-v1"
...
def _retrieve_result(
    *,
    ...
    error = "result retrieval did not run"
    for attempt in range(1, 5):
        completed = subprocess.run(
            [
                ...
                f"{scheduler._ssh_target_for_node(node)}:{remote_path}",
                str(local_path),
            ],
            text=True,
            capture_output=True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--v70-result-local", type=Path, required=True)
    parser.add_argument("--v70-result-remote", type=Path, required=True)
    ...
    if args.local_out.exists() or args.launch_out.exists():
        raise FileExistsError("refusing to overwrite local state-conditioned diagnostic")
    stage = _read_json(args.stage_manifest)
    snapshot = Path(stage["snapshot_root"])
    scheduler = _load_scheduler(args.scheduler)
    node = str(args.node)
    if node not in {f"node{index:03d}" for index in range(1, 7)}:
        raise ValueError("state-conditioned diagnostic node is outside node001-node006")
    ...
    if int(code) != 0:
        raise FileExistsError(
            f"refusing to overwrite remote state-conditioned result: {stderr}"
        )
    command = shlex.join(
        [
            str(snapshot / "scripts/cluster/run_cfcmt_sumo122.sh"),
            "-m",
            "cf_h2o.eval.traffic_signal_state_conditioned_latent_diagnostic",
            "--diagnostic-protocol",
            str(snapshot / DIAGNOSTIC_PROTOCOL_RELATIVE),
            "--v70-protocol",
            str(snapshot / V70_PROTOCOL_RELATIVE),
            "--v70-result",
            str(args.v70_result_remote),
            "--v72-protocol",
    ...
    payload: dict[str, Any] = {
        ...
        "v70_result_local_sha256": _sha256(args.v70_result_local),
    ...
    if int(returncode) != 0 or sync_error is not None or result is None:
        raise RuntimeError(
            "state-conditioned latent diagnostic failed: "

#!/usr/bin/env python3
"""Freeze the v81 multihorizon estimand-aligned closed-loop development matrix."""
...
from cf_h2o.eval.traffic_signal_multihorizon_estimand_alignment_diagnostic import (  # noqa: E402
    AUTHORIZATION_DECISION as ALIGNMENT_AUTHORIZATION_DECISION,
    GLOBAL_COOLDOWN_CONTROLS,
    PRIORITY_THRESHOLDS,
    RESULT_PROTOCOL as ALIGNMENT_DIAGNOSTIC_PROTOCOL,
)
from cf_h2o.traffic_signal.dataset_cache import atomic_write_json  # noqa: E402
from scripts.cluster.audit_tsc_external_multihorizon_state_latent_closed_loop import (  # noqa: E402
...
PROTOCOL = "tsc-v83r79-external-v9-multihorizon-estimand-aligned-closed-loop-v1"
...
PRIMARY_CANDIDATE_SPECS = (
    ("mh_compact_g0_p005", 0, 0.05),
    ("mh_compact_g2_p005", 2, 0.05),
    ("mh_compact_g5_p000", 5, 0.00),
    ("mh_compact_g5_p002", 5, 0.02),
    ("mh_compact_g5_p005", 5, 0.05),
    ("mh_compact_g5_p010", 5, 0.10),
    ("mh_compact_g5_p015", 5, 0.15),
)
DIAGNOSTIC_CANDIDATE_SPECS = (
    ("mh_state_action_g5_p005", 5, 0.05),
)
PRIMARY_METHOD_KEYS = tuple(row[0] for row in PRIMARY_CANDIDATE_SPECS)
DIAGNOSTIC_METHOD_KEYS = tuple(row[0] for row in DIAGNOSTIC_CANDIDATE_SPECS)
...
def freeze_protocol(
    *,
    ...
    if output_path.exists():
        raise FileExistsError(f"refusing to overwrite v81 protocol: {output_path}")
    v80_protocol = _read_json(v80_protocol_path)
    v80_audit = _read_json(v80_audit_path)
    alignment_diagnostic = _read_json(alignment_diagnostic_path)
    ...
    if (
        v80_protocol.get("protocol") != V80_PROTOCOL
        or v80_audit.get("protocol") != V80_AUDIT_PROTOCOL
        or v80_audit.get("status") != "PASS"
        or v80_audit.get("decision") != V80_REJECTION_DECISION
        or v80_audit.get("integrity_gate", {}).get("passed") is not True
        or v80_audit.get("protocol_sha256") != _sha256(v80_protocol_path)
        or alignment_diagnostic.get("protocol")
        != ALIGNMENT_DIAGNOSTIC_PROTOCOL
        or alignment_diagnostic.get("status") != "PASS"
        or alignment_diagnostic.get("decision")
        != ALIGNMENT_AUTHORIZATION_DECISION
        or alignment_diagnostic.get("integrity_gate", {}).get("passed")
        is not True
        or alignment_diagnostic.get("v80_protocol_sha256")
        != _sha256(v80_protocol_path)
        or alignment_diagnostic.get("v80_audit_sha256")
        != _sha256(v80_audit_path)
        or tuple(alignment_diagnostic.get("priority_thresholds", ()))
        != PRIORITY_THRESHOLDS
        or tuple(alignment_diagnostic.get("global_cooldown_controls", ()))
        != GLOBAL_COOLDOWN_CONTROLS
        or int(
            alignment_diagnostic.get(
                "estimand_aligned_global_cooldown_intervals", -1
            )
        )
        != 5
        or artifact_protocol.get("protocol") != ARTIFACT_PROTOCOL
        ...
        raise ValueError("v81 parent, artifact, or runtime authorization changed")
    ...
    if any(v80_protocol.get(key) is None for key in inherited_sections):
        raise ValueError("v81 cannot inherit incomplete v80 protocol")
    ...
    for key in ARTIFACT_KEYS:
        ...
        if (
            ...
            raise ValueError(f"v81 artifact {key!r} identity changed")
    ...
    if (
        ...
        raise ValueError("v81 primary/diagnostic artifact roles changed")
    ...
    if any(
        ...
        raise ValueError("v81 parent runtime evidence changed")
    ...
    if (
        ...
        raise ValueError("v81 development partition or sealed roots changed")

    execution_candidates: list[dict[str, Any]] = []
    for artifact_key, specs in (
        (PRIMARY_ARTIFACT_KEY, PRIMARY_CANDIDATE_SPECS),
        (SECONDARY_ARTIFACT_KEY, DIAGNOSTIC_CANDIDATE_SPECS),
    ):
        artifact_record = artifact_records[artifact_key]
        for key, global_cooldown, min_priority in specs:
    ...
    payload = {
        ...
        "stage": "post_v80_rejection_and_alignment_diagnostic_pre_v81_outcomes",
        "development_parent": {
            "v80_protocol": _path_record(v80_protocol_path),
            "v80_independent_audit": _path_record(v80_audit_path),
            "estimand_alignment_diagnostic": _path_record(
                alignment_diagnostic_path
            ),
            "v80_decision": V80_REJECTION_DECISION,
            "alignment_decision": ALIGNMENT_AUTHORIZATION_DECISION,
        },
        "artifact": {
        ...
        "development": {
            ...
            "selection_rule": {
                ...
                "selector": (
                    ...
                    "bootstrap_upper_then_fewer_overrides_then_higher_priority_"
                    "threshold_then_longer_global_cooldown"
                ),
            },
            "bootstrap": {
                "replicates": 10000,
        ...
        "scientific_status": {
            ...
            "v80_closed_loop_outcomes_inspected_before_freeze": True,
            "v81_closed_loop_outcomes_inspected_before_freeze": False,
            "global_cooldown_grid_derived_from_v80_only": True,
            "priority_grid_derived_from_v80_only": True,
            "exact_estimand_alignment_global_cooldown_intervals": 5,
        },
        "claim_boundary": (
            "This 198-rollout matrix is adaptive successor development after the "
            "v80 rejection. The global-cooldown and priority grids were frozen "
            "from v80 traces before any v81 outcome. Only the seven compact-h60 "
            "candidates may authorize an untouched v84 experiment. The "
            "state-action-h60 candidate is diagnostic only. No efficacy or "
            "generalization claim is authorized by v81."
        ),
    }
    if payload["development"]["matrix_size"] != 198:
        raise RuntimeError("v81 closed-loop matrix size changed")

#!/usr/bin/env python3
"""Submit the frozen v40 14-network cross-fitted anchored OOF matrix."""
...
ASSIGNMENTS = {
    ...
    "node004": ("hangzhou_4x4", "hangzhou_4x4_hetero", "manhattan_28x7"),
    "node005": (
...
def build_specs(
    *,
    ...
    runner = snapshot_root / "scripts/cluster/run_tsc_cross_fitted_anchored_oof_shard.py"
    ...
    if len(assigned) != len(set(assigned)) or set(assigned) != expected_targets:
        raise ValueError("v40 node assignments do not exactly cover eligible targets")
    specs = []
    for node, targets in ASSIGNMENTS.items():
        for target in targets:
            ...
            if target_local.exists():
                raise FileExistsError(f"refusing to overwrite local v40 target: {target_local}")
            command = shlex.join(
                [
                    str(wrapper),
                    str(runner),
            ...
            specs.append(
                {
                    "description": f"CFCMT TSC v40 cross-fitted OOF {target}",
                    "project": "CFCMT-TSC-v40",
                    "cmd": command,
                    "cwd": str(PROJECT_ROOT),
                    "signature": f"CFCMT/tsc-v40/cross-fitted-oof/{target}",
                    "vram": 0,
                    "ram_mb": int(ram_mb),
                    "cpu": int(cpu_cores),
                    "priority": "normal",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload: dict[str, Any] = {
        "protocol": "v40r36-cross-fitted-anchored-scheduleurm-submission-v1",
    ...
    if args.submit:
        ...
        if completed.returncode != 0:
            ...
            raise RuntimeError("scheduleurm v40 bulk submission failed")
    _atomic_json(args.out, payload)
    print(
        json.dumps(
            {

#!/usr/bin/env python3
"""Submit complete-demand route admission for the Boston v9 package."""
...
LAUNCH_PROTOCOL = "tsc-v118r-eth-boston-route-admission-launch-v7b"
SIGNATURE = "CFCMT/v118r/eth-boston-route-admission-v7b-ramp-merge-yield"
...
def build_spec(
    *,
    ...
    spec.update(
        {
            "description": "CFCMT V118r ETH Boston v9 complete-demand route admission",
            "signature": SIGNATURE,
            "ram_resource_family": "CFCMT-v118-eth-boston-route-admission-v9",
        }
    )
    return spec
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite Boston v9 route launch artifact")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote Boston v9 route preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != [args.expected_package_manifest_sha256]:
        raise RuntimeError(
            "remote Boston v9 package identity changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v118r-eth-boston-route-admission-v7b",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected Boston v9 route admission")
    return 0

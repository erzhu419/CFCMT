#!/usr/bin/env python3
"""Submit seven V150M pressure-aligned source-utility targets."""
...
LAUNCH_PROTOCOL = "tsc-v150m-pressure-aligned-source-utility-launch-v1"
CONFIG_RELATIVE = Path(
    "cf_h2o/config/traffic_signal_tsc_v150m_pressure_aligned_source_utility.json"
)
MODULE = "cf_h2o.eval.traffic_signal_pressure_aligned_source_utility"
SIGNATURE_PREFIX = "CFCMT/v150m/pressure-aligned-source-utility-v1"
RESOURCE_FAMILY = "CFCMT-v150m-pressure-aligned-source-utility"
...
def _validate_config(config: Mapping[str, Any]) -> None:
    ...
    if (
        config.get("protocol")
        != "tsc-v150m-pressure-aligned-source-utility-v1"
        ...
        != "nested_per_state_pressure_aligned_source_mechanism_utility"
        ...
        != "separately_nested_same_capacity_pressure_aligned_utility_gate"
        ...
        raise ValueError("V150M frozen configuration changed")
...
def main(
    ...
    *,
    ...
    config_validator: Any = _validate_config,
    module: str = MODULE,
    signature_prefix: str = SIGNATURE_PREFIX,
    resource_family: str = RESOURCE_FAMILY,
    version_label: str = "V150M",
    launch_protocol: str = LAUNCH_PROTOCOL,
    intent_label: str = "CFCMT-v150m-pressure-aligned-source-utility-v1",
) -> int:
    ...
    parser.add_argument(
        "--target-city", action="append", choices=EXPECTED_CITY_GROUPS
    )
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError(
            f"refusing to overwrite {version_label} launch or results"
        )
    if not 1 <= int(args.cache_workers) <= 8:
        raise ValueError(f"{version_label} cache workers must be in [1, 8]")
    config_validator(_read_json(PROJECT_ROOT / config_relative))
    ...
    if (
        ...
        raise ValueError(f"{version_label} requires passing V150C authorization")
    if (
        rejection.get("protocol") != V150J_AGGREGATE_PROTOCOL
        or rejection.get("development_gate", {}).get("passed") is not False
    ):
        raise ValueError(f"{version_label} requires the frozen V150J rejection")
    if fit.get("protocol") != FIT_RESULT_PROTOCOL or fit.get("city") != "jinan":
        raise ValueError(f"{version_label} requires the frozen V115 architecture fit")
    if (
        ...
        raise ValueError(f"{version_label} source cache audit changed")
    ...
    remote_model_roots = [args.runtime_model_root / city for city in target_cities]
    checks = [
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote {version_label} preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            f"remote {version_label} evidence identities changed: {remote_hashes}; "
            f"{hash_stderr}"
        )
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"scheduler rejected {version_label} target matrix")
    return 0

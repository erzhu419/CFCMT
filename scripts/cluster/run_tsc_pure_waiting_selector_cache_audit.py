#!/usr/bin/env python3
"""Audit the V116 selector cache on node001 and retrieve compact JSON."""
...
from scripts.cluster.launch_tsc_pure_waiting_selector_development_cache import (  # noqa: E402
    MANIFEST_RELATIVE,
)


LAUNCH_PROTOCOL = "tsc-v116-pure-waiting-selector-cache-audit-launch-v1"
    ...
    *,
    ...
    return shlex.join(
        [
            ...
            "-m",
            "cf_h2o.eval.traffic_signal_pure_waiting_selector_cache_audit",
            "--protocol",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.local_out.exists() or args.launch_out.exists():
        raise FileExistsError("refusing to overwrite local selector-cache audit")
    ...
    if int(code) != 0:
        raise FileExistsError(
            f"refusing to overwrite remote selector-cache audit: {stderr}"
        )

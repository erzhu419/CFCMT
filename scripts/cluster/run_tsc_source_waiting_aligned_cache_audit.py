#!/usr/bin/env python3
"""Run the V112 source-cache audit on node001 and retrieve compact JSON."""
...
from scripts.cluster.launch_tsc_source_waiting_aligned_cache import (  # noqa: E402
    MANIFEST_RELATIVE,
)


LAUNCH_PROTOCOL = "tsc-v114-source-pure-waiting-cache-audit-launch-v1"
PROTOCOL_RELATIVE = Path(
    "cf_h2o/config/traffic_signal_tsc_v114_source_pure_waiting_admission.json"
)
...
def build_remote_command(
    *,
    ...
    cache_root: Path,
    task_root: Path,
    native_safety_audit: Path,
    remote_out: Path,
    read_workers: int,
    protocol_relative: Path = PROTOCOL_RELATIVE,
) -> str:
    return shlex.join(
        [
            "env",
            ...
            "-m",
            "cf_h2o.eval.traffic_signal_source_waiting_aligned_cache_audit",
            "--protocol",
            str(snapshot / protocol_relative),
            "--manifest",
            str(snapshot / MANIFEST_RELATIVE),
            "--native-safety-audit",
            str(native_safety_audit),
            "--conversion-root",
            str(conversion_root),
            "--cache-root",
            str(cache_root),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.local_out.exists() or args.launch_out.exists():
        raise FileExistsError("refusing to overwrite local source-cache audit")
    ...
    if int(code) != 0:
        raise FileExistsError(f"refusing to overwrite remote source-cache audit: {stderr}")
    command = build_remote_command(
        snapshot=snapshot,
        conversion_root=args.conversion_root,
        cache_root=args.cache_root,
        task_root=args.task_root,
        native_safety_audit=args.native_safety_audit,
        remote_out=args.remote_out,
        read_workers=args.read_workers,
        protocol_relative=Path("cf_h2o/config") / args.protocol.name,
    )
    returncode, stdout, run_stderr = scheduler.run_on(
        "node001", command, timeout=int(args.timeout_sec), check=False
    )
    sync_error = None
    if int(returncode) == 0:

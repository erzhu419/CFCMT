#!/usr/bin/env python3
"""Submit same-information external-city baseline freeze tasks."""
...
from cf_h2o.eval.traffic_signal_external_city_baseline_freeze import (  # noqa: E402
    BENCHMARK_FAMILIES,
)
...
def build_specs(
    *,
    ...
    if tuple(target.get("same_information_baseline_families", ())) != (
        BENCHMARK_FAMILIES
    ):
        raise ValueError("external baseline family protocol changed")
    if set(target["external_city_scenarios"]) != set(CITY_NODES):
        raise ValueError("external baseline city assignment changed")
    ...
    for city, node in CITY_NODES.items():
        ...
        if (local_result / "freeze.json").exists() or (
            local_result / "baseline_model_ensemble.pkl"
        ).exists():
            raise FileExistsError(
                f"refusing to overwrite external baseline freeze: {local_result}"
            )
        command = shlex.join(
            [
                str(wrapper),
                "-m",
                "cf_h2o.eval.traffic_signal_external_city_baseline_freeze",
                "--source-cache-root",
                ...
                str(fold_workers),
                "--model-out",
                str(remote_result / "baseline_model_ensemble.pkl"),
                "--out",
        ...
        specs.append(
            {
                "description": f"CFCMT v42r38 external baseline freeze {city}",
                "project": "CFCMT",
                "cmd": command,
                "cwd": str(snapshot_root),
                "signature": f"CFCMT/v42r38/external-baseline-freeze-v1/{city}",
                "resource_family": "CFCMT-external-baseline-freeze-v1",
                "vram": 0,
                "ram_mb": int(ram_mb),
                "cpu": int(cpu_cores),
                "priority": "high",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload: dict[str, Any] = {
        "protocol": "v42r38-external-city-baseline-freeze-submission-v1",
    ...
    if args.submit:
        completed = subprocess.run(
            [
                ...
                "--json",
                "--intent-label",
                "CFCMT-v42r38-external-baseline-freeze",
            ],
        ...
        if completed.returncode != 0:
            ...
            raise RuntimeError("external baseline freeze submission failed")
    _atomic_json(args.out, payload)
    print(
        json.dumps(
            {

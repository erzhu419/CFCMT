#!/usr/bin/env python3
"""Submit the frozen V126 source-conditioned ranker feasibility test."""
...
LAUNCH_PROTOCOL = "tsc-v126-source-ranker-feasibility-launch-v1"
SIGNATURE = "CFCMT/v126/source-ranker-feasibility-b500-v1"
TARGET_BUDGET = 500
    ...
    *,
    ...
    command = shlex.join(
        [
            ...
            str(wrapper),
            "-m",
            "cf_h2o.eval.traffic_signal_source_ranker_feasibility",
            "--prediction-artifact",
    ...
    return {
        "description": "CFCMT V126 source-conditioned ranker feasibility",
        "project": "CFCMT",
        "cmd": command,
        "cwd": str(snapshot_root),
        "signature": SIGNATURE,
        "resource_family": "CFCMT-v126-source-ranker-feasibility",
        "vram": 0,
        "ram_mb": 65536,
        "cpu": 20,
        "priority": "high",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite V126 launch artifact")
    if not 1 <= int(args.cache_workers) <= 20:
        raise ValueError("V126 cache workers must be in [1, 20]")
    if not 1 <= int(args.fold_workers) <= 20:
        raise ValueError("V126 fold workers must be in [1, 20]")
    if len(str(args.prediction_artifact_sha256)) != 64:
        raise ValueError("V126 prediction artifact SHA-256 is malformed")
    ...
    if (
        ...
        raise ValueError("V126 selector cache audit changed")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote V126 preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote V126 evidence identities changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v126-source-ranker-feasibility-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V126 source ranker")
    return 0

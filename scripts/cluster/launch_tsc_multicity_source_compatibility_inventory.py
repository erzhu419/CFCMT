#!/usr/bin/env python3
"""Submit the seven complete-city V144 ordered-pair inventory tasks."""
...
LAUNCH_PROTOCOL = "tsc-v144-multicity-source-compatibility-inventory-launch-v1"
CONFIG_RELATIVE = Path(
    "cf_h2o/config/traffic_signal_tsc_v144_multicity_source_compatibility_inventory.json"
)
MODULE = "cf_h2o.eval.traffic_signal_multicity_source_compatibility_inventory"
SIGNATURE_PREFIX = "CFCMT/v144/multicity-source-compatibility-inventory-v1"
RESOURCE_FAMILY = "CFCMT-v144-multicity-source-compatibility"


def _validate_config(config: Mapping[str, Any]) -> None:
    if (
        config.get("protocol")
        != "tsc-v144-multicity-source-compatibility-inventory-v1"
        or tuple(str(value) for value in config.get("target_city_groups", ()))
        != EXPECTED_CITY_GROUPS
        or int(config.get("target_budget", -1)) != TARGET_BUDGET
        or int(config.get("ordered_pair_count", -1)) != 42
        or config.get("city_signature_information")
        != "features_and_static_context_only_no_priors_or_targets"
    ):
        raise ValueError("V144 frozen protocol configuration changed")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite V144 launch or local results")
    if not 1 <= int(args.cache_workers) <= 8 or not 1 <= int(args.fit_workers) <= 8:
        raise ValueError("V144 workers must be in [1, 8]")
    ...
    if fit.get("protocol") != FIT_RESULT_PROTOCOL or fit.get("city") != "jinan":
        raise ValueError("V144 requires the frozen V115 architecture fit")
    if (
        ...
        raise ValueError("V144 source cache audit changed")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote V144 preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote V144 evidence identities changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v144-multicity-source-compatibility-inventory-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V144 ordered-pair inventory")
    return 0

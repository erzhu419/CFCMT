#!/usr/bin/env python3
"""Submit complete-demand route admission for the Boston v11 package."""
...
from scripts.cluster.launch_eth_boston_route_admission_v8 import (
...
LAUNCH_PROTOCOL = "tsc-v118r-eth-boston-route-admission-launch-v9"
SIGNATURE = (
    "CFCMT/v118r/eth-boston-route-admission-v9-"
...
def build_spec(
    *,
    ...
    spec.update(
        {
            "description": "CFCMT V118r ETH Boston v11 complete-demand route admission",
            "signature": SIGNATURE,
            "ram_resource_family": "CFCMT-v118-eth-boston-route-admission-v11",
        }
    )
    return spec
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite Boston v11 route admission")
    ...
    validate_protected_uncontrolled_merge_manifest(package_manifest)
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote Boston v11 route preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != [args.expected_package_manifest_sha256]:
        raise RuntimeError(
            "remote Boston v11 package identity changed: "
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected Boston v11 route admission")
    return 0

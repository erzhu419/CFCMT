#!/usr/bin/env python3
"""Run one process-isolated shard of the frozen v83 closed-loop matrix."""
...
from cf_h2o.eval.traffic_signal_uncertainty_horizon_closed_loop_development import (
    ...
    run_uncertainty_horizon_closed_loop_development,
)
...
SHARD_PROTOCOL = "tsc-v83r79-uncertainty-horizon-process-shard-v1"
...
def shard_identities(
    ...
    if shard_count < 1 or not 0 <= shard_index < shard_count:
        raise ValueError("invalid v83 shard index/count")
    return tuple(
        identity
...
def _validate_existing(
    *,
    ...
    if result_path.is_file() != tripinfo_path.is_file():
        raise RuntimeError(f"partial v83 result exists: {result_path.parent}")
    ...
    if not (
        ...
        raise RuntimeError(f"existing v83 result is invalid: {result_path}")
    return result
...
def _validated_completed_summary(
    *,
    ...
    if not (
        ...
        raise RuntimeError(f"completed v83 shard is invalid: {summary_path}")
    for identity in identities:
        ...
        if (
            ...
            raise RuntimeError(f"completed v83 shard row changed: {result_path}")
...
def run_shard(
    *,
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("v83 shard protocol changed")
        ...
        tasks.append(
            {
                ...
                "protocol_sha256": protocol_sha,
                "v81_protocol_path": str(v81_protocol_path),
                "v81_audit_path": str(v81_audit_path),
    ...
    if not passed:
        raise RuntimeError(f"v83 shard coverage failed: {shard_index}")
    return payload

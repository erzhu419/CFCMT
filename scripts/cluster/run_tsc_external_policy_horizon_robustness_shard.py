#!/usr/bin/env python3
"""Run one process-parallel shard of the frozen v49 robustness matrix."""
...
from cf_h2o.eval.traffic_signal_external_policy_horizon_robustness import (
    ...
    run_policy_horizon_robustness,
)


SHARD_PROTOCOL = "tsc-v49r45-policy-horizon-robustness-process-shard-v1"
...
def shard_identities(
    ...
    if shard_count < 1 or not 0 <= shard_index < shard_count:
        raise ValueError("invalid v49 shard index/count")
    return tuple(
        identity
...
def _validate_existing(
    *, result_path: Path, tripinfo_path: Path, identity: tuple[str, str, int, str]
    ...
    if result_path.is_file() != tripinfo_path.is_file():
        raise RuntimeError(f"partial v49 result exists: {result_path.parent}")
    ...
    if not valid:
        raise RuntimeError(f"existing v49 result failed validation: {result_path}")
    return result


def _run_one(payload: Mapping[str, Any]) -> dict[str, Any]:
    city, scenario, seed, policy = tuple(payload["identity"])
    identity = (str(city), str(scenario), int(seed), str(policy))
    result_path = Path(str(payload["result_path"]))
    tripinfo_path = Path(str(payload["tripinfo_path"]))
    existing = _validate_existing(
        result_path=result_path, tripinfo_path=tripinfo_path, identity=identity
    )
    ...
    if existing:
        ...
        result = run_policy_horizon_robustness(
            ...
            adaptation_selection_audit_path=Path(
                str(payload["adaptation_selection_audit_path"])
            ),
            expected_adaptation_selection_sha256=str(
                payload["expected_adaptation_selection_sha256"]
            ),
...
def run_shard(
    *,
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("v49 shard protocol changed")
    ...
    if summary_path.exists():
        raise FileExistsError(f"refusing to overwrite v49 shard audit: {summary_path}")
    tasks = []
        ...
        tasks.append(
            {
                ...
                "adaptation_selection_audit_path": str(
                    adaptation_selection_audit_path
                ),
                "expected_adaptation_selection_sha256": expected_adaptation_selection_sha256,
    ...
    payload = {
        ...
        "adaptation_selection_audit_sha256": _sha256(
            adaptation_selection_audit_path
        ),
    ...
    if not passed:
        raise RuntimeError(f"v49 shard coverage failed: {shard_index}")
    return payload

#!/usr/bin/env python3
"""Launch ordered Boston v16 implicit no-TLS priority admission stages."""
...
from scripts.cluster.launch_eth_boston_v15_admission import (
    ...
    validate_trigger_result as _validate_v15_trigger_result,
)
...
LAUNCH_PROTOCOL = "tsc-v150-eth-boston-v16-admission-pipeline-launch-v1"
SIGNATURE_ROOT = "CFCMT/v150/eth-boston-v16-implicit-no-tls-major"
...
def validate_trigger_package(manifest: Mapping[str, Any]) -> None:
    ...
    if (
        manifest.get("protocol") != V16_PACKAGE_PROTOCOL
        or manifest.get("passed") is not True
        or manifest.get("city_code") != "BOS"
        or gates.get("boston_implicit_no_tls_major_states_repaired") is not True
        ...
        raise ValueError("Boston v16 package does not cover the trigger evidence window")


def validate_trigger_result(
    result: Mapping[str, Any], *, expected_package_manifest_sha256: str
) -> None:
    _validate_v15_trigger_result(
        result,
    ...
    if any(final_time <= value for value in FORMER_FAILURE_TIMES_SEC):
        raise ValueError("Boston v16 trigger did not cross every prior failure")
    ...
    *,
    ...
    command = shlex.join(
        [
            ...
            str(wrapper),
            "-m",
            "scripts.data.repair_eth_boston_implicit_no_tls_major",
            "--base-root",
            str(base_root),
            "--output-root",
            str(output_root),
    ...
    return {
        "description": "CFCMT Boston v16 implicit no-TLS major package repair",
        "project": "CFCMT",
        "cmd": "mkdir -p "
        ...
        "resource_family": "CFCMT-v150-eth-boston-v16-package",
        "ram_resource_family": "CFCMT-v150-eth-boston-v16-package",
        "vram": 0,
        "ram_mb": 32_768,
        "cpu": 2,
        "priority": "high",
...
def _version_spec(spec: Mapping[str, Any], *, stage: str) -> dict[str, Any]:
    ...
    value.update(
        {
            "description": f"CFCMT Boston v16 {stage} admission",
            "signature": f"{SIGNATURE_ROOT}/{stage}-v1",
            "ram_resource_family": f"CFCMT-v150-eth-boston-{stage}-admission-v16",
        }
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError(f"refusing to overwrite Boston v16 {args.stage} evidence")
    ...
    if args.stage == "package":
        ...
        if base_manifest.get("protocol") != V15_PACKAGE_PROTOCOL:
            raise ValueError("Boston v16 repair requires the admitted v15 package")
        output_package_root = (
            args.package_root.parent / "boston_complete_published_package_v16"
        )
        _verify_remote_inputs(
            scheduler=scheduler,
            paths_and_hashes=(
    ...
    completed = _submit(
        ...
        spec=spec,
        intent_label=f"CFCMT-v150-eth-boston-v16-{args.stage}",
    )
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"scheduler rejected Boston v16 {args.stage} admission")
    return 0

#!/usr/bin/env python3
"""Run one process-isolated shard of the waiting-aligned latent matrix."""
...
from cf_h2o.eval.traffic_signal_waiting_aligned_latent_closed_loop_development import (
    ...
    run_waiting_aligned_latent_closed_loop_development,
)
...
SHARD_PROTOCOL = "tsc-v83r79-waiting-aligned-latent-process-shard-v1"
...
def shard_identities(
    ...
    if shard_count < 1 or not 0 <= shard_index < shard_count:
        raise ValueError("invalid waiting-aligned latent shard index/count")
    return tuple(
        identity
...
def _validate_existing(
    *,
    ...
    if result_path.is_file() != tripinfo_path.is_file():
        raise RuntimeError(f"partial waiting-aligned result exists: {result_path.parent}")
    ...
    if not (
        ...
        raise RuntimeError(f"existing waiting-aligned result is invalid: {result_path}")
    return result
...
def _validated_completed_summary(
    *,
    ...
    if not (
        ...
        raise RuntimeError(f"completed waiting-aligned shard is invalid: {summary_path}")
    for identity in identities:
        ...
        if (
            ...
            raise RuntimeError(f"completed shard row hash changed: {result_path}")
...
def run_shard(
    *,
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("waiting-aligned shard protocol changed")
    ...
    if not passed:
        raise RuntimeError(f"waiting-aligned shard coverage failed: {shard_index}")
    return payload

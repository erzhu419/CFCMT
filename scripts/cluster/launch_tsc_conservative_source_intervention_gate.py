#!/usr/bin/env python3
"""Submit the frozen V124 conservative source-intervention gate."""
...
LAUNCH_PROTOCOL = "tsc-v124-conservative-source-intervention-gate-launch-v1"
SIGNATURE = "CFCMT/v124/conservative-source-intervention-gate-v1"
    ...
    *,
    ...
    command = shlex.join(
        [
            ...
            str(wrapper),
            "-m",
            "cf_h2o.eval.traffic_signal_conservative_source_intervention_gate",
            "--prediction-artifact",
    ...
    return {
        "description": "CFCMT V124 conservative source-intervention gate",
        "project": "CFCMT",
        "cmd": command,
        "cwd": str(snapshot_root),
        "signature": SIGNATURE,
        "resource_family": "CFCMT-v124-conservative-source-gate",
        "vram": 0,
        "ram_mb": 65536,
        "cpu": 20,
        "priority": "high",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite V124 launch artifact")
    if not 1 <= int(args.cache_workers) <= 20:
        raise ValueError("V124 cache workers must be in [1, 20]")
    if len(str(args.prediction_artifact_sha256)) != 64:
        raise ValueError("V124 prediction artifact SHA-256 is malformed")
    ...
    if (
        ...
        raise ValueError("V124 selector cache audit changed")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote V124 preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote V124 evidence identities changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v124-conservative-source-intervention-gate-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V124 conservative source gate")
    return 0

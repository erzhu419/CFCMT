#!/usr/bin/env python3
"""Launch the ordered Boston v14 package, route, trigger, and full admissions."""
...
from scripts.cluster.launch_eth_boston_v13_admission import (
    ...
    build_full_spec as _build_v13_full_spec,
    build_route_spec as _build_v13_route_spec,
    build_trigger_spec as _build_v13_trigger_spec,
    validate_trigger_result as _validate_v13_trigger_result,
)
...
LAUNCH_PROTOCOL = "tsc-v150-eth-boston-v14-admission-pipeline-launch-v1"
SIGNATURE_ROOT = "CFCMT/v150/eth-boston-v14-uncontrolled-major-state"
...
def validate_trigger_package(manifest: Mapping[str, Any]) -> None:
    ...
    if (
        manifest.get("protocol") != V14_PACKAGE_PROTOCOL
        or manifest.get("passed") is not True
        or manifest.get("city_code") != "BOS"
        or gates.get("boston_systemic_uncontrolled_major_states_repaired")
        is not True
        ...
        raise ValueError("Boston v14 package does not cover the trigger evidence window")


def validate_trigger_result(
    result: Mapping[str, Any], *, expected_package_manifest_sha256: str
) -> None:
    _validate_v13_trigger_result(
        result,
    ...
    if any(final_time <= failure_time for failure_time in FORMER_FAILURE_TIMES_SEC):
        raise ValueError("Boston v14 trigger did not cross every prior failure")
    ...
    *,
    ...
    command = shlex.join(
        [
            ...
            str(wrapper),
            "-m",
            "scripts.data.repair_eth_boston_systemic_uncontrolled_major",
            "--base-root",
            str(base_root),
            "--output-root",
            str(output_root),
    ...
    return {
        "description": "CFCMT Boston v14 systemic uncontrolled-major package repair",
        "project": "CFCMT",
        "cmd": "mkdir -p "
        ...
        "resource_family": "CFCMT-v150-eth-boston-v14-package",
        "ram_resource_family": "CFCMT-v150-eth-boston-v14-package",
        "vram": 0,
        "ram_mb": 32_768,
        "cpu": 2,
        "priority": "high",
...
def _version_spec(
    spec: Mapping[str, Any], *, stage: str
) -> dict[str, Any]:
    value = dict(spec)
    value.update(
        {
            "description": f"CFCMT Boston v14 {stage} admission",
            "signature": f"{SIGNATURE_ROOT}/{stage}-v1",
            "ram_resource_family": f"CFCMT-v150-eth-boston-{stage}-admission-v14",
        }
    )
...
def build_route_spec(**kwargs: Any) -> dict[str, Any]:
    return _version_spec(_build_v13_route_spec(**kwargs), stage="route")


def build_trigger_spec(**kwargs: Any) -> dict[str, Any]:
    return _version_spec(_build_v13_trigger_spec(**kwargs), stage="trigger")


def build_full_spec(**kwargs: Any) -> dict[str, Any]:
    return _version_spec(_build_v13_full_spec(**kwargs), stage="full")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError(f"refusing to overwrite Boston v14 {args.stage} evidence")
    ...
    if args.stage == "package":
        ...
        if base_manifest.get("protocol") != V13_PACKAGE_PROTOCOL:
            raise ValueError("Boston v14 repair requires the admitted v13 package")
        output_package_root = (
            args.package_root.parent / "boston_complete_published_package_v14"
        )
        _verify_remote_inputs(
            scheduler=scheduler,
            paths_and_hashes=(
    ...
    completed = _submit(
        ...
        spec=spec,
        intent_label=f"CFCMT-v150-eth-boston-v14-{args.stage}",
    )
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"scheduler rejected Boston v14 {args.stage} admission")
    return 0

#!/usr/bin/env python3
"""Audit the frozen v78 paired development matrix and its advance gate."""
...
from scripts.cluster.run_tsc_external_topology_veto_closed_loop_development_shard import (  # noqa: E402
    SHARD_PROTOCOL,
)


AUDIT_PROTOCOL = "tsc-v78r74-topology-veto-closed-loop-development-audit-v2"
PASS_DECISION = "authorize_topology_veto_untouched_validation_freeze_only"
FAIL_DECISION = "reject_topology_veto_development_and_preserve_sealed_seeds"
...
def _active_city_summary(
    *,
    ...
    if set(by_key) != expected:
        raise ValueError(f"v78 active city pair matrix changed for {city}")
    seed_rows = {}
    for seed in seeds:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError(f"refusing to overwrite v78 audit: {args.out}")
    payload = audit(

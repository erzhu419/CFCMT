#!/usr/bin/env python3
"""Independently audit and select the frozen v81 closed-loop matrix."""
...
from scripts.cluster.launch_tsc_external_multihorizon_estimand_aligned_closed_loop import (  # noqa: E402
    LAUNCH_PROTOCOL,
)
from scripts.cluster.run_tsc_external_multihorizon_estimand_aligned_closed_loop_shard import (  # noqa: E402
    SHARD_PROTOCOL,
)


AUDIT_PROTOCOL = "tsc-v83r79-multihorizon-estimand-aligned-closed-loop-audit-v1"
AUTHORIZATION_DECISION = "authorize_estimand_aligned_confirmatory_freeze"
DIAGNOSTIC_ONLY_DECISION = (
    "retain_phase_pressure_and_require_new_primary_after_estimand_alignment"
)
REJECTION_DECISION = "retain_phase_pressure_and_reject_estimand_alignment"
...
def _load_rollout(
    *,
    ...
    if not (
        ...
        raise ValueError(f"v81 rollout changed: {result_path}")
    return result
...
def select_primary_candidate(
    ...
    return min(
        ...
        key=lambda row: (
            ...
            -float(row["min_priority"]),
            -int(row["global_cooldown_intervals"]),
            str(row["policy"]),
        ),
        default=None,
    )
...
def audit_matrix(
    *, protocol_path: Path, launch_path: Path, results_root: Path
    ...
    if (
        ...
        or int(launch.get("matrix_size", -1)) != 198
        ...
        raise ValueError("v81 launch evidence changed")
    ...
    for shard_index in range(len(NODES)):
        ...
        if not (
            ...
            and shard.get("v80_protocol_sha256")
            == protocol["development_parent"]["v80_protocol"]["sha256"]
            and shard.get("v80_audit_sha256")
            == protocol["development_parent"]["v80_independent_audit"][
                "sha256"
            ]
            and shard.get("alignment_diagnostic_sha256")
            == protocol["development_parent"][
                "estimand_alignment_diagnostic"
            ]["sha256"]
        ):
            raise ValueError(f"v81 shard changed: {path}")
        shard_rows.append(
            {
    ...
    contract_ok = True
    parent_hashes = {
        "v80_protocol_sha256": protocol["development_parent"]["v80_protocol"][
            "sha256"
        ],
        "v80_audit_sha256": protocol["development_parent"][
            "v80_independent_audit"
        ]["sha256"],
        "alignment_diagnostic_sha256": protocol["development_parent"][
            "estimand_alignment_diagnostic"
        ]["sha256"],
    }
    for seed in seeds:
        ...
        contract_ok = contract_ok and (
            ...
            and all(baseline.get(key) == value for key, value in parent_hashes.items())
        )
        for policy in METHOD_KEYS:
            ...
            contract_ok = contract_ok and (
                ...
                and all(method.get(key) == value for key, value in parent_hashes.items())
    ...
    integrity = {
        ...
        "matrix_size_exact": len(results) == 198,
...
def _report(payload: Mapping[str, Any]) -> str:
    lines = [
        "# Multihorizon Estimand-Aligned Closed-Loop Development Audit",
        "",
        ...
        "",
        "| Policy | Role | Global cooldown | Min priority | Mean relative delta | 95% CI | Improved seeds | Active seeds | Overrides | Feasible |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|:---:|",
    ]
    for row in payload["candidate_summaries"]:
        ...
        lines.append(
            ...
            f"{row['global_cooldown_intervals']} | {row['min_priority']:.2f} | "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.report.exists():
        raise FileExistsError("refusing to overwrite v81 audit")

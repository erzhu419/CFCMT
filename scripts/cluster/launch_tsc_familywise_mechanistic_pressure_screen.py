#!/usr/bin/env python3
"""Submit the frozen V132 familywise generalized-pressure screen."""
...
LAUNCH_PROTOCOL = "tsc-v132-familywise-mechanistic-pressure-screen-launch-v1"
SIGNATURE = "CFCMT/v132/familywise-mechanistic-pressure-screen-v1"
    ...
    *,
    ...
    command = shlex.join(
        [
            ...
            str(wrapper),
            "-m",
            "cf_h2o.eval.traffic_signal_familywise_mechanistic_pressure_screen",
            "--selector-cache-root",
    ...
    return {
        "description": "CFCMT V132 familywise mechanistic pressure screen",
        "project": "CFCMT",
        "cmd": command,
        "cwd": str(snapshot_root),
        "signature": SIGNATURE,
        "resource_family": "CFCMT-v132-familywise-mechanistic-pressure-screen",
        "vram": 0,
        "ram_mb": RAM_MB,
        "cpu": 20,
        "priority": "high",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite V132 launch artifact")
    if not 1 <= int(args.cache_workers) <= 20:
        raise ValueError("V132 cache workers must be in [1, 20]")
    if not 1 <= int(args.fold_workers) <= 20:
        raise ValueError("V132 fold workers must be in [1, 20]")
    nodes = tuple(value.strip() for value in args.nodes.split(",") if value.strip())
    if not nodes or len(set(nodes)) != len(nodes):
        raise ValueError("V132 execution nodes must be nonempty and unique")
    if not set(nodes).issubset(ALLOWED_NODES):
        raise ValueError("V132 execution nodes include a non-admitted CPU node")
    ...
    if (
        ...
        raise ValueError("V132 selector cache audit changed")
    ...
    for node in nodes:
        ...
        if int(code) != 0:
            raise RuntimeError(f"remote V132 preflight failed on {node}: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != [selector_sha]:
        raise RuntimeError(
            "remote V132 selector evidence changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v132-familywise-mechanistic-pressure-screen-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V132 familywise pressure screen")
    return 0

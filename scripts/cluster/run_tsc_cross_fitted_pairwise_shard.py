#!/usr/bin/env python3
"""Run and validate one v36/r32 cross-fitted target shard."""
...
MODULE = "cf_h2o.eval.traffic_signal_cross_fitted_pairwise_screen"
...
def run_shard(
    *,
    ...
    if any(path.exists() for path in (result_path, log_path, launch_path, shard_path)):
        raise FileExistsError(f"refusing to overwrite cross-fit shard: {target_root}")
    command = [
        sys.executable,
        "-m",
        MODULE,
    ...
    _atomic_json(
        launch_path,
        {
            "protocol": "v36r32-single-target-cross-fit-launch-v1",
    ...
    payload: dict[str, Any] = {
        "protocol": "v36r32-single-target-cross-fit-shard-v1",
        "hostname": socket.gethostname(),
        "target": target,
        "started_at_utc": started_at,
    ...
    if completed.returncode == 0 and result_path.exists():
        ...
        if (
            ...
            and isinstance(fold_results, list)
            and len(fold_results) == 5
            and isinstance(oof_rows, list)
            and len(oof_rows) == 60
    ...
    if not payload["passed"]:
        raise RuntimeError(f"cross-fit shard failed: {payload}")
    return payload

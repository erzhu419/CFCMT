#!/usr/bin/env python3
"""Audit the six-seed, four-scenario v65 long-horizon cache."""
...
from scripts.cluster.launch_tsc_external_long_horizon_target_veto_cache import (  # noqa: E402
    LAUNCH_PROTOCOL,
    NODES,
)
from scripts.cluster.recover_tsc_external_long_horizon_target_veto_cache_sync import (  # noqa: E402
    RECOVERY_PROTOCOL,
)
...
AUDIT_PROTOCOL = "tsc-v65r61-external-v9-long-horizon-target-veto-cache-audit-v1"
AUDIT_DECISION = "authorize_target_veto_oof_training_only"
...
def audit_long_horizon_cache(
    *,
    ...
    recovery = dict(launch.get("recovery", {}))
    recovery_gate = dict(recovery.get("gate", {}))
    launch_gate = {
        ...
        == len(scenarios) * shards,
        "local_cache_synced": launch.get("cache_synced") is True,
        "transport_recovery_audited_if_present": not recovery
        or (
            recovery.get("protocol") == RECOVERY_PROTOCOL
            and bool(recovery_gate.get("passed", False))
            and int(recovery.get("remote_cache_file_count", -1))
            == int(collection["expected_cache_file_count"])
            and int(recovery.get("local_cache_file_count", -1))
            == int(collection["expected_cache_file_count"])
            and len(recovery.get("summary_evidence", ())) == len(seeds)
        ),
    }
    ...
    for seed in seeds:
        ...
        setting_gate = {
            ...
            == float(collection["duration_sec"])
            and int(setting.get("control_interval_sec", -1))
            == int(collection["control_interval_sec"])
            and float(setting.get("warmup_sec", -1))
            == float(collection["warmup_sec"])
            and int(setting.get("counterfactual_horizon_intervals", -1))
            == int(collection["counterfactual_horizon_intervals"])
            and int(setting.get("collection_shards", -1)) == shards
            and int(setting.get("workers", -1))
            == int(collection["workers_per_seed"])
            and setting.get("behavior_policy") == collection["behavior_policy"]
            and setting.get("cache_version") == COUNTERFACTUAL_CACHE_VERSION,
            "authorization_matches": setting.get(
        ...
        for scenario in scenarios:
            ...
            files = tuple(
                Path(value).name for value in diagnostic.get("cache_files", ())
            )
            declared_files.extend(files)
            scenario_gate = {
                "unique_shards": len(files) == shards
                and len(set(files)) == shards,
                "full_tls_coverage": float(
                    diagnostic.get("tls_coverage_fraction", -1)
                )
                == 1.0,
                "three_hundred_second_rollout_value": int(
                    diagnostic.get("rollout_value_horizon_sec", -1)
                )
                == int(collection["rollout_value_horizon_sec"]),
            ...
            scenario_rows.append(
                {
                    ...
                    "rollout_value_horizon_sec": diagnostic.get(
    ...
    try:
        ...
        errors.append(f"cannot hash v65 cache: {exc}")
    observed_files = {path.name for path in Path(cache_root).glob("*.npz")}
    expected_file_count = int(collection["expected_cache_file_count"])
    integrity_gate = {
    ...
    return {
        ...
        "claim_boundary": (
            "Cache provenance, full-horizon coverage, deterministic replay, and "
            "safety only. No veto-model or closed-loop efficacy is evaluated."
        ),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError(f"refusing to overwrite v65 cache audit: {args.out}")
    payload = audit_long_horizon_cache(

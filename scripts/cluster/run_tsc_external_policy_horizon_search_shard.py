#!/usr/bin/env python3
"""Run one process-parallel shard of the frozen v48 policy-horizon matrix."""
...
SHARD_PROTOCOL = "tsc-v48r44-policy-horizon-process-shard-v1"
...
def shard_identities(
    ...
    if shard_count < 1 or not 0 <= shard_index < shard_count:
        raise ValueError("invalid v48 shard index/count")
    return tuple(
        identity
...
def _validate_existing(
    *, result_path: Path, tripinfo_path: Path, identity: tuple[str, str, int, str]
) -> dict[str, Any]:
    city, scenario, seed, candidate = identity
    if result_path.is_file() != tripinfo_path.is_file():
        raise RuntimeError(f"partial v48 result exists: {result_path.parent}")
    ...
    if not valid:
        raise RuntimeError(f"existing v48 result failed validation: {result_path}")
    return result
...
def _run_one(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if existing:
        ...
        result = run_policy_horizon_candidate(
            policy_protocol_path=Path(str(payload["policy_protocol_path"])),
            failed_v47_audit_path=Path(str(payload["failed_v47_audit_path"])),
            expected_failed_v47_audit_sha256=str(
                payload["expected_failed_v47_audit_sha256"]
            ),
            ...
            seed=int(seed),
            rollout_kwargs={
                key: Path(str(value)) if key.endswith("_path") or key.endswith("_root") else value
...
def run_shard(
    *,
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("v48 shard protocol changed")
    ...
    if summary_path.exists():
        raise FileExistsError(f"refusing to overwrite v48 shard audit: {summary_path}")
    tasks = []
        ...
        tasks.append(
            {
                ...
                "policy_protocol_path": str(policy_protocol_path),
                "failed_v47_audit_path": str(failed_v47_audit_path),
                "expected_failed_v47_audit_sha256": expected_failed_v47_audit_sha256,
    ...
    passed = bool(rows) and observed_set == expected_set
    payload = {
        ...
        "elapsed_sec": float(time.monotonic() - started),
        "policy_protocol_sha256": _sha256(policy_protocol_path),
        "failed_v47_audit_sha256": _sha256(failed_v47_audit_path),
    ...
    if not passed:
        raise RuntimeError(f"v48 shard coverage failed: {shard_index}")
    return payload

#!/usr/bin/env python3
"""Fail-closed audit for the frozen v36/r32 cross-fitted selector."""
...
from cf_h2o.eval.traffic_signal_cross_fitted_pairwise import (
    ...
    summarize_cross_fitted_deployment,
...
def _validate_reference_audit(
    ...
    *,
    expected_sha256: str,
    expected_stage_passed: bool,
    expected_decision: str,
) -> dict[str, Any]:
    if _sha256(path) != expected_sha256:
        raise ValueError(f"reference audit SHA-256 changed: {path}")
    payload = _read_json(path)
    if (
        payload.get("integrity_passed") is not True
        or payload.get("stage_passed") is not expected_stage_passed
        or payload.get("decision") != expected_decision
    ):
        raise ValueError(f"reference audit status changed: {path}")
    return payload


def _validate_explicit_fit(
    ...
    *,
    ...
    expected_ids = tuple(sorted(str(value) for value in target_group_ids))
    if (
        ...
        != len(expected_ids)
        or tuple(diagnostics.get("target_adaptation_group_ids", ())) != expected_ids
        or set(diagnostics.get("source_domains", ())) != expected_domains
        or set(diagnostics.get("fit", {})) != set(DEPLOYMENT_FAMILIES)
    ):
        raise ValueError(f"{source}: explicit target fit contract changed")
    ...
    return {
        "target_group_count": len(expected_ids),
...
def audit(
    *,
    ...
    if (
        v34.get("frozen_cache", {}).get("aggregate_sha256")
        != cache_aggregate_sha256
        or v35.get("frozen_cache", {}).get("aggregate_sha256")
        != cache_aggregate_sha256
    ):
        raise ValueError("reference audits do not use the frozen cache")
    ...
    for target in TARGETS:
        ...
        expected_assignment = assign_seed_stratified_folds(records)
        ...
        if (
            ...
            raise ValueError(f"{shard_path}: cross-fit shard did not pass")
        ...
        if (
            result.get("protocol")
            != "tsc-v36r32-cross-fitted-pairwise-deployment-screen-v1"
            ...
            != expected_selected
            or not _same_json(result.get("fold_assignment"), expected_assignment)
        ):
            raise ValueError(f"{result_path}: cross-fit result contract changed")
        ...
        if not isinstance(fold_results, list) or len(fold_results) != 5:
            raise ValueError(f"{result_path}: cross-fit fold set incomplete")
        for fold_index, fold_row in enumerate(fold_results):
            validation_ids = tuple(
                sorted(
                    group
                    for group, assigned_fold in expected_assignment[
                        "assignments"
                    ].items()
                    if int(assigned_fold) == fold_index
                )
            )
            training_ids = tuple(
                group for group in expected_selected if group not in set(validation_ids)
            )
            if (
                int(fold_row.get("fold_index", -1)) != fold_index
                or tuple(fold_row.get("training_group_ids", ())) != training_ids
                or tuple(fold_row.get("validation_group_ids", ()))
                != validation_ids
            ):
                raise ValueError(f"{result_path}: fold {fold_index} identities changed")
            diagnostics = fold_row.get("model_diagnostics")
            if not isinstance(diagnostics, dict):
                raise ValueError(f"{result_path}: fold diagnostics missing")
            if (
                ...
                raise ValueError(f"{result_path}: fold city holdout changed")
        ...
        if not isinstance(oof_rows, list) or len(oof_rows) != 60:
            raise ValueError(f"{result_path}: OOF row count changed")
        oof_ids = tuple(sorted(str(row.get("group_id", "")) for row in oof_rows))
        if oof_ids != expected_selected:
            raise ValueError(f"{result_path}: OOF group identities changed")
        for row in oof_rows:
            group_id = str(row["group_id"])
            expected_record = record_by_id[group_id]
            family_regrets = row.get("family_regrets")
            if not isinstance(family_regrets, dict) or set(family_regrets) != set(
                DEPLOYMENT_FAMILIES
            ):
                raise ValueError(f"{result_path}: OOF family set changed")
            ...
            if (
                ...
                != int(expected_assignment["assignments"][group_id])
                or int(row.get("simulator_seed", -1))
                != int(expected_record["simulator_seed"])
                or str(row.get("tls_id", "")) != str(expected_record["tls_id"])
                or not math.isclose(
                    float(row.get("snapshot_time_sec", float("nan"))),
                    float(expected_record["snapshot_time_sec"]),
                    rel_tol=0.0,
                    abs_tol=1e-12,
                )
                or not math.isclose(
                ...
                raise ValueError(f"{result_path}: OOF row accounting changed")
        recomputed_decision = select_cross_fitted_pairwise(oof_rows)
        recorded_decision = result.get("deployment_decision")
        if not _same_json(recomputed_decision, recorded_decision):
            raise ValueError(f"{result_path}: OOF decision is not reproducible")
        ...
        if not isinstance(final_diagnostics, dict):
            raise ValueError(f"{result_path}: final model diagnostics missing")
        if (
        ...
        integrity_rows.append(
            {
                ...
                "oof_gate_passed": bool(recorded_decision["gate"]["passed"]),
                "mean_oof_paired_gain": float(
                    recorded_decision["mean_oof_paired_gain"]
                ),
                "oof_nonworse_fraction": float(
                    recorded_decision["oof_nonworse_fraction"]
                ),
                "worst_seed_gain": float(recorded_decision["worst_seed_gain"]),
                "worst_fold_gain": float(recorded_decision["worst_fold_gain"]),
    ...
    selection = summarize_cross_fitted_deployment(
        decisions=decisions,
        evaluation_regrets=evaluation_regrets,
    )
    return {
        "protocol": "tsc-v36r32-cross-fitted-pairwise-audit-v1",
...
def _markdown(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        "60-group provenance, five-fold OOF, explicit-fit, complete-city "
        "holdout, decision reproduction, and external evaluation audits.",
        "",
    ...
    for row in payload["integrity_rows"]:
        lines.append(
            f"| {row['target']} | {row['mean_oof_paired_gain']:.6f} | "
            f"{row['oof_nonworse_fraction']:.3f} | "
            f"{row['worst_seed_gain']:.6f} | "
            f"{row['worst_fold_gain']:.6f} | {row['selected_family']} | "
...
def _write_csv(path: Path, payload: Mapping[str, Any]) -> None:
    fields = (
        ...
        "selected_family",
        "oof_gate_passed",
        "mean_oof_paired_gain",
        "oof_nonworse_fraction",
        "worst_seed_gain",
        "worst_fold_gain",
        "fallback_regret",
        "pairwise_regret",

#!/usr/bin/env python3
"""Submit seven V150G complete-selector crossfit targets."""
...
LAUNCH_PROTOCOL = "tsc-v150g-crossfitted-source-selector-launch-v1"
CONFIG_RELATIVE = Path(
    "cf_h2o/config/traffic_signal_tsc_v150g_crossfitted_source_selector.json"
)
MODULE = "cf_h2o.eval.traffic_signal_crossfitted_source_selector"
SIGNATURE_PREFIX = "CFCMT/v150g/crossfitted-source-selector-v1"
RESOURCE_FAMILY = "CFCMT-v150g-crossfitted-source-selector"


def _validate_config(config: Mapping[str, Any]) -> None:
    gate = dict(config.get("development_gate", {}))
    if (
        config.get("protocol") != "tsc-v150g-crossfitted-source-selector-v1"
        or config.get("redeveloped_after") != V150E_AGGREGATE_PROTOCOL
        ...
        raise ValueError("V150G frozen configuration changed")
...
def build_specs(
    *,
    ...
    v150e_rejection_remote: Path,
    v150e_rejection_sha256: str,
    ...
    return _build_rigid_specs(
        ...
        extra_target_args=(
            "--v150e-rejection",
            str(v150e_rejection_remote),
            "--v150e-rejection-sha256",
            str(v150e_rejection_sha256),
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--v150e-rejection-local", type=Path, required=True)
    parser.add_argument("--v150e-rejection-remote", type=Path, required=True)
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite V150G launch or results")
    if not 1 <= int(args.cache_workers) <= 8:
        raise ValueError("V150G cache workers must be in [1, 8]")
    _validate_config(_read_json(PROJECT_ROOT / CONFIG_RELATIVE))

    authorization = _read_json(args.authorization_result_local)
    rejection = _read_json(args.v150e_rejection_local)
    ...
    if (
        ...
        raise ValueError("V150G requires passing V150C authorization")
    if (
        rejection.get("protocol") != V150E_AGGREGATE_PROTOCOL
        or rejection.get("development_gate", {}).get("passed") is not False
    ):
        raise ValueError("V150G requires rejected V150E aggregate")
    if fit.get("protocol") != FIT_RESULT_PROTOCOL or fit.get("city") != "jinan":
        raise ValueError("V150G requires the frozen V115 architecture fit")
    if (
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote V150G preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            f"remote V150G evidence identities changed: {remote_hashes}; {hash_stderr}"
        )
    specs = build_specs(
        snapshot_root=snapshot_root,
        authorization_result_remote=args.authorization_result_remote,
        authorization_sha256=expected_hashes[0],
        v150e_rejection_remote=args.v150e_rejection_remote,
        v150e_rejection_sha256=expected_hashes[1],
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v150g-crossfitted-source-selector-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V150G target matrix")
    return 0

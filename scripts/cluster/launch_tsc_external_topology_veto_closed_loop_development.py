#!/usr/bin/env python3
"""Launch the frozen v78 paired matrix on node001-node006."""
...
LAUNCH_PROTOCOL = "v78r74-topology-veto-development-six-node-launch-v2"
...
def _sync_new(
    *, local_path: Path, remote_path: Path, scheduler_path: Path, directory: bool
    ...
    code, stdout, stderr = scheduler.run_on(
        "node001",
        f"if test -e {shlex.quote(str(remote_path))}; then "
        "printf 'REUSE_EXISTING'; else "
        f"mkdir -p {shlex.quote(str(remote_path.parent))}; fi",
        timeout=120,
        check=False,
    )
    if int(code) != 0:
        raise RuntimeError(f"remote v78 input check failed: {remote_path}: {stderr}")
    if str(stdout).strip() == "REUSE_EXISTING":
        return
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"v78 input sync failed: {completed.stderr[-4000:]}")
...
def _input_visibility(
    *, checks: Sequence[tuple[Path, str]], scheduler_path: Path
    ...
    if not all(row["returncode"] == 0 for row in rows):
        raise RuntimeError(f"v78 inputs are not visible on every node: {rows}")
...
def _sync_results(
    *,
    ...
    if local_results_root.exists():
        raise FileExistsError(f"refusing to overwrite local v78 results: {local_results_root}")
    ...
    raise RuntimeError(f"v78 result sync failed: {rows[-1]}")
...
def build_specs(
    *,
    ...
    if (
        ...
        raise ValueError("v78 launch authorization changed")
    ...
    for shard_index, node in enumerate(NODES):
        ...
        specs.append(
            {
                "project": "CFCMT",
                "signature": f"CFCMT/v78r74/topology-veto-development/shard-{shard_index}",
                "description": f"CFCMT v78 topology-veto development shard {shard_index}",
                "cmd": command,
                ...
                "vram": 0,
                "priority": "high",
                "resource_family": "CFCMT-topology-veto-closed-loop-v1",
                "require_node": node,

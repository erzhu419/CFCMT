#!/usr/bin/env python3
"""Run one process-isolated shard of the frozen v81 validation matrix."""
...
from cf_h2o.eval.traffic_signal_external_target_regime_trust_validation import (
    ...
    run_target_regime_trust_validation,
)


SHARD_PROTOCOL = "tsc-v81r77-target-regime-validation-process-shard-v1"
...
def shard_identities(
    ...
    if shard_count < 1 or not 0 <= shard_index < shard_count:
        raise ValueError("invalid v81 shard index/count")
    return tuple(
        identity
...
def _validate_existing(
    *,
    ...
    if result_path.is_file() != tripinfo_path.is_file():
        raise RuntimeError(f"partial v81 result exists: {result_path.parent}")
    ...
    if not (
        ...
        and (result.get("city"), result.get("scenario"), int(result.get("seed", -1)), result.get("policy"))
        == (city, scenario, seed, policy)
        and result.get("protocol_sha256") == hashes["protocol_sha256"]
        and result.get("regime_trust_freeze_sha256")
        == hashes["regime_trust_freeze_sha256"]
        and result.get("regime_trust_artifact_sha256")
        == hashes["regime_trust_artifact_sha256"]
        and result.get("tripinfo_evidence", {}).get("sha256") == _sha256(tripinfo_path)
        and bool(result.get("metrics", {}).get("ok", False))
    ):
        raise RuntimeError(f"existing v81 result is invalid: {result_path}")
    return result
...
def _run_one(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not result:
        ...
        result = run_target_regime_trust_validation(
            protocol_path=Path(str(payload["protocol_path"])),
            proposal_parent_protocol_path=Path(str(payload["proposal_parent_protocol_path"])),
            hierarchical_freeze_audit_path=Path(str(payload["hierarchical_freeze_audit_path"])),
            hierarchical_freeze_root=Path(str(payload["hierarchical_freeze_root"])),
            veto_freeze_result_path=Path(str(payload["veto_freeze_result_path"])),
            veto_joint_audit_path=Path(str(payload["veto_joint_audit_path"])),
            veto_artifact_root=Path(str(payload["veto_artifact_root"])),
            regime_freeze_result_path=Path(str(payload["regime_freeze_result_path"])),
            regime_artifact_path=Path(str(payload["regime_artifact_path"])),
...
def run_shard(
    *,
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("v81 shard protocol changed")
    identities = shard_identities(protocol, shard_index=shard_index, shard_count=shard_count)
    ...
    if summary_path.exists():
        ...
        if (
            ...
            raise RuntimeError(f"completed v81 shard is invalid: {summary_path}")
        ...
        tasks.append(
            {
                ...
                "hierarchical_freeze_audit_path": str(hierarchical_freeze_audit_path),
                ...
                "regime_freeze_result_path": str(regime_freeze_result_path),
                "regime_artifact_path": str(regime_artifact_path),
    ...
    with context.Pool(processes=process_count, maxtasksperchild=1) as pool:
        for completed, row in enumerate(pool.imap_unordered(_run_one, tasks, chunksize=1), start=1):
            rows.append(row)
            print(
                f"CFCMT_PROGRESS {completed}/{len(tasks)} elapsed={time.monotonic() - started:.1f}s",
                flush=True,
            )
    order = {identity: index for index, identity in enumerate(identities)}
    rows.sort(key=lambda row: order[(row["city"], row["scenario"], int(row["seed"]), row["policy"])])
    observed = {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--regime-freeze-result", type=Path, required=True)
    ...
    payload = run_shard(
        ...
        regime_freeze_result_path=args.regime_freeze_result,
    ...
    print(json.dumps({"passed": payload["passed"], "node": payload["hostname"], "shard": payload["shard_index"], "tasks": payload["task_count"], "elapsed_sec": payload["elapsed_sec"]}, sort_keys=True))

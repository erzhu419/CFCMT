#!/usr/bin/env python3
"""Audit and select the frozen waiting-aligned latent development matrix."""
...
from scripts.cluster.launch_tsc_external_waiting_aligned_latent_closed_loop import (  # noqa: E402
    LAUNCH_PROTOCOL,
    NODES,
)
from scripts.cluster.run_tsc_external_waiting_aligned_latent_closed_loop_shard import (  # noqa: E402
    SHARD_PROTOCOL,
)


AUDIT_PROTOCOL = "tsc-v83r79-waiting-aligned-latent-closed-loop-audit-v1"
AUTHORIZATION_DECISION = "authorize_waiting_aligned_latent_successor_freeze"
REJECTION_DECISION = "retain_phase_pressure_and_reject_waiting_aligned_latent"
...
def _load_rollout(
    *,
    ...
    if not (
        ...
        raise ValueError(f"waiting-aligned rollout changed: {result_path}")
    return result
    ...
    *,
    ...
    for shard_index in range(len(NODES)):
        ...
        if not (
            ...
            and int(shard.get("workers", -1)) == 8
        ):
            raise ValueError(f"waiting-aligned shard changed: {path}")
        shard_rows.append(
            {
    ...
    for seed in seeds:
        ...
        for policy in METHOD_KEYS:
            ...
            contract_ok = contract_ok and (
                method.get("proposal_origin")
                == "target_spatiotemporal_action_latent"
    ...
    integrity = {
        "protocol_and_launch_chain_exact": True,
        "matrix_size_exact": len(results) == 88,
        "identity_set_exact": set(results) == set(identities),
        "six_shards_exact": len(shard_rows) == 6,
        "eight_workers_per_shard": all(row["workers"] == 8 for row in shard_rows),
...
def _report(payload: Mapping[str, Any]) -> str:
    lines = [
        "# Waiting-Aligned Latent Closed-Loop Development Audit",
        "",
        ...
        "",
        "| Policy | Cooldown | Mean relative delta | 95% CI | Improved seeds | Active seeds | Overrides | Feasible |",
        "|---|---:|---:|---:|---:|---:|---:|:---:|",
    ]
    for row in payload["candidate_summaries"]:
        ci = row["bootstrap"]["ci95"]
        lines.append(
            f"| {row['policy']} | {row['cooldown_intervals']} | "
            f"{row['mean_relative_delta']:.4%} | [{ci[0]:.4%}, {ci[1]:.4%}] | "
            f"{row['improved_seed_fraction']:.1%} | {row['active_seed_fraction']:.1%} | "
            f"{row['total_executed_overrides']} | {'yes' if row['feasible'] else 'no'} |"
        )
    lines.extend(
        [
            "",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.report.exists():
        raise FileExistsError("refusing to overwrite waiting-aligned latent audit")
        ...
        json.dumps(
            {
                ...
                "feasible": payload["feasible_candidate_count"],
            },
            sort_keys=True,
        )
    )

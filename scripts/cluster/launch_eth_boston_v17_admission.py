#!/usr/bin/env python3
"""Launch ordered Boston v17 no-TLS response-state admission stages."""
...
from scripts.cluster.launch_eth_boston_v15_admission import (
    ...
    build_full_spec as _build_v15_full_spec,
    build_route_spec as _build_v15_route_spec,
    build_trigger_spec as _build_v15_trigger_spec,
)
from scripts.cluster.launch_eth_boston_v16_admission import (
    FORMER_FAILURE_TIMES_SEC as V16_FAILURE_TIMES_SEC,
    validate_trigger_result as _validate_v16_trigger_result,
)
...
from scripts.data.repair_eth_boston_implicit_no_tls_major import (
    PACKAGE_PROTOCOL as V16_PACKAGE_PROTOCOL,
...
LAUNCH_PROTOCOL = "tsc-v150-eth-boston-v17-admission-pipeline-launch-v1"
SIGNATURE_ROOT = "CFCMT/v150/eth-boston-v17-no-tls-major-response"
FORMER_FAILURE_TIMES_SEC = (*V16_FAILURE_TIMES_SEC, 12_201.0)
...
def validate_trigger_package(manifest: Mapping[str, Any]) -> None:
    ...
    if (
        manifest.get("protocol") != V17_PACKAGE_PROTOCOL
        or manifest.get("passed") is not True
        or manifest.get("city_code") != "BOS"
        or gates.get("boston_no_tls_major_response_states_repaired") is not True
        ...
        raise ValueError("Boston v17 package does not cover the trigger window")


def validate_trigger_result(
    result: Mapping[str, Any], *, expected_package_manifest_sha256: str
) -> None:
    _validate_v16_trigger_result(
        result,
    ...
    if any(final_time <= value for value in FORMER_FAILURE_TIMES_SEC):
        raise ValueError("Boston v17 trigger did not cross every prior failure")
...
def build_package_spec(
    *,
    ...
    return {
        "description": "CFCMT Boston v17 no-TLS response-state package repair",
        "project": "CFCMT",
        "cmd": "mkdir -p "
        ...
        "resource_family": "CFCMT-v150-eth-boston-v17-package",
        "ram_resource_family": "CFCMT-v150-eth-boston-v17-package",
        "vram": 0,
        "ram_mb": 32_768,
        "cpu": 2,
        "priority": "high",
...
def _version_spec(spec: Mapping[str, Any], *, stage: str) -> dict[str, Any]:
    ...
    value.update(
        {
            "description": f"CFCMT Boston v17 {stage} admission",
            "signature": f"{SIGNATURE_ROOT}/{stage}-v1",
            "ram_resource_family": f"CFCMT-v150-eth-boston-{stage}-admission-v17",
        }
    )
...
def build_route_spec(**kwargs: Any) -> dict[str, Any]:
    return _version_spec(_build_v15_route_spec(**kwargs), stage="route")


def build_trigger_spec(**kwargs: Any) -> dict[str, Any]:
    return _version_spec(_build_v15_trigger_spec(**kwargs), stage="trigger")


def build_full_spec(**kwargs: Any) -> dict[str, Any]:
    return _version_spec(_build_v15_full_spec(**kwargs), stage="full")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError(
            f"refusing to overwrite Boston v17 {args.stage} evidence"
        )
    ...
    if args.stage == "package":
        ...
        if base_manifest.get("protocol") != V16_PACKAGE_PROTOCOL:
            raise ValueError("Boston v17 repair requires the admitted v16 package")
        output_package_root = (
            args.package_root.parent / "boston_complete_published_package_v17"
        )
        _verify_remote_inputs(
            scheduler=scheduler,
            paths_and_hashes=(
    ...
    completed = _submit(
        ...
        spec=spec,
        intent_label=f"CFCMT-v150-eth-boston-v17-{args.stage}",
    )
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"scheduler rejected Boston v17 {args.stage} admission")
    return 0

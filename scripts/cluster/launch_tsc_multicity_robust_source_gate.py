#!/usr/bin/env python3
"""Submit the seven complete-city V145 robust source-gate tasks."""
...
LAUNCH_PROTOCOL = "tsc-v145-multicity-robust-source-gate-launch-v1"
CONFIG_RELATIVE = Path(
    "cf_h2o/config/traffic_signal_tsc_v145_multicity_robust_source_gate.json"
)
MODULE = "cf_h2o.eval.traffic_signal_multicity_robust_source_gate"
SIGNATURE_PREFIX = "CFCMT/v145/multicity-robust-source-gate-v1"
RESOURCE_FAMILY = "CFCMT-v145-multicity-robust-source-gate"
...
def _validate_config(config: Mapping[str, Any]) -> None:
    ...
    if (
        config.get("protocol") != "tsc-v145-multicity-robust-source-gate-v1"
        ...
        or float(rule.get("maximum_adaptation_intervention_fraction", -1.0))
        != 0.9
        or int(gate.get("minimum_improving_cities", -1)) != 5
        or float(gate.get("maximum_city_regression", -1.0)) != 0.01
    ):
        raise ValueError("V145 frozen protocol configuration changed")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite V145 launch or local results")
    if not 1 <= int(args.cache_workers) <= 8 or not 1 <= int(args.fit_workers) <= 8:
        raise ValueError("V145 workers must be in [1, 8]")
    ...
    if fit.get("protocol") != FIT_RESULT_PROTOCOL or fit.get("city") != "jinan":
        raise ValueError("V145 requires the frozen V115 architecture fit")
    if (
        ...
        raise ValueError("V145 source cache audit changed")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote V145 preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote V145 evidence identities changed: "
    ...
    specs = build_specs(
        ...
        module=MODULE,
        description_prefix="CFCMT V145 robust source-gate target",
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v145-multicity-robust-source-gate-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V145 robust source gate")
    return 0

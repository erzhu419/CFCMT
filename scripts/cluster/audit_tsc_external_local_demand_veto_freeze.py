#!/usr/bin/env python3
"""Audit v74 artifacts, city activation, and runtime scoring contracts."""
...
from cf_h2o.traffic_signal.local_demand_target_veto import (  # noqa: E402
    ARTIFACT_PROTOCOL,
    VETO_PROTOCOL,
...
AUDIT_PROTOCOL = "tsc-v74r70-local-demand-veto-joint-freeze-audit-v1"
AUDIT_DECISION = "authorize_local_demand_veto_closed_loop_development_only"


def _runtime_probe(veto: LocalDemandTargetVeto) -> dict[str, Any]:
    tls_id = sorted(veto.topology_context.tls_features)[0]
    time_sec = float(veto.local_demand_context.begin_sec + 600.0)
    ...
    return {
        "passed": (
            ...
            and float(result["runtime_time_sec"]) == time_sec
            and (
    ...
    *, protocol_path: Path, result_path: Path, artifact_root: Path
    ...
    for city, expected_scenarios in sorted(cache_protocol["city_scenarios"].items()):
        ...
        gates = {
            ...
            "veto_types": all(
                isinstance(veto, LocalDemandTargetVeto) for veto in vetoes.values()
            ),
    ...
    validation_paths = protocol["future_file_counts_at_freeze"]
    gates = {
        ...
        "sealed_absent_at_freeze": all(
            int(value) == 0 for value in validation_paths.values()
        ),
    }
    gates["passed"] = all(gates.values())
    return {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError(f"refusing to overwrite v74 audit: {args.out}")
    payload = audit(

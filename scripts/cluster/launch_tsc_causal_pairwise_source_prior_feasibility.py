#!/usr/bin/env python3
"""Submit the frozen V127 causal pairwise source-prior feasibility test."""
...
LAUNCH_PROTOCOL = "tsc-v127-causal-pairwise-source-prior-feasibility-launch-v3"
    ...
    *,
    ...
    command = shlex.join(
        [
            ...
            str(wrapper),
            "-m",
            "cf_h2o.eval.traffic_signal_causal_pairwise_source_prior_feasibility",
            "--prediction-artifact",
            str(prediction_artifact_remote),
            "--prediction-artifact-sha256",
            str(prediction_artifact_sha256),
            "--selector-cache-root",
    ...
    return {
        "description": (
            "CFCMT V127 causal pairwise source-prior feasibility JSON-safe rerun"
        ),
        "project": "CFCMT",
        "cmd": command,
        "cwd": str(snapshot_root),
        "signature": SIGNATURE,
        "resource_family": "CFCMT-v127-causal-pairwise-source-prior",
        "vram": 0,
        "ram_mb": RAM_MB,
        "cpu": 20,
        "priority": "high",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite V127 launch artifact")
    if not 1 <= int(args.cache_workers) <= 20:
        raise ValueError("V127 cache workers must be in [1, 20]")
    if not 1 <= int(args.fold_workers) <= 20:
        raise ValueError("V127 fold workers must be in [1, 20]")
    if len(str(args.prediction_artifact_sha256)) != 64:
        raise ValueError("V127 prediction artifact SHA-256 is malformed")
    nodes = tuple(value.strip() for value in args.nodes.split(",") if value.strip())
    if not nodes or len(set(nodes)) != len(nodes):
        raise ValueError("V127 execution nodes must be nonempty and unique")
    if not set(nodes).issubset(ALLOWED_NODES):
        raise ValueError("V127 execution nodes include a non-admitted CPU node")
    ...
    if (
        ...
        raise ValueError("V127 selector cache audit changed")
    ...
    for node in nodes:
        code, _, stderr = scheduler.run_on(
            node, preflight, timeout=120, check=False
        )
        if int(code) != 0:
            raise RuntimeError(f"remote V127 preflight failed on {node}: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote V127 evidence identities changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v127-causal-pairwise-source-prior-feasibility-v3-flexible-nodes",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V127 pairwise source prior")
    return 0

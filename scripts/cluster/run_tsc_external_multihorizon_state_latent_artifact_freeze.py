#!/usr/bin/env python3
"""Fit, independently audit, and retrieve the v79 state-latent artifacts."""
...
LAUNCH_PROTOCOL = "v83r79-multihorizon-state-latent-artifact-launch-v1"
...
def _rsync_to_remote(
    *, scheduler: Any, node: str, source: Path, destination: Path
) -> None:
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"evidence staging failed: {completed.stderr[-4000:]}")
...
def _rsync_from_remote(
    *, scheduler: Any, node: str, source: Path, destination: Path, tree: bool
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"artifact retrieval failed: {completed.stderr[-4000:]}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--nested-result-local", type=Path, required=True)
    parser.add_argument("--nested-oof-local", type=Path, required=True)
    ...
    if any(path.exists() for path in local_outputs):
        raise FileExistsError("refusing to overwrite local v79 artifact evidence")
    if str(args.node) not in {f"node{index:03d}" for index in range(1, 7)}:
        raise ValueError("artifact node is outside node001-node006")
    stage = _read_json(args.stage_manifest)
    snapshot = Path(stage["snapshot_root"])
    scheduler = _load_scheduler(args.scheduler)
    remote_outputs = (
        args.remote_evidence_root,
        args.remote_artifact_root,
        args.remote_freeze_result,
        args.remote_audit_result,
    )
    preflight = shlex.join(
        ["test", "!", "-e", str(remote_outputs[0])]
    )
    for path in remote_outputs[1:]:
        preflight += " && " + shlex.join(["test", "!", "-e", str(path)])
    code, _, stderr = scheduler.run_on(
        str(args.node), preflight, timeout=120, check=False
    )
    if int(code) != 0:
        raise FileExistsError(f"refusing to overwrite remote v79 evidence: {stderr}")
    code, _, stderr = scheduler.run_on(
        str(args.node),
        shlex.join(["mkdir", "-p", str(args.remote_evidence_root)]),
        timeout=120,
        check=False,
    )
    if int(code) != 0:
        raise RuntimeError(f"remote evidence directory creation failed: {stderr}")
    nested_result_remote = args.remote_evidence_root / "nested_result_v1.json"
    nested_oof_remote = args.remote_evidence_root / "deployment_oof_v1.npz"
    _rsync_to_remote(
        scheduler=scheduler,
        node=str(args.node),
        source=args.nested_result_local,
        destination=nested_result_remote,
    )
    _rsync_to_remote(
        scheduler=scheduler,
        node=str(args.node),
        source=args.nested_oof_local,
        destination=nested_oof_remote,
    )
    ...
    freeze_command = shlex.join(
        [
            str(wrapper),
            "-m",
            "cf_h2o.eval.traffic_signal_multihorizon_state_latent_artifact_freeze",
            *common,
            "--nested-protocol",
            str(snapshot / NESTED_PROTOCOL_RELATIVE),
            "--nested-result",
            str(nested_result_remote),
            "--nested-deployment-oof",
            str(nested_oof_remote),
            "--screen-protocol",
    ...
    returncode, stdout, run_stderr, durable = run_durable_remote_capture(
        ...
        run_id="cfcmt-v79-multihorizon-state-latent-artifacts-v1",
    ...
    payload = {
        ...
        "nested_result_local_sha256": _sha256(args.nested_result_local),
        "nested_oof_local_sha256": _sha256(args.nested_oof_local),
        "node": str(args.node),
        "workers": max(1, int(args.workers)),
        "remote_evidence_root": str(args.remote_evidence_root),
    ...
    if not passed:
        raise RuntimeError(
            "multihorizon artifact freeze/audit failed: "

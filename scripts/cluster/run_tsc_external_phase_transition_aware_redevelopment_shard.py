#!/usr/bin/env python3
"""Run one process-isolated shard of the frozen v87 development matrix."""
...
from cf_h2o.eval.traffic_signal_phase_transition_aware_redevelopment import (
    ...
    run_phase_transition_aware_redevelopment,
)
...
SHARD_PROTOCOL = "tsc-v87r83-phase-transition-aware-process-shard-v1"
...
def shard_identities(
    ...
    if shard_count < 1 or not 0 <= shard_index < shard_count:
        raise ValueError("invalid v87 shard index/count")
    return tuple(
        identity
...
def _validate_existing(
    *,
    ...
    if result_path.is_file() != tripinfo_path.is_file():
        raise RuntimeError(f"partial v87 result exists: {result_path.parent}")
    ...
    if not (
        ...
        raise RuntimeError(f"existing v87 result is invalid: {result_path}")
    return result
...
def _run_one(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not result:
        ...
        result = run_phase_transition_aware_redevelopment(
            ...
            diagnostic_audit_path=Path(str(payload["diagnostic_audit_path"])),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("v87 shard protocol changed")
    ...
    common = {
        ...
        "diagnostic_audit_path": str(args.diagnostic_audit),
    ...
    if summary_path.exists():
        ...
        if existing_summary.get("passed") is not True:
            raise RuntimeError(f"invalid existing v87 shard summary: {summary_path}")
    else:
        summary_path.parent.mkdir(parents=True, exist_ok=True)
        _atomic_json(summary_path, summary)
    if not passed:
        raise RuntimeError(f"v87 shard coverage failed: {args.shard_index}")
    print(
        json.dumps(
            {
                "status": "PASS",

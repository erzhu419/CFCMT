#!/usr/bin/env python3
"""Submit six node-local process shards for frozen v52 development."""
...
LAUNCH_PROTOCOL = "v52r48-global-cooldown-development-six-node-submission-v1"
...
def build_specs(
    *,
    snapshot_root: Path,
    failed_v51_audit_local: Path,
    failed_v51_audit_remote: Path,
    ...
    global_path = PROJECT_ROOT / GLOBAL_PROTOCOL_RELATIVE
    v51_path = PROJECT_ROOT / V51_PROTOCOL_RELATIVE
    adaptation_path = PROJECT_ROOT / ADAPTATION_PROTOCOL_RELATIVE
    pressure_path = PROJECT_ROOT / PRESSURE_PROTOCOL_RELATIVE
    protocol = _read_json(global_path)
    v51 = _read_json(v51_path)
    adaptation = _read_json(adaptation_path)
    failed = _read_json(failed_v51_audit_local)
    hashes = {
        "global_protocol": _sha256(global_path),
        "v51_protocol": _sha256(v51_path),
        "adaptation_protocol": _sha256(adaptation_path),
        "pressure_protocol": _sha256(pressure_path),
        "failed_v51_audit": _sha256(failed_v51_audit_local),
    ...
    city_scope = dict(protocol["city_deployment_scope"])
    authorized = (
        protocol.get("protocol") == PROTOCOL
        and failed.get("protocol") == FAILED_V51_AUDIT_PROTOCOL
        and failed.get("status") == "PASS"
        and failed.get("decision") == FAILED_V51_DECISION
        and hashes["failed_v51_audit"]
        == str(protocol["failed_v51_selection_audit"]["sha256"])
        and hashes["v51_protocol"]
        == str(protocol["parent_execution_trust_region_protocol"]["sha256"])
        == str(failed["trust_protocol_sha256"])
        ...
        and len(identities) == int(protocol["development_grid"]["matrix_size"])
        and len(identities) == 240
        and len(candidates) == 24
        and all(
            candidate.execution_trust_region.global_cooldown_intervals > 0
        ...
        and protocol["development_grid"]["coordination_mode"] == "sparse"
    ...
    if not authorized:
        raise ValueError("v52 global-cooldown launch authorization changed")

    wrapper = snapshot_root / "scripts/cluster/run_cfcmt_sumo122.sh"
    runner = (
        snapshot_root
        / "scripts/cluster/run_tsc_external_estimand_aligned_global_cooldown_development_shard.py"
    )
    global_protocol = snapshot_root / GLOBAL_PROTOCOL_RELATIVE
    v51_protocol = snapshot_root / V51_PROTOCOL_RELATIVE
    ...
    for shard_index, node in enumerate(NODES):
        ...
        if local_shard.exists():
            raise FileExistsError(f"refusing to overwrite v52 shard: {local_shard}")
        command = shlex.join(
            [
                str(wrapper),
                str(runner),
                ...
                "--failed-v51-audit",
                str(failed_v51_audit_remote),
                "--expected-failed-v51-audit-sha256",
        ...
        specs.append(
            {
                "description": f"CFCMT v52 global cooldown shard {shard_index}",
                "project": "CFCMT",
                "cmd": command,
                "cwd": str(snapshot_root),
                "signature": f"CFCMT/v52r48/global-cooldown/shard-{shard_index}",
                "resource_family": "CFCMT-v52-global-cooldown-process-shard-v1",
                "vram": 0,
                "ram_mb": int(ram_mb),
                "cpu": int(cpu_cores),
                "priority": "high",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--failed-v51-audit-local", type=Path, required=True)
    parser.add_argument("--failed-v51-audit-remote", type=Path, required=True)
    ...
    specs, hashes = build_specs(
        ...
        failed_v51_audit_local=args.failed_v51_audit_local,
        failed_v51_audit_remote=args.failed_v51_audit_remote,
    ...
    if args.submit:
        completed = subprocess.run(
            [
                ...
                "--json",
                "--intent-label",
                "CFCMT-v52r48-global-cooldown-development",
            ],
        ...
        if completed.returncode != 0:
            ...
            raise RuntimeError("v52 global-cooldown submission failed")
    _atomic_json(args.out, payload)
    print(
        json.dumps(
            {

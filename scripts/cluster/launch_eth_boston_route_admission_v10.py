#!/usr/bin/env python3
"""Submit complete-demand route admission for the Boston v12 package."""
...
LAUNCH_PROTOCOL = "tsc-v118r-eth-boston-route-admission-launch-v10"
SIGNATURE = (
    "CFCMT/v118r/eth-boston-route-admission-v10-"
...
def build_spec(
    *,
    ...
    spec.update(
        {
            "description": "CFCMT V118r ETH Boston v12 complete-demand route admission",
            "signature": SIGNATURE,
            "ram_resource_family": "CFCMT-v118-eth-boston-route-admission-v12",
        }
    )
    return spec
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite Boston v12 route admission")
    ...
    validate_joined_tls_uncontrolled_merge_manifest(package_manifest)
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote Boston v12 route preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != [args.expected_package_manifest_sha256]:
        raise RuntimeError(
            "remote Boston v12 package identity changed: "
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected Boston v12 route admission")
    return 0

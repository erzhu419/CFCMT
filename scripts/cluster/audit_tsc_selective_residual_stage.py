#!/usr/bin/env python3
"""Fail-closed audit for the frozen v27/r23 selective-residual screen."""
...
def _validate_reference(
    ...
    *,
    ...
    if _sha256(path) != expected_sha256:
        raise ValueError(f"{path}: v26 reference audit SHA-256 mismatch")
    payload = _read_json(path)
    if (
        payload.get("protocol")
        != "tsc-v26r22-rigid-anchored-residual-stage-audit-v1"
        or payload.get("integrity_passed") is not True
    ):
        raise ValueError(f"{path}: v26 reference audit is not integrity-passed")
    if payload.get("frozen_cache", {}).get("aggregate_sha256") != cache_aggregate_sha256:
        raise ValueError(f"{path}: reference cache SHA-256 mismatch")
    if payload.get("source_rule_baseline", {}).get("sha256") != source_rule_baseline_sha256:
        raise ValueError(f"{path}: reference source-rule SHA-256 mismatch")
    if tuple(payload.get("targets", ())) != TARGETS:
        raise ValueError(f"{path}: reference target order changed")
    rigid = payload.get("selection", {}).get("rigid", {})
    targets = rigid.get("targets") if isinstance(rigid, dict) else None
    if not isinstance(targets, dict) or set(targets) != set(TARGETS):
        raise ValueError(f"{path}: reference rigid target matrix is incomplete")
    group_counts = payload.get("group_counts")
    if not isinstance(group_counts, dict) or set(group_counts) != set(TARGETS):
        raise ValueError(f"{path}: reference group-count matrix is incomplete")
    return (
        {target: float(targets[target]) for target in TARGETS},
        {target: int(group_counts[target]) for target in TARGETS},
        {
            "path": str(path),
            "sha256": expected_sha256,
            "protocol": payload["protocol"],
            "integrity_passed": True,
            "snapshot_sha256": payload.get("snapshot", {}).get("snapshot_sha256"),
        },
    )
...
def _validate_gate_diagnostics(
    ...
    *,
    ...
    if not all(isinstance(value, dict) for value in (config, source_fit, target_fit)):
        raise ValueError(f"{source}: incomplete selective residual diagnostics")

    candidate = CANDIDATE_BY_FAMILY[family]
    if config.get("source_stack_protocol") != (
        RIGID_ANCHORED_MECHANISM_RESIDUAL_STACK_PROTOCOL
    ):
        raise ValueError(f"{source}: rigid-residual stack protocol mismatch")
    if config.get("source_expert_gate_protocol") != (
        SOURCE_OOF_RIDGE_LCB_EXPERT_GATE_PROTOCOL
    ):
        raise ValueError(f"{source}: source expert-gate protocol mismatch")
    if not math.isclose(
        float(config.get("source_expert_gate_error_quantile", -1.0)),
        candidate.error_quantile,
        rel_tol=0.0,
        abs_tol=1e-15,
    ):
        raise ValueError(f"{source}: frozen error quantile mismatch")
    if not math.isclose(
        float(config.get("source_expert_gate_ridge_alpha", -1.0)),
        10.0,
        rel_tol=0.0,
        abs_tol=1e-15,
    ):
        raise ValueError(f"{source}: frozen gate Ridge alpha mismatch")
    if int(config.get("source_expert_gate_min_disagreement_groups", -1)) != 24:
        raise ValueError(f"{source}: frozen disagreement minimum mismatch")
    ...
    if source_fit.get("source_expert_gate_protocol") != (
        SOURCE_OOF_RIDGE_LCB_EXPERT_GATE_PROTOCOL
    ):
        raise ValueError(f"{source}: source fit gate protocol mismatch")
    ...
    if source_fit.get("stack_enabled") is not True:
        raise ValueError(f"{source}: residual stack was not source-selected")

    gate = source_fit.get("source_expert_gate")
    if not isinstance(gate, dict) or gate.get("enabled") is not True:
        raise ValueError(f"{source}: requested source expert gate was not enabled")
    if gate.get("protocol") != SOURCE_OOF_RIDGE_LCB_EXPERT_GATE_PROTOCOL:
        raise ValueError(f"{source}: gate diagnostic protocol mismatch")
    if int(gate.get("source_domain_count", -1)) != 5:
        raise ValueError(f"{source}: gate source-domain count mismatch")
    if int(gate.get("source_city_oof_fold_count", -1)) != 5:
        raise ValueError(f"{source}: gate source-city OOF count mismatch")
    if int(gate.get("disagreement_group_count", -1)) < 24:
        raise ValueError(f"{source}: insufficient gate disagreement groups")
    if tuple(gate.get("feature_names", ())) != SOURCE_EXPERT_GATE_FEATURE_NAMES:
        raise ValueError(f"{source}: gate feature contract mismatch")
    if gate.get("target_labels_used") is not False:
        raise ValueError(f"{source}: target labels entered source gate")
    if not math.isclose(
        float(gate.get("error_quantile", -1.0)),
        candidate.error_quantile,
        rel_tol=0.0,
        abs_tol=1e-15,
    ):
        raise ValueError(f"{source}: gate diagnostic error quantile mismatch")
    if not math.isclose(
        float(gate.get("ridge_alpha", -1.0)),
        10.0,
        rel_tol=0.0,
        abs_tol=1e-15,
    ):
        raise ValueError(f"{source}: gate diagnostic Ridge alpha mismatch")
    ...
    return {
        ...
        "ridge_alpha": 10.0,
        "disagreement_group_count": int(gate["disagreement_group_count"]),
        "selected_residual_fraction_of_all_groups": float(
            gate["selected_residual_fraction_of_all_groups"]
        ),
        "one_sided_error_margin": float(gate["one_sided_error_margin"]),
        "source_city_oof_one_sided_coverage": float(
            gate["source_city_oof_one_sided_coverage"]
        ),
    }
...
def audit(
    *,
    ...
    regrets = {RIGID_FAMILY: rigid_regrets}
    regrets.update({family: {} for family in EVALUATION_FAMILIES})
    ...
    for target in TARGETS:
        ...
        if not isinstance(children, list) or len(children) != len(EVALUATION_FAMILIES):
        ...
        for family in EVALUATION_FAMILIES:
            ...
            gate_fit = _validate_gate_diagnostics(row, family=family, source=result_path)
    ...
    selection = select_selective_residual_family(regrets)
    return {
        "protocol": "tsc-v27r23-source-only-selective-residual-stage-audit-v1",
...
def _markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "complete-city holdout, zero-target-label, source-only arbiter, "
    ...
    for row in selection["family_summaries"]:
        lines.append(
            f"| {row['label']} | {row['error_quantile']:.2f} | "
    ...
    lines.extend(["", "Salt Lake data were not read and cannot alter this decision.", ""])
    return "\n".join(lines)

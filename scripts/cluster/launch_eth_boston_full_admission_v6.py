#!/usr/bin/env python3
"""Submit Boston full microscopic admission after the v8 trigger passes."""
...
LAUNCH_PROTOCOL = "tsc-v118r-eth-boston-full-admission-launch-v6"
SIGNATURE = "CFCMT/v118r/eth-boston-full-admission-v6-uncontrolled-merge-yield"
...
def validate_trigger_result(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("Boston v8 trigger-window admission did not pass")
    if setting.get("package_manifest_sha256") != expected_package_manifest_sha256:
        raise ValueError("Boston v8 trigger-window package identity changed")
    if (
        ...
        raise ValueError("Boston v8 trigger-window execution contract changed")
    if any(
        float(observed["final_time_sec"]) <= failure_time
        for failure_time in FORMER_FAILURE_TIMES_SEC
    ):
        raise ValueError("Boston v8 trigger did not cross all prior failures")
...
def build_spec(
    *,
    ...
    spec.update(
        {
            "description": "CFCMT V118r ETH Boston v8 complete-demand libsumo full admission",
            "signature": SIGNATURE,
            "ram_resource_family": "CFCMT-v118-eth-boston-full-admission-v8-complete",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite Boston v8 full launch artifact")
    if _sha256(args.package_manifest_local) != args.expected_package_manifest_sha256:
        raise ValueError("local Boston v8 package manifest identity changed")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote Boston v8 full preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote Boston v8 full identities changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v118r-eth-boston-full-admission-v6",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected Boston v8 full admission")
    return 0

#!/usr/bin/env python3
"""Run one process-isolated shard of the frozen v89 confirmation matrix."""
...
SHARD_PROTOCOL = "tsc-v89r85-fresh-confirmation-process-shard-v1"
...
def shard_identities(
    ...
    if shard_count < 1 or not 0 <= shard_index < shard_count:
        raise ValueError("invalid v89 shard index/count")
    return tuple(
        identity
...
def _validate_existing(
    *,
    ...
    if result_path.is_file() != tripinfo_path.is_file():
        raise RuntimeError(f"partial v89 result exists: {result_path.parent}")
    ...
    phase_evidence_valid = bool(
        policy == PHASE_POLICY and alignment is None and not guard
    )
    if not (
        ...
        and (method_evidence_valid or phase_evidence_valid)
    ):
        raise RuntimeError(f"existing v89 result is invalid: {result_path}")
    return result
...
def _run_one(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not result:
        ...
        result = run_fresh_confirmation(
            ...
            v88_protocol_path=Path(str(payload["v88_protocol_path"])),
            v88_audit_path=Path(str(payload["v88_audit_path"])),
    ...
    return {
        ...
        "switch_overrides": int(guard.get("executed_switch_overrides", 0)),
        "stay_overrides": int(guard.get("executed_stay_overrides", 0)),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("v89 shard protocol changed")
    ...
    common = {
        ...
        "v88_protocol_path": str(args.v88_protocol),
        "v88_audit_path": str(args.v88_audit),
    ...
    if summary_path.exists():
        ...
        if existing_summary.get("passed") is not True:
            raise RuntimeError(f"invalid existing v89 shard summary: {summary_path}")
    else:
        summary_path.parent.mkdir(parents=True, exist_ok=True)
        _atomic_json(summary_path, summary)
    if not passed:
        raise RuntimeError(f"v89 shard coverage failed: {args.shard_index}")
    print(
        json.dumps(
            {
                "status": "PASS",

#!/usr/bin/env python3
"""Submit the frozen V134 local-mechanism surrogate oracle screen."""
...
LAUNCH_PROTOCOL = "tsc-v134-local-mechanism-surrogate-oracle-launch-v1"
SIGNATURE = "CFCMT/v134/local-mechanism-surrogate-oracle-v1"
    ...
    *,
    ...
    command = shlex.join(
        [
            ...
            str(wrapper),
            "-m",
            "cf_h2o.eval.traffic_signal_local_mechanism_surrogate_oracle",
            "--selector-cache-root",
    ...
    return {
        "description": "CFCMT V134 local mechanism surrogate oracle",
        "project": "CFCMT",
        "cmd": command,
        "cwd": str(snapshot_root),
        "signature": SIGNATURE,
        "resource_family": "CFCMT-v134-local-mechanism-surrogate-oracle",
        "vram": 0,
        "ram_mb": RAM_MB,
        "cpu": 20,
        "priority": "high",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite V134 launch artifact")
    if not 1 <= int(args.cache_workers) <= 20:
        raise ValueError("V134 cache workers must be in [1, 20]")
    nodes = tuple(value.strip() for value in args.nodes.split(",") if value.strip())
    if not nodes or len(set(nodes)) != len(nodes):
        raise ValueError("V134 execution nodes must be nonempty and unique")
    if not set(nodes).issubset(ALLOWED_NODES):
        raise ValueError("V134 execution nodes include a non-admitted CPU node")
    ...
    if (
        ...
        raise ValueError("V134 selector cache audit changed")
    ...
    for node in nodes:
        ...
        if int(code) != 0:
            raise RuntimeError(f"remote V134 preflight failed on {node}: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != [selector_sha]:
        raise RuntimeError(
            "remote V134 selector evidence changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v134-local-mechanism-surrogate-oracle-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V134 surrogate oracle screen")
    return 0

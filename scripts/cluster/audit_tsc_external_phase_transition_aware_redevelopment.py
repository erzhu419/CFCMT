#!/usr/bin/env python3
"""Audit v87 against frozen v86 phase/global references on the server."""
...
from cf_h2o.eval.traffic_signal_phase_transition_aware_redevelopment import (  # noqa: E402
    ALIGNMENT_PROTOCOL,
    RESULT_PROTOCOL,
...
from scripts.cluster.launch_tsc_external_phase_transition_aware_redevelopment import (  # noqa: E402
    LAUNCH_PROTOCOL,
)
from scripts.cluster.run_tsc_external_phase_transition_aware_redevelopment_shard import (  # noqa: E402
    SHARD_PROTOCOL,
)


AUDIT_PROTOCOL = "tsc-v87r83-phase-transition-aware-redevelopment-audit-v1"
...
def audit(
    *,
    ...
    integrity = {
        ...
        "diagnostic_parent_passed": diagnostic.get("status") == "PASS",
    ...
    if not integrity["passed"]:
        raise ValueError(f"v87 integrity gate failed: {integrity}")
    ...
    return {
        ...
        "candidate_summaries": [
            ...
            summaries_by_policy[ACTION_AWARE_POLICY],
        ],
        "selection_gate": {
        ...
        "scientific_status": {
            ...
            "aggregate_safety_estimand_applied_prospectively_to_v87_selection": True,
...
def _markdown(payload: Mapping[str, Any]) -> str:
    lines = [
        "# V87 phase-transition-aware redevelopment audit",
        "",
    ...
    lines.extend(
        [
            "",
            "V87 is adaptive evidence only. V86 remains rejected under its original "
            "seed-wise gate, and v85 remains sealed.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.markdown_out.exists():
        raise FileExistsError("refusing to overwrite v87 audit")
    payload = audit(

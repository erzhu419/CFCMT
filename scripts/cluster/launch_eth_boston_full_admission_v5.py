#!/usr/bin/env python3
"""Submit Boston full microscopic admission after the v7 trigger passes."""
...
from scripts.cluster.launch_eth_boston_full_admission_v4 import (  # noqa: E402
    CPU_CORES,
    RAM_MB,
    build_spec as _build_v4_spec,
)
from scripts.cluster.launch_eth_boston_trigger_admission_v5 import (  # noqa: E402
    ALLOWED_NODES,
...
LAUNCH_PROTOCOL = "tsc-v118r-eth-boston-full-admission-launch-v5"
SIGNATURE = "CFCMT/v118r/eth-boston-full-admission-v5-merge-tls-yield"
...
def validate_trigger_result(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("Boston v7 trigger-window admission did not pass")
    if setting.get("package_manifest_sha256") != expected_package_manifest_sha256:
        raise ValueError("Boston v7 trigger-window package identity changed")
    if (
        ...
        or float(observed.get("final_time_sec", -1.0))
        != TRIGGER_STOP_TIME_SEC
    ):
        raise ValueError("Boston v7 trigger-window execution contract changed")
    if any(
        float(observed["final_time_sec"]) <= failure_time
        for failure_time in FORMER_FAILURE_TIMES_SEC
    ):
        raise ValueError("Boston v7 trigger did not cross both prior failures")
...
def build_spec(
    *,
    ...
    spec.update(
        {
            "description": (
                "CFCMT V118r ETH Boston v7 complete-demand libsumo full admission"
            ),
            "signature": SIGNATURE,
            "ram_resource_family": (
                "CFCMT-v118-eth-boston-full-admission-v7-complete"
            ),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote Boston full-admission identities changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v118r-eth-boston-full-admission-v5",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected Boston v7 full admission")
    return 0

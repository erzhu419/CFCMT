#!/usr/bin/env python3
"""Run and retrieve the v76 multihorizon cache audit from node001."""
...
LAUNCH_PROTOCOL = "v83r79-multihorizon-cache-audit-launch-v1"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.local_out.exists() or args.launch_out.exists():
        raise FileExistsError("refusing to overwrite local multihorizon cache audit")
    ...
    command = shlex.join(
        [
            ...
            "-m",
            "cf_h2o.eval.traffic_signal_multihorizon_counterfactual_cache_audit",
            "--protocol",
    ...
    if int(returncode) != 0 or sync_error is not None or audit is None:
        raise RuntimeError(
            "multihorizon cache audit failed: "

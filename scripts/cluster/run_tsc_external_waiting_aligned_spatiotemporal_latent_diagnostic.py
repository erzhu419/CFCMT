#!/usr/bin/env python3
"""Run and retrieve the waiting-aligned spatiotemporal latent diagnostic."""
...
LAUNCH_PROTOCOL = "v83r79-waiting-aligned-spatiotemporal-latent-launch-v1"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--ranker-result-local", type=Path, required=True)
    parser.add_argument("--ranker-result-remote", type=Path, required=True)
    ...
    if args.local_out.exists() or args.launch_out.exists():
        raise FileExistsError("refusing to overwrite local latent diagnostic")
    stage = _read_json(args.stage_manifest)
    snapshot = Path(stage["snapshot_root"])
    scheduler = _load_scheduler(args.scheduler)
    if str(args.node) not in {f"node{index:03d}" for index in range(1, 7)}:
        raise ValueError("latent diagnostic node is outside node001-node006")
    ...
    if int(code) != 0:
        raise FileExistsError(f"refusing to overwrite remote latent result: {stderr}")
    command = shlex.join(
        [
            str(snapshot / "scripts/cluster/run_cfcmt_sumo122.sh"),
            "-m",
            "cf_h2o.eval.traffic_signal_waiting_aligned_spatiotemporal_latent_diagnostic",
            "--diagnostic-protocol",
            ...
            str(args.cache_root),
            "--ranker-protocol",
            str(snapshot / RANKER_PROTOCOL_RELATIVE),
            "--ranker-result",
            str(args.ranker_result_remote),
            "--partition",
    ...
    payload: dict[str, Any] = {
        ...
        "ranker_result_local_sha256": _sha256(args.ranker_result_local),
    ...
    if int(returncode) != 0 or sync_error is not None or result is None:
        raise RuntimeError(
            "waiting latent diagnostic failed: "

#!/usr/bin/env python3
"""Run one process-parallel shard of the frozen v51 development matrix."""
...
from cf_h2o.eval.traffic_signal_external_execution_trust_region_development import (
    ...
    run_execution_trust_region_candidate,
)


SHARD_PROTOCOL = "tsc-v51r47-execution-trust-region-development-process-shard-v1"
...
def shard_identities(
    ...
    *,
    ...
    if shard_count < 1 or not 0 <= shard_index < shard_count:
        raise ValueError("invalid v51 shard index/count")
    return tuple(
        identity
...
def _validate_existing(
    *,
    ...
    trust_protocol_sha256: str,
) -> dict[str, Any]:
    city, scenario, seed, candidate = identity
    if result_path.is_file() != tripinfo_path.is_file():
        raise RuntimeError(f"partial v51 result exists: {result_path.parent}")
    ...
    valid = (
        ...
        and result.get("execution_trust_region_protocol_sha256")
        == trust_protocol_sha256
        and result.get("tripinfo_evidence", {}).get("sha256")
        == _sha256(tripinfo_path)
        and bool(result.get("metrics", {}).get("ok", False))
    )
    if not valid:
        raise RuntimeError(f"existing v51 result failed validation: {result_path}")
    return result
...
def _run_one(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    existing = _validate_existing(
        ...
        identity=identity,
        trust_protocol_sha256=str(payload["trust_protocol_sha256"]),
    )
    ...
    if existing:
        ...
        result = run_execution_trust_region_candidate(
            trust_protocol_path=Path(str(payload["trust_protocol_path"])),
            ...
            failed_v50_audit_path=Path(str(payload["failed_v50_audit_path"])),
            expected_failed_v50_audit_sha256=str(
                payload["expected_failed_v50_audit_sha256"]
            ),
...
def run_shard(
    *,
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("v51 shard protocol changed")
    ...
    if summary_path.exists():
        raise FileExistsError(f"refusing to overwrite v51 shard audit: {summary_path}")
    trust_sha = _sha256(trust_protocol_path)
    tasks = []
        ...
        tasks.append(
            {
                ...
                "trust_protocol_path": str(trust_protocol_path),
                "trust_protocol_sha256": trust_sha,
                ...
                "failed_v50_audit_path": str(failed_v50_audit_path),
                "expected_failed_v50_audit_sha256": expected_failed_v50_audit_sha256,
    ...
    if not passed:
        raise RuntimeError(f"v51 shard coverage failed: {shard_index}")
    return payload

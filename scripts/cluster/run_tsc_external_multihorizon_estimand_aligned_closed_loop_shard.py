#!/usr/bin/env python3
"""Run one process-isolated shard of the frozen v81 closed-loop matrix."""
...
from cf_h2o.eval.traffic_signal_multihorizon_estimand_aligned_closed_loop_development import (
    ...
    run_multihorizon_estimand_aligned_closed_loop_development,
)
...
SHARD_PROTOCOL = "tsc-v83r79-multihorizon-estimand-aligned-process-shard-v1"
...
def _run_one(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not result:
        ...
        result = run_multihorizon_estimand_aligned_closed_loop_development(
            ...
            v80_protocol_path=Path(str(payload["v80_protocol_path"])),
            v80_audit_path=Path(str(payload["v80_audit_path"])),
            alignment_diagnostic_path=Path(
                str(payload["alignment_diagnostic_path"])
            ),
    ...
    *,
    ...
    for identity in identities:
        ...
        if (
            ...
            raise RuntimeError(f"completed v81 shard row changed: {result_path}")
    ...
    *,
        ...
        tasks.append(
            {
                ...
                "protocol_sha256": protocol_sha,
                "v80_protocol_path": str(v80_protocol_path),
                "v80_audit_path": str(v80_audit_path),
                "alignment_diagnostic_path": str(alignment_diagnostic_path),
    ...
    payload = {
        ...
        "artifact_model_sha256": {
            key: _sha256(Path(artifact_root) / key / "model.pkl")
    ...
    if not passed:
        raise RuntimeError(f"v81 shard coverage failed: {shard_index}")
    return payload

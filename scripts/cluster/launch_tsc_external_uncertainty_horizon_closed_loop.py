#!/usr/bin/env python3
"""Launch the frozen v83 matrix durably on six 192-core CPU nodes."""
...
LAUNCH_PROTOCOL = "v83r79-uncertainty-horizon-six-node-durable-launch-v1"
...
def build_specs(
    *,
    ...
    hashes: dict[str, Any] = {
        ...
        "v81_protocol": _sha256(PROJECT_ROOT / V81_PROTOCOL_RELATIVE),
        "v81_audit": _sha256(v81_audit_local),
    ...
    expected = {
        "v81_protocol": protocol["development_parent"]["v81_protocol"][
            "sha256"
        ],
        "v81_audit": protocol["development_parent"][
    ...
    if (
        ...
        or len(identities) != 110
        or not 1 <= int(workers) <= int(cpu_cores) <= 192
    ):
        raise ValueError("v83 launch authorization changed")
    wrapper = snapshot_root / "scripts/cluster/run_cfcmt_sumo122.sh"
    runner = (
        snapshot_root
        / "scripts/cluster/run_tsc_external_uncertainty_horizon_closed_loop_shard.py"
    )
    specs = []
    for shard_index, node in enumerate(NODES):
        ...
        specs.append(
            {
                "description": f"CFCMT v83 uncertainty-horizon shard {shard_index}",
                "project": "CFCMT",
                "cmd": command,
                "cwd": str(snapshot_root),
                "signature": f"CFCMT/v83r79/uncertainty-horizon/shard-{shard_index}",
                "resource_family": "CFCMT-v83-uncertainty-horizon-closed-loop-v1",
                "vram": 0,
                "ram_mb": int(ram_mb),
                "cpu": int(cpu_cores),
                "priority": "high",
...
def _sync_results(
    *,
    ...
    if local_results_root.exists():
        raise FileExistsError(f"refusing to overwrite local v83 results: {local_results_root}")
    ...
    for attempt in range(1, 6):
        ...
        if completed.returncode == 0:
            return attempt
        if attempt == 5:
            raise RuntimeError(f"v83 result sync failed: {completed.stderr[-4000:]}")
        time.sleep(2**attempt)
    raise AssertionError("unreachable v83 rsync retry state")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.transport_root.exists():
        raise FileExistsError("refusing to overwrite v83 launch evidence")
    ...
    try:
        ...
        def run(spec: Mapping[str, Any]) -> dict[str, Any]:
            ...
            returncode, stdout, run_stderr, durable = run_durable_remote_capture(
                ...
                run_id=f"cfcmt-v83-uncertainty-horizon-{node}-v1",
        ...
        if not all(row["returncode"] == 0 for row in payload["direct_results"]):
            raise RuntimeError("one or more durable v83 shards failed")
        ...
        payload["execution_mode"] = (
            "durable_six_physical_nodes_up_to_nineteen_workers_each"
        )

#!/usr/bin/env python3
"""Submit seven V150J conditional mechanism-prior targets."""
...
from cf_h2o.eval.traffic_signal_conditional_mechanism_prior import (
    DESIGN_PROTOCOL,
...
LAUNCH_PROTOCOL = "tsc-v150j-conditional-mechanism-prior-launch-v1"
CONFIG_RELATIVE = Path(
    "cf_h2o/config/traffic_signal_tsc_v150j_conditional_mechanism_prior.json"
)
MODULE = "cf_h2o.eval.traffic_signal_conditional_mechanism_prior"
SIGNATURE_PREFIX = "CFCMT/v150j/conditional-mechanism-prior-v1"
RESOURCE_FAMILY = "CFCMT-v150j-conditional-mechanism-prior"
RAM_MB = 8_192
...
def _validate_config(config: Mapping[str, Any]) -> None:
    ...
    if (
        config.get("protocol") != "tsc-v150j-conditional-mechanism-prior-v1"
        or config.get("redeveloped_after") != V150I_AGGREGATE_PROTOCOL
        ...
        or int(config.get("common_evaluation_reserve_budget", -1)) != TARGET_BUDGET
        ...
        or config.get("mechanism_design") != DESIGN_PROTOCOL
        ...
        raise ValueError("V150J frozen configuration changed")
...
def build_specs(
    *,
    ...
    v150i_rejection_remote: Path,
    v150i_rejection_sha256: str,
    ...
    specs = _build_rigid_specs(
        ...
        extra_target_args=(
            "--v150i-rejection",
            str(v150i_rejection_remote),
            "--v150i-rejection-sha256",
            str(v150i_rejection_sha256),
        ),
    )
    for spec in specs:
        spec["ram_mb"] = RAM_MB
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--v150i-rejection-local", type=Path, required=True)
    parser.add_argument("--v150i-rejection-remote", type=Path, required=True)
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite V150J launch or results")
    if not 1 <= int(args.cache_workers) <= 8:
        raise ValueError("V150J cache workers must be in [1, 8]")
    _validate_config(_read_json(PROJECT_ROOT / CONFIG_RELATIVE))

    authorization = _read_json(args.authorization_result_local)
    rejection = _read_json(args.v150i_rejection_local)
    ...
    if (
        ...
        raise ValueError("V150J requires passing V150C authorization")
    if (
    ...
    if fit.get("protocol") != FIT_RESULT_PROTOCOL or fit.get("city") != "jinan":
        raise ValueError("V150J requires the frozen V115 architecture fit")
    if (
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote V150J preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            f"remote V150J evidence identities changed: {remote_hashes}; {hash_stderr}"
        )
    specs = build_specs(
        snapshot_root=snapshot_root,
        authorization_result_remote=args.authorization_result_remote,
        authorization_sha256=expected_hashes[0],
        v150i_rejection_remote=args.v150i_rejection_remote,
        v150i_rejection_sha256=expected_hashes[1],
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v150j-conditional-mechanism-prior-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V150J target matrix")
    return 0

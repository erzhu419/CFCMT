#!/usr/bin/env python3
"""Submit the seven complete-city V146 OOF compatibility tasks."""
...
LAUNCH_PROTOCOL = "tsc-v146-multicity-oof-compatibility-gate-launch-v1"
CONFIG_RELATIVE = Path(
    "cf_h2o/config/traffic_signal_tsc_v146_multicity_oof_compatibility_gate.json"
)
MODULE = "cf_h2o.eval.traffic_signal_multicity_oof_compatibility_gate"
SIGNATURE_PREFIX = "CFCMT/v146/multicity-oof-compatibility-gate-v1"
RESOURCE_FAMILY = "CFCMT-v146-multicity-oof-compatibility-gate"
...
def _validate_config(config: Mapping[str, Any]) -> None:
    ...
    if (
        config.get("protocol")
        != "tsc-v146-multicity-oof-compatibility-gate-v1"
        ...
        or config.get("scored_adaptation_labels_used") is not False
        ...
        or float(rule.get("minimum_history_mean_improvement", -1.0)) != 0.0005
        or rule.get("source_mass")
        != "chance_corrected_oof_action_agreement"
        or int(gate.get("minimum_improving_cities", -1)) != 5
        or float(gate.get("maximum_city_regression", -1.0)) != 0.01
        or config.get("failure_rule")
        != "close_oof_compatibility_source_gate_without_threshold_retuning"
    ):
        raise ValueError("V146 frozen protocol configuration changed")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists() or args.local_output_root.exists():
        raise FileExistsError("refusing to overwrite V146 launch or local results")
    if not 1 <= int(args.cache_workers) <= 8 or not 1 <= int(args.fit_workers) <= 8:
        raise ValueError("V146 workers must be in [1, 8]")
    ...
    if fit.get("protocol") != FIT_RESULT_PROTOCOL or fit.get("city") != "jinan":
        raise ValueError("V146 requires the frozen V115 architecture fit")
    if (
        ...
        raise ValueError("V146 source cache audit changed")
    ...
    if int(code) != 0:
        raise RuntimeError(f"remote V146 preflight failed: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote V146 evidence identities changed: "
    ...
    specs = build_specs(
        ...
        module=MODULE,
        description_prefix="CFCMT V146 OOF compatibility target",
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v146-multicity-oof-compatibility-gate-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V146 OOF compatibility gate")
    return 0

#!/usr/bin/env python3
"""Submit the frozen V128 anchored source-null residual feasibility test."""
...
LAUNCH_PROTOCOL = "tsc-v128-anchored-source-null-residual-launch-v1"
...
def build_spec(
    *,
    ...
    return {
        "description": (
            "CFCMT V128 anchored source-null reference-residual feasibility"
        ),
        "project": "CFCMT",
        "cmd": command,
        "cwd": str(snapshot_root),
        "signature": SIGNATURE,
        "resource_family": "CFCMT-v128-anchored-source-null-residual",
        "vram": 0,
        "ram_mb": RAM_MB,
        "cpu": 20,
        "priority": "high",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError("refusing to overwrite V128 launch artifact")
    if not 1 <= int(args.cache_workers) <= 20:
        raise ValueError("V128 cache workers must be in [1, 20]")
    if not 1 <= int(args.fold_workers) <= 20:
        raise ValueError("V128 fold workers must be in [1, 20]")
    if len(str(args.prediction_artifact_sha256)) != 64:
        raise ValueError("V128 prediction artifact SHA-256 is malformed")
    nodes = tuple(value.strip() for value in args.nodes.split(",") if value.strip())
    if not nodes or len(set(nodes)) != len(nodes):
        raise ValueError("V128 execution nodes must be nonempty and unique")
    if not set(nodes).issubset(ALLOWED_NODES):
        raise ValueError("V128 execution nodes include a non-admitted CPU node")
    ...
    if (
        ...
        raise ValueError("V128 selector cache audit changed")
    ...
    for node in nodes:
        ...
        if int(code) != 0:
            raise RuntimeError(f"remote V128 preflight failed on {node}: {stderr}")
    ...
    if int(hash_code) != 0 or remote_hashes != expected_hashes:
        raise RuntimeError(
            "remote V128 evidence identities changed: "
    ...
    completed = subprocess.run(
        [
            ...
            "--json",
            "--intent-label",
            "CFCMT-v128-anchored-source-null-reference-residual-v1",
        ],
    ...
    if completed.returncode != 0:
        raise RuntimeError("scheduler rejected V128 source-null residual")
    return 0

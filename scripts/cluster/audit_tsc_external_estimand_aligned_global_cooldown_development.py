#!/usr/bin/env python3
"""Audit v52 estimand-aligned development and freeze or reject its selector."""
...
from cf_h2o.eval.traffic_signal_external_estimand_aligned_global_cooldown_development import (  # noqa: E402
...
from scripts.cluster.audit_tsc_external_execution_trust_region_development import (  # noqa: E402
    AUDIT_PROTOCOL as V51_AUDIT_PROTOCOL,
    REJECTION_DECISION as V51_REJECTION_DECISION,
)
from scripts.cluster.audit_tsc_external_policy_horizon_expanded_development import (  # noqa: E402
    select_city,
)
from scripts.cluster.launch_tsc_external_estimand_aligned_global_cooldown_development import (  # noqa: E402
    LAUNCH_PROTOCOL,
    NODES,
)
from scripts.cluster.run_tsc_external_estimand_aligned_global_cooldown_development_shard import (  # noqa: E402
    SHARD_PROTOCOL,
)


AUDIT_PROTOCOL = "tsc-v52r48-estimand-aligned-global-cooldown-selection-audit-v1"
ADVANCE_DECISION = "authorize_frozen_v52_selector_for_new_untouched_validation"
REJECTION_DECISION = "reject_v52_and_preserve_new_validation_and_prospective_seeds"
...
def _phase_references(
    *,
    ...
    errors = []
    frozen = dict(failed_v51.get("phase_references", {}))
    for seed in sorted(expected_seeds):
        row = frozen.get(str(seed))
    ...
    if missing:
        errors.append(f"missing v51 phase references: {missing}")
...
def audit_estimand_aligned_global_cooldown_development(
    *,
    results_root: Path,
    ...
    hashes = {
        "global_protocol": _sha256(global_protocol_path),
        "v51_protocol": _sha256(v51_protocol_path),
        "adaptation_protocol": _sha256(adaptation_protocol_path),
        "pressure_protocol": _sha256(pressure_protocol_path),
        "failed_v51_audit": _sha256(failed_v51_audit_path),
    ...
    launch_gate = {
        ...
        "global_protocol_matches": protocol.get("protocol") == PROTOCOL,
        "global_protocol_hash_matches": launch_hashes.get("global_protocol")
        == hashes["global_protocol"],
        "failed_v51_audit_matches": failed_v51.get("protocol")
        in {FAILED_V51_AUDIT_PROTOCOL, V51_AUDIT_PROTOCOL}
        and failed_v51.get("status") == "PASS"
        and failed_v51.get("decision")
        in {FAILED_V51_DECISION, V51_REJECTION_DECISION}
        and hashes["failed_v51_audit"]
        == str(protocol["failed_v51_selection_audit"]["sha256"])
        == str(launch_hashes.get("failed_v51_audit")),
        "v51_protocol_hash_matches": hashes["v51_protocol"]
        == str(protocol["parent_execution_trust_region_protocol"]["sha256"])
        == str(failed_v51["trust_protocol_sha256"])
        == str(launch_hashes.get("v51_protocol")),
        "adaptation_protocol_hash_matches": hashes["adaptation_protocol"]
        == str(protocol["candidate_grid_protocol"]["sha256"])
        == str(v51["candidate_grid_protocol"]["sha256"])
    ...
    for shard_index in range(len(NODES)):
        ...
        passed = bool(
            ...
            and shard.get("global_protocol_sha256") == hashes["global_protocol"]
            and shard.get("adaptation_protocol_sha256")
            == hashes["adaptation_protocol"]
            and shard.get("failed_v51_audit_sha256") == hashes["failed_v51_audit"]
        ...
        for row in rows:
            ...
            if key in shard_rows:
                raise ValueError(f"duplicate v52 shard identity: {key}")
    ...
    for _, identity_scenario, seed, candidate in identities:
        ...
        if not result_path.is_file():
            errors.append(f"missing v52 result: {identity_scenario}/{seed}/{candidate}")
            continue
        ...
        row_gate = {
            ...
            == "estimand_aligned_global_cooldown_development",
            "identity_matches": (
                result.get("city"),
                result.get("scenario"),
            ...
            "candidate_spec_matches": result.get("global_cooldown_candidate")
            ...
            "global_protocol_hash_matches": result.get(
                "global_cooldown_protocol_sha256"
            )
            == hashes["global_protocol"],
            "v51_protocol_hash_matches": result.get(
                "parent_v51_protocol_sha256"
            )
            == hashes["v51_protocol"],
            ...
            == hashes["adaptation_protocol"],
            "failed_v51_hash_matches": result.get(
                "failed_v51_selection_audit_sha256"
            )
            == hashes["failed_v51_audit"],
            ...
            "global_cooldown_contract_matches": metrics.get(
                "residual_deployment", {}
            ).get("global_cooldown_contract")
        ...
        if not row_gate["passed"]:
            errors.append(f"v52 row gate failed: {identity_scenario}/{seed}/{candidate}")
        safety = {
            ...
            paired_rows.append(
                {
                    ...
                    "method_evidence_source": "v52_remote_tripinfo_hash_bound_result",
                    "phase_evidence_source": "hash_frozen_v51_phase_reference",
                }
            )

    candidates = [
        asdict(value)
        for value in global_cooldown_candidates(protocol, adaptation)
    ]
    ...
    city_selection = {
        ...
        "jinan": {
            ...
            "grid": [],
            "frozen_without_v52_rollouts": True,
        },
    }
    active_cities = (
        [city]
    ...
    return {
        ...
        "decision": decision,
        "claim_boundary": (
            "Post-v51 estimand-alignment development over the same ten disclosed "
            "development seeds and the frozen v8 conversion. It is neither untouched "
            "validation nor prospective confirmation."
        ),
        "preview": preview,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.out.exists():
        raise FileExistsError(f"refusing to overwrite v52 selection audit: {args.out}")
    payload = audit_estimand_aligned_global_cooldown_development(

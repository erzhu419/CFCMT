#!/usr/bin/env python3
"""Run one process-isolated shard of the v78 development matrix."""
...
from cf_h2o.eval.traffic_signal_external_topology_veto_closed_loop_development import (
    ...
    run_topology_veto_closed_loop_development,
)


SHARD_PROTOCOL = "tsc-v78r74-topology-veto-development-process-shard-v2"
...
def shard_identities(
    ...
    if shard_count < 1 or not 0 <= shard_index < shard_count:
        raise ValueError("invalid v78 shard index/count")
    return tuple(
        identity
...
def _validate_existing(
    *,
    ...
    if result_path.is_file() != tripinfo_path.is_file():
        raise RuntimeError(f"partial v78 result exists: {result_path.parent}")
    ...
    if not (
        ...
        and result.get("topology_veto_freeze_result_sha256")
        == hashes["topology_veto_freeze_result_sha256"]
        and result.get("topology_veto_joint_audit_sha256")
        == hashes["topology_veto_joint_audit_sha256"]
        and result.get("tripinfo_evidence", {}).get("sha256")
        == _sha256(tripinfo_path)
        and bool(result.get("metrics", {}).get("ok", False))
    ):
        raise RuntimeError(f"existing v78 result is invalid: {result_path}")
    return result
...
def _run_one(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not result:
        ...
        result = run_topology_veto_closed_loop_development(
...
def run_shard(
    *,
    ...
    if protocol.get("protocol") != PROTOCOL:
        raise ValueError("v78 shard protocol changed")
    ...
    if summary_path.exists():
        ...
        if (
            ...
            raise RuntimeError(f"completed v78 shard is invalid: {summary_path}")

#!/usr/bin/env python3
"""Submit the 48-run post-hoc v43 deployment-mechanism diagnostic matrix."""
...
from cf_h2o.eval.traffic_signal_external_closed_loop_diagnostic import (  # noqa: E402
    ANALYSIS_PROTOCOL,
    POLICIES,
    RESULT_PROTOCOL,
)
...
LAUNCH_PROTOCOL = "v43r39-external-closed-loop-diagnostic-submission-v1"
...
def build_specs(
    *,
    ...
    if (
        ...
        raise ValueError("offline evidence does not authorize diagnostic reuse")
    ...
    if len(combinations) != 4 * 2 * 6 or len(set(combinations)) != len(combinations):
    ...
    for index, (scenario, seed, policy) in enumerate(combinations):
        ...
        if local_result.exists():
            raise FileExistsError(
                f"refusing to overwrite diagnostic result: {local_result}"
            )
        command = shlex.join(
            [
                str(wrapper),
                "-m",
                "cf_h2o.eval.traffic_signal_external_closed_loop_diagnostic",
                "--protocol-spec",
                str(protocol_path),
        ...
        specs.append(
            {
                "description": (
                    f"CFCMT post-hoc deployment diagnostic {scenario} "
                    f"seed {seed} {policy}"
                ),
                "project": "CFCMT",
                "cmd": command,
                ...
                "signature": (
                    f"CFCMT/v43r39/closed-loop-diagnostic-v1/"
                    f"{scenario}/seed-{seed}/{policy}"
                ),
                "resource_family": "CFCMT-external-closed-loop-diagnostic-v1",
                "vram": 0,
                "ram_mb": int(ram_mb),
                "cpu": int(cpu_cores),
                "priority": "high",
    ...
    if args.submit:
        completed = subprocess.run(
            [
                ...
                "--json",
                "--intent-label",
                "CFCMT-v43r39-deployment-diagnostic",
            ],
        ...
        if completed.returncode != 0:
            ...
            raise RuntimeError("closed-loop diagnostic submission failed")
    _atomic_json(args.out, payload)
    print(
        json.dumps(
            {

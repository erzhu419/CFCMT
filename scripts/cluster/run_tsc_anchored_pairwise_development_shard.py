#!/usr/bin/env python3
"""Run one independently placed v39 anchored-pairwise development target."""
...
MODULE = "cf_h2o.eval.traffic_signal_anchored_pairwise_development"
...
def run_shard(
    *,
    ...
    if any(path.exists() for path in (result_path, log_path, launch_path, shard_path)):
        raise FileExistsError(f"refusing to overwrite v39 shard: {target_root}")
    command = [
        sys.executable,
        "-m",
        MODULE,
    ...
    _atomic_json(
        launch_path,
        {
            "protocol": "v39br35-single-target-anchored-pairwise-launch-v1",
    ...
    payload: dict[str, Any] = {
        "protocol": "v39br35-single-target-anchored-pairwise-shard-v1",
        "hostname": socket.gethostname(),
        "target": target,
        "started_at_utc": started_at,
    ...
    if completed.returncode == 0 and result_path.exists():
        ...
        valid = (
            ...
            and result.get("model_families") == list(BASE_FAMILIES)
            and set(candidates) == set(CANDIDATE_KEYS)
            and int(evaluation.get("candidate_count", -1)) == len(CANDIDATE_KEYS)
            and int(evaluation.get("evaluation_group_count", -1)) > 0
            and int(evaluation.get("excluded_group_count", -1))
            == TARGET_GROUP_BUDGET
        )
        if valid:
            payload.update(
    ...
    if not payload["passed"]:
        raise RuntimeError(f"v39 anchored-pairwise shard failed: {payload}")
    return payload

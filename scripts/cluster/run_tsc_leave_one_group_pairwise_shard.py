#!/usr/bin/env python3
"""Run and validate one v37/r33 leave-one-group-out target shard."""
...
MODULE = "cf_h2o.eval.traffic_signal_leave_one_group_pairwise_screen"
...
def run_shard(
    *,
    ...
    if any(path.exists() for path in (result_path, log_path, launch_path, shard_path)):
        raise FileExistsError(f"refusing to overwrite LOGO shard: {target_root}")
    command = [
        sys.executable,
        "-m",
        MODULE,
    ...
    _atomic_json(
        launch_path,
        {
            "protocol": "v37r33-single-target-logo-launch-v1",
    ...
    payload: dict[str, Any] = {
        "protocol": "v37r33-single-target-logo-shard-v1",
        "hostname": socket.gethostname(),
        "target": target,
        "started_at_utc": started_at,
    ...
    if completed.returncode == 0 and result_path.exists():
        ...
        if (
            ...
            and int(result.get("final_fit_group_count", -1)) == 60
            and len(result.get("selected_group_ids", ())) == 60
            and len(result.get("logo_fits", ())) == 60
            and len(result.get("logo_rows", ())) == 60
    ...
    if not payload["passed"]:
        raise RuntimeError(f"LOGO shard failed: {payload}")
    return payload

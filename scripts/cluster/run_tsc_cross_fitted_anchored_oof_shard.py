#!/usr/bin/env python3
"""Run one independently placed v40 cross-fitted anchored OOF target."""
...
from cf_h2o.eval.traffic_signal_cross_fitted_anchored_selection import FOLD_COUNT


MODULE = "cf_h2o.eval.traffic_signal_cross_fitted_anchored_development"
...
def run_shard(
    *,
    ...
    if any(path.exists() for path in (result_path, log_path, launch_path, shard_path)):
        raise FileExistsError(f"refusing to overwrite v40 shard: {target_root}")
    command = [
        sys.executable,
        "-m",
        MODULE,
    ...
    _atomic_json(
        launch_path,
        {
            "protocol": "v40r36-single-target-cross-fitted-anchored-launch-v1",
    ...
    payload: dict[str, Any] = {
        "protocol": "v40r36-single-target-cross-fitted-anchored-shard-v1",
        "hostname": socket.gethostname(),
        "target": target,
        "started_at_utc": started_at,
    ...
    if completed.returncode == 0 and result_path.exists():
        ...
        fold_contract = all(
            len(fold.get("training_group_ids", ())) == 48
            and len(fold.get("validation_group_ids", ())) == GROUPS_PER_FOLD
            and set(fold.get("candidate_metrics", {})) == set(CANDIDATE_KEYS)
            for fold in folds
        )
        valid = (
            ...
            and all(
                len(row.get("fold_regrets", ())) == FOLD_COUNT
    ...
    if not payload["passed"]:
        raise RuntimeError(f"v40 cross-fitted anchored shard failed: {payload}")
    return payload

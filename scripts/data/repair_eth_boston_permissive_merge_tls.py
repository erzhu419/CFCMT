#!/usr/bin/env python3
"""Remove one collision-observed permissive Boston merge movement."""
...
TLS_ID = "65200956"
JUNCTION_ID = "65200956"
MERGE_EDGE_ID = "846300601#2"
...
def _validate_declared_merge_topology(network: Path) -> dict[str, Any]:
    ...
    if len(programs) != 1:
        raise ValueError("declared Boston permissive-merge TLS changed")
    phases = programs[0].findall("phase")
    if len(phases) <= 2:
        raise ValueError("declared Boston permissive-merge phases changed")
    phase_zero = str(phases[0].get("state", ""))
    phase_two = str(phases[2].get("state", ""))
    if (
        len(phase_zero) <= 7
        or len(phase_two) <= 11
        or phase_zero[3] != "G"
        ...
        raise ValueError("declared Boston permissive-merge phase service changed")

    edges = [row for row in root.iter("edge") if row.get("id") == MERGE_EDGE_ID]
    if len(edges) != 1 or edges[0].get("from") != JUNCTION_ID:
        raise ValueError("declared Boston permissive-merge edge changed")
    lanes = [row for row in edges[0].findall("lane") if row.get("id") == MERGE_LANE_ID]
    if len(lanes) != 1 or lanes[0].get("index") != "0":
        raise ValueError("declared Boston permissive-merge lane changed")
    connections = list(root.iter("connection"))
    for identity in (RIGHT_TURN_CONNECTION, STRAIGHT_CONNECTION):
        if sum(_matches(row.attrib, identity) for row in connections) != 1:
            raise ValueError("declared Boston permissive-merge connection changed")
    junctions = [row for row in root.iter("junction") if row.get("id") == JUNCTION_ID]
    if len(junctions) != 1 or str(junctions[0].get("intLanes", "")) != "":
        raise ValueError("declared Boston permissive-merge junction changed")
    return {
        "tls_id": TLS_ID,
        "junction_id": JUNCTION_ID,
        "merge_edge_id": MERGE_EDGE_ID,
        "merge_lane_id": MERGE_LANE_ID,
        "right_turn_connection": dict(RIGHT_TURN_CONNECTION),
        "straight_connection": dict(STRAIGHT_CONNECTION),
        "phase_zero_state": phase_zero,
        "phase_two_state_before": phase_two,
        "collision_evidence": {
            "time_sec": 9735,
            "right_turn_vehicle": "3610",
            "straight_vehicle": "4793",
        },
    }


def validate_permissive_merge_manifest(manifest: Mapping[str, Any]) -> None:
    if manifest.get("protocol") != PACKAGE_PROTOCOL:
        raise ValueError(
            f"unexpected Boston v10 package protocol: {manifest.get('protocol')}"
        )
    predecessor = dict(manifest)
    predecessor["protocol"] = BASE_PACKAGE_PROTOCOL
    validate_ramp_merge_manifest(predecessor)
    section = dict(manifest.get("permissive_merge_tls_repair", {}))
    ...
    if (
        ...
        or topology.get("right_turn_connection") != RIGHT_TURN_CONNECTION
        or topology.get("straight_connection") != STRAIGHT_CONNECTION
        or dict(manifest.get("gates", {})).get(
            "boston_phase2_permissive_merge_removed"
        )
        is not True
    ):
        raise ValueError("Boston permissive merge repair manifest is incomplete")
...
def repair_package(
    *,
    ...
    if observed_sha256 != expected_base_manifest_sha256:
        raise ValueError("Boston v9 base package manifest identity changed")
    base_manifest = json.loads(base_manifest_path.read_text(encoding="utf-8"))
    validate_ramp_merge_manifest(base_manifest)
    if base_manifest.get("passed") is not True:
        raise ValueError("Boston v9 base package is not admitted for remediation")
    ...
    try:
        ...
        checks = {
            ...
            "alternative_right_turn_service_retained_in_phase_zero": (
                topology["phase_zero_state"][3] == "G"
            ),
        ...
        gates["boston_phase2_permissive_merge_removed"] = all(checks.values())
        payload["gates"] = gates
        ...
        instantiation["transformation"] = (
            ...
            + "_plus_phase2_permissive_single_lane_merge_removal"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(json.dumps(payload["permissive_merge_tls_repair"], indent=2, sort_keys=True))

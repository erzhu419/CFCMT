#!/usr/bin/env python3
"""Yield one Boston protected turn that merges with an uncontrolled mainline."""
...
PROTOCOL = "cfcmt-eth-boston-protected-uncontrolled-merge-yield-repair-v1"
PACKAGE_PROTOCOL = "cfcmt-eth-boston-complete-published-package-v11"
TLS_ID = "joinedS_1100"
JUNCTION_ID = "cluster_73140898_73175840_765364560"
MERGE_EDGE_ID = "-34665460#50"
MERGE_LANE_ID = "-34665460#50_0"
...
def _validate_declared_merge_topology(network: Path) -> dict[str, Any]:
    ...
    if len(programs) != 1:
        raise ValueError("declared Boston protected/uncontrolled TLS changed")
    phases = programs[0].findall("phase")
    if len(phases) <= 13:
        raise ValueError("declared Boston protected/uncontrolled phases changed")
    collision_phase = str(phases[12].get("state", ""))
    clearance_phase = str(phases[13].get("state", ""))
    if (
        len(collision_phase) <= 10
        or len(clearance_phase) <= 10
        or collision_phase[10] != "G"
        or clearance_phase[10] != "y"
    ):
        raise ValueError("declared Boston protected-turn service changed")

    edges = [row for row in root.iter("edge") if row.get("id") == MERGE_EDGE_ID]
    if len(edges) != 1 or edges[0].get("from") != JUNCTION_ID:
        raise ValueError("declared Boston protected/uncontrolled merge edge changed")
    lanes = [row for row in edges[0].findall("lane") if row.get("id") == MERGE_LANE_ID]
    if len(lanes) != 1 or lanes[0].get("index") != "0":
        raise ValueError("declared Boston protected/uncontrolled merge lane changed")

    connections = list(root.iter("connection"))
    for identity in (CONTROLLED_LEFT_CONNECTION, UNCONTROLLED_STRAIGHT_CONNECTION):
        if sum(_matches(row.attrib, identity) for row in connections) != 1:
            raise ValueError("declared Boston protected/uncontrolled connection changed")
    junctions = [row for row in root.iter("junction") if row.get("id") == JUNCTION_ID]
    if (
        len(junctions) != 1
        or junctions[0].get("type") != "traffic_light_right_on_red"
        or str(junctions[0].get("intLanes", "")) != ""
    ):
        raise ValueError("declared Boston protected/uncontrolled junction changed")
    return {
        "tls_id": TLS_ID,
        "junction_id": JUNCTION_ID,
        "merge_edge_id": MERGE_EDGE_ID,
        "merge_lane_id": MERGE_LANE_ID,
        "controlled_left_connection": dict(CONTROLLED_LEFT_CONNECTION),
        "uncontrolled_straight_connection": dict(UNCONTROLLED_STRAIGHT_CONNECTION),
        "collision_phase_state_before": collision_phase,
        "clearance_phase_state": clearance_phase,
        "collision_evidence": {
            "time_sec": 11871,
            "phase_index": 12,
            "traffic_light_state": collision_phase,
            "collider_vehicle": "38770",
            "victim_vehicle": "37256",
            "lane_id": MERGE_LANE_ID,
        },
    }


def validate_protected_uncontrolled_merge_manifest(
    manifest: Mapping[str, Any],
) -> None:
    if manifest.get("protocol") != PACKAGE_PROTOCOL:
        raise ValueError(
            f"unexpected Boston v11 package protocol: {manifest.get('protocol')}"
        )
    predecessor = dict(manifest)
    predecessor["protocol"] = BASE_PACKAGE_PROTOCOL
    validate_permissive_merge_manifest(predecessor)
    section = dict(manifest.get("protected_uncontrolled_merge_tls_repair", {}))
    ...
    if (
        ...
        or topology.get("controlled_left_connection") != CONTROLLED_LEFT_CONNECTION
        or topology.get("uncontrolled_straight_connection")
        != UNCONTROLLED_STRAIGHT_CONNECTION
        or dict(manifest.get("gates", {})).get(
            "boston_protected_uncontrolled_merge_yield_repaired"
        )
        is not True
    ):
        raise ValueError(
            "Boston protected/uncontrolled merge repair manifest is incomplete"
        )
...
def repair_package(
    *,
    ...
    if observed_sha256 != expected_base_manifest_sha256:
        raise ValueError("Boston v10 base package manifest identity changed")
    base_manifest = json.loads(base_manifest_path.read_text(encoding="utf-8"))
    validate_permissive_merge_manifest(base_manifest)
    if base_manifest.get("passed") is not True:
        raise ValueError("Boston v10 base package is not admitted for remediation")
    ...
    try:
        ...
        checks = {
            ...
            "yellow_clearance_retained": topology["clearance_phase_state"][10] == "y",
        ...
        gates["boston_protected_uncontrolled_merge_yield_repaired"] = all(
            checks.values()
        )
        payload["gates"] = gates
        ...
        instantiation["transformation"] = (
            ...
            + "_plus_protected_uncontrolled_single_lane_merge_yield_repair"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    section = payload["protected_uncontrolled_merge_tls_repair"]
